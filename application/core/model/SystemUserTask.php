...
class SystemUserTask extends ModelBasic
{
    ...
    {
        $isComplete=false;
        $SumPayPrice=self::getDb('store_order')->where('paid',1)->where('is_del',0)->where('uid',$uid)->sum('pay_price');
    ...
    {
        $isComplete=false;
        $countPay=self::getDb('store_order')->where('paid',1)->where('is_del',0)->where('uid',$uid)->count();
    ...
    {
        $isComplete=false;
        $sumNumber=UserBill::where(['uid'=>$uid,'category'=>'integral','pm'=>1])->where('type','in',['system_add','sign','gain'])->sum('number');
    ...
    public static function getTashList($level_id,$uid=0,$level=null,$expire=1400)
    {
       ...
       return ['reach_count'=>$reach_count,'is_clear'=>$is_clear,'task'=>self::tidyTask($list,$uid,$is_clear,$startTime),'star_time'=>$startTime,'uid'=>$uid,'level'=>$level];
    }
    ...
    public static function getTaskComplete($level_id,$uid,$isCount=false)
    {
        ...
        if($taskIdsCount){
            if($isCount){
                ...
                $finishCount=self::getDb('user_task_finish')->group('task_id')->where('status',0)->where('uid',$uid)->where('task_id','in',$taskIds)->count();
            }
