...
class PaymentBehavior
{
    ...
    public static function wechatPaySuccessProductr($orderId, $notify)
    {
        try{
            ...
            return StoreOrderRoutineModel::paySuccess($orderId,'routine');
        }catch (\Exception $e){
            return false;
        }
    }
     ...
     * @return bool
     */
    public static function wechatPaySuccessShop($orderId, $notify)
    {
        try{
            if(ShopOrder::be(['order_id'=>$orderId,'paid'=>1])) return true;
            return ShopOrder::paySuccess($orderId,'routine');
        }catch (\Exception $e){
            return false;
        }
    }
     ...
     * vip礼品卡订单支付成功后 小程序
     */
    public static function wechatPaySuccessCard($orderId,$notify){
        try{
            if(StoreOrderRoutineModel::be(['order_id'=>$orderId,'paid'=>1])) return true;
            return  StoreOrderRoutineModel::paySuccessCard($orderId,'routine');
        }catch (\Exception $e){
            return false;
        }
    }
