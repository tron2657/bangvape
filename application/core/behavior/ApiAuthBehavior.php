...
class ApiAuthBehavior implements BehaviorIntterface
{
    ...
    public function run(){
        ...
        if(Cache::has(ApiLogs::AB_API_INFO . $hash)){
            ...
            $apiInfo = ApiMenus::get(['hash' => $hash]);
    ...
    public function checkLogin($needLogin)
    {
        ...
        if(!$userInfo || !isset($userInfo)) return JsonService::returnData(ReturnCode::ERROR,'用户信息获取失败,没有这样的用户!');
        if(!$userInfo->status) return JsonService::returnData(ReturnCode::USER_STATUS_ERROR,'您已被禁止登录');
    }
