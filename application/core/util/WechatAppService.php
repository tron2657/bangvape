...
class WechatAppService
{
    ...
    {
        $appPayment = SystemConfigService::more(['pay_weixin_app_appid','pay_weixin_app_mchid','pay_weixin_app_key','pay_weixin_app_open']);
        $config=[];
        if(isset($appPayment['pay_weixin_app_open']) && $appPayment['pay_weixin_app_open'] == 1){
            $config['payment'] = [
                'app_id'=>$appPayment['pay_weixin_app_appid'],
                'mch_id'=>$appPayment['pay_weixin_app_mchid'],
                'key'=>$appPayment['pay_weixin_app_key'],
                'notify_url'=>Request::instance()->domain().Url::build('ebapi/Notify/notifyApp'),
                'device_info'=>'WEB',
            ];
        }else{
            exception('未开通App微信支付!');
            exit;
        }
        return $config;
    }
     ...
     */
    public static function paymentService($notify_url='')
    {
        return self::application($notify_url)->payment;
    }
...
//        ])->getBody()->getContents());
    ...
    public static function handleNotify()
    {
        self::paymentService()->handleNotify(function($notify, $successful){
            if($successful && isset($notify->out_trade_no)){
                ...
                return HookService::listen('wechat_pay_success',$notify,null,true,PaymentBehavior::wechatPaySuccess($notify));
            }
        });
    }

    public static function handleNotifyShop()
    {
        self::paymentService()->handleNotify(function($notify, $successful){
            if($successful && isset($notify->out_trade_no)){
                WechatMessage::setOnceMessage($notify,$notify->openid,'payment_success',$notify->out_trade_no);
                return HookService::listen('wechat_pay_success',$notify,null,true,PaymentBehavior::wechatPaySuccessProductShop($notify->out_trade_no,$notify));
            }
        });
    }
     ...
     * 支付系统的内容监听
     */
    public static function handleNotifyNew()
    {
        self::paymentService()->handleNotify(function($notify, $successful){
            if($successful && isset($notify->out_trade_no)){
                WechatMessage::setOnceMessage($notify,$notify->openid,'payment_success',$notify->out_trade_no);
                return HookService::listen('wechat_pay_success',$notify,null,true,UserOrder::beforeOrder($notify->out_trade_no,'weixin',$notify));
            }
        });
    }
