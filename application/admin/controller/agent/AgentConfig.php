...
class AgentConfig extends AuthController
{
    ...
    public function index()
    {
        ...
        if($agent_config['agent_way']==3){
            ...
            if($agent_config['goods_id']){
                if (isset($agent_config['goods_type']) && $agent_config['goods_type']=='column_goods') {
                    $column = db('column_text')->where('id',$agent_config['goods_id'])->where('status',1)->where('is_show',1)->find();
                    if ($column) {
                        $goods_info = $column;
                        $goods_info['store_name'] = $column['name'];
                    }
                } else {
                    $store_product=StoreProduct::where('id',$agent_config['goods_id'])->where('is_del',0)->find();
                    if (!empty($store_product)) {
                        $goods_info = $store_product->toArray();
                    }
                }
        ...
        if($request->isPost()){
            ...
            foreach ($post as $k=>&$v){
                if($k=='agent_tixian_config_rules'){
                    ...
                    $last = SystemConfig::getValue('agent_tixian_config_rules');
                    if($v == $last){
                        $some =1;
                    }else{$some = 2;}
                }
            ...
            if($some == 2){
                db('all_agreement')->where('id',5)->update(['update_time'=>time()]);
            }
            return JsonService::successful('修改成功');
        }
    }
    ...
    public function saveXieyi()
    {
        ...
        if($request->isPost()){
            ...
            db('all_agreement')->where('id',3)->update(['update_time'=>time()]);
            return JsonService::successful('修改成功');
        }
    }
    ...
    public function saveIncomeStatement()
    {
        ...
        if($request->isPost()){
            ...
            db('all_agreement')->where('id',4)->update(['update_time'=>time()]);
            return JsonService::successful('修改成功');
        }
    }
