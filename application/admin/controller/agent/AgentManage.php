...
//    }
    ...
    public function auditSeller()
    {
        ...
        if($data['status']==1){
            ...
            $phone=SystemConfig::getValue('shop_phone');
            RoutineTemplateService::sendTemplate(WechatUser::getMiniOpenId($data['uid']),RoutineTemplateService::AGENT_MANAGE, [
                'phrase3'=>['value'=>'审核通过'],
                'name1'=>['value'=>'分销商'],
                'phone_number2'=>['value'=>$phone],
                'thing4'=>['value'=>'分销商'],
            ],'','/packageB/promotion-center/index');
        }else{
            ...
            if($set['status']==1){
                $message_id=Message::sendMessage($data['uid'],0,$template,1,$set['title'],1,'','promotion_center');
                $read_id=MessageRead::createMessageRead($data['uid'],$message_id,$set['popup'],1);
            }
            if($set['sms']==1&&$set['status']==1){
                $account=User::where('uid',$data['uid'])->value('phone');
