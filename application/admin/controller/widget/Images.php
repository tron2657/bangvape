...
//       $this->assign(compact('typearray'));
    ...
    public function upload()
    {
        ...
        switch ($upload_type) {
            case 'Tencent_COS':
                if(!isset($_FILES[$fileName])) Json::fail('上传文件不存在!');
                ...
                if($result['result']==true){
                    ...
                    SystemAttachmentModel::attachmentAdd($file_info['Key'],$file_info['size'],$file_info['type'],$file_info['path'],$file_info['path'],$pid);
                    $info = array(
                        'code' =>200,
                        'msg'  =>'上传成功',
                        'src'  =>$file_info['path']
            ...
            case 'local':
            default:
                $res = Upload::image($fileName,'attach'.DS.date('Y').DS.date('m').DS.date('d'));
