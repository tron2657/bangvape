...
class User extends AuthController
{
    ...
    public function edit($uid)
    {
        ...
        $f[] = Form::input('email','邮箱',$user->getData('email'));
        ...
        $f[] = Form::radio('is_bus','是否商家',$user['is_bus'])->options([['value'=>1,'label'=>'是'],['value'=>0,'label'=>'否']]);
    ...
    public function update(Request $request, $uid)
    {
        ...
        if($data['email']){
            $email=UserModel::where('uid','neq',$uid)->where('email',$data['email'])->find();
            if($email){
                return Json::fail('该邮箱已存在');
            }
        }
        ...
        if($data['is_sell']==1){
            ...
            if($set['status']==1){
                $message_id=Message::sendMessage($uid,0,$set['template'],1,$set['title'],1,'','promotion_center');
        ...
        if($res2!==false){
            ...
            if(in_array('im',$open_list)){
                $code=$this->_getCode();
                ImModel::update_user_info($uid,$code);
            }
    ...
    public function update_score(){
        ...
        foreach ($post['uids'] as $val){
            ...
            if($set['status']==1){
                $message_id=Message::sendMessage($val,0,$template,1,$set['title'],1,'','my');
