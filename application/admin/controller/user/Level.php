...
class Level extends AuthController
{
    ...
    public function save($id=0)
    {
        ...
        try{
            ...
            if($id){
                ...
                //晋级用户组添加
                $map['bind_condition']=$id;
                $map['type']=3;
                $value['name']=$data['name'];
                $value['remark']=$data['explain'];
                db('group')->where($map)->update($value);
    ...
    public function delete($id=0)
    {
        if(SystemUserGrade::edit(['is_del'=>1],$id)){
            $map['bind_condition']=$id;
            $map['type']=3;
            db('group')->where($map)->update(['status'=>-1]);
    ...
    public function change_field(){
        ...
        if(empty($id)){
            ...
            if($data['type'] == 3){
                $result_all = db('all_agreement')->where('id',6)->update($datas);
            }
            if($data['type'] == 4){
                $result_all = db('all_agreement')->where('id',7)->update($datas);
            }
            if($data['type'] == 1){
                $result_all = db('all_agreement')->where('id',8)->update($datas);
            }
        }
