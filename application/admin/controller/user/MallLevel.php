...
use app\admin\model\system\SystemMallGrade;
 ...
 */
class MallLevel extends AuthController
{
    ...
    public function create($id=0)
    {
        ...
        if($id) $vipinfo=SystemMallGrade::get($id);

        $field[]= Form::input('name','等级名称',isset($vipinfo) ? $vipinfo->name : '')->col(Form::col(24));
        $field[]= Form::number('experience','消费金额上限',isset($vipinfo) ? $vipinfo->experience : 0)->min(0)->col(24);
        $field[]= Form::number('zhe','享受折扣',isset($vipinfo) ? $vipinfo->zhe : 0)->min(0)->col(24);
    ...
    public function save($id=0)
    {
        ...
        SystemMallGrade::beginTrans();
        try{
            //修改
            if($id){
                $res = SystemMallGrade::where('id',$id)->update($data);

                if($res!==false){
                    SystemMallGrade::commitTrans();
                    return JsonService::successful('修改成功');
                }else{
                    SystemMallGrade::rollbackTrans();
                    return JsonService::fail('修改失败');
                }
            }else{
                //新增
                $data['add_time']=time();
                if(SystemMallGrade::set($data)){
                    SystemMallGrade::commitTrans();
    ...
    public function get_system_vip_list()
    {
        ...
        return JsonService::successlayui(SystemMallGrade::getSytemList($where));
    }
    ...
    {
        if(SystemMallGrade::edit(['is_del'=>1],$id))
    ...
    public function set_show($is_show='',$id=''){
        ...
        $res=SystemMallGrade::where(['id'=>$id])->update(['is_show'=>(int)$is_show]);
        if($res){
    ...
    public function set_value($field='',$id='',$value=''){
        ...
        if(SystemMallGrade::where(['id'=>$id])->update([$field=>$value]))
