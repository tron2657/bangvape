 ...
 */
class Jifen extends AuthController
{
    ...
    public function dealtwo($item)
    {
        ...
        if(strpos($item['flag'],'one')!==false){
            $father['field'] = 'firstmax' ;
            $father['title'] = '最大' ;
            $father['edit'] = 'firstmax' ;
        }
        if(strpos($item['flag'],'two')!==false){
            $father['field'] = 'twomax' ;
            $father['title'] = '最大' ;
        ...
        if(strpos($item['flag'],'three')!==false){
            $father['field'] = 'threemax' ;
            $father['title'] = '最大' ;
            $father['edit'] = 'threemax' ;
        }
        if(strpos($item['flag'],'four')!==false){
            $father['field'] = 'fourmax' ;
            $father['title'] = '最大' ;
            $father['edit'] = 'fourmax' ;
        }
        if(strpos($item['flag'],'five')!==false){
            $father['field'] = 'fivemax' ;
            $father['title'] = '最大' ;
            $father['edit'] = 'fivemax' ;
        }
        return $father;
    ...
    public function create($id=0 )
    {
        ...
        if($id) $vipinfo=SystemJifen::get($id);
...
//        if(!$data['leixing']) return JsonService::fail('请输入经验值上限');
        ...
        SystemJifen::beginTrans();
        try{
            //修改
            if($id){
                if(SystemJifen::edit($data,$id)){
                    SystemJifen::commitTrans();
                    return JsonService::successful('修改成功');
                }else{
                    SystemJifen::rollbackTrans();
                    return JsonService::fail('修改失败');
                }
            }else{
                //新增
                $data['add_time']=time();
                if(SystemJifen::set($data)){
                    SystemJifen::commitTrans();
    ...
    public function get_system_vip_list()
    {
        ...
        return JsonService::successlayui(SystemJifen::getSytemList($where));
    }
    ...
    {
        if(SystemJifen::edit(['is_del'=>1],$id))
    ...
    public function set_show($is_show='',$id=''){
        ...
        $res=SystemJifen::where(['id'=>$id])->update(['is_show'=>(int)$is_show]);
        if($res){
    ...
    public function set_value($field='',$id='',$value=''){
        ...
        if(SystemJifen::where(['id'=>$id])->update([trim($field)=>trim($value)])){
            db('cache_flush')->where('name','cacheFlush_sign_signnum')->update(['update_time'=>time()]);
    ...
    public function get_jifen_log_list()
    {
        $where=UtilService::getMore([
            ['page',0],
            ['limit',10],
        ...
        return JsonService::successlayui(RenwuJiafenLog::getLogList($where));
    }
