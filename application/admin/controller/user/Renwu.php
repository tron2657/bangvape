 ...
 */
class Renwu extends AuthController
{
    ...
    public function create($id=0 ,$type=1)
    {
        ...
        if($type == 1){
            ...
            $field[]= Form::select('leixing','任务类型',empty($vipinfo->leixing) ? 0 : $vipinfo->leixing )->options([['label'=>'自定义任务','value'=>'4']  ])->col(24);
        }
        ...
        $field[]= Form::frameImageOne('icon','图标(90*90)',Url::build('admin/widget.images/index',array('fodder'=>'icon')),isset($vipinfo) ? $vipinfo->icon : '')->icon('image')->width('100%')->height('500px');
    ...
    public function get_system_vip_list()
    {
        ...
        return JsonService::successlayui(SystemRenwu::getSytemList($where));
    }
    ...
    public function set_value($field='',$id='',$value=''){
        ...
        if($res){
            db('cache_flush')->where('name','cacheFlush_sign_renwulist')->update(['update_time'=>time()]);
