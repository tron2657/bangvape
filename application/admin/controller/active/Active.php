...
use app\admin\model\active\Active as EventModel;
use app\admin\model\active\ActiveCheck as EventCheck;
use app\admin\model\active\ActiveEnroller  as EventEnroller;
use app\admin\model\active\ActiveField as ActiveEnroller ;
use app\admin\model\active\ActiveMessage;
...
class Active extends AuthController
{
    ...
    public function event_detail(){
        ...
        $event['enroll_end_time']=$event['enroll_end_time']?date('Y-m-d H:i:s',$event['enroll_end_time']):'';
        $cate=Cate::get_check_id();
        $group=db('active_bind_group')->where(['event_id'=>$id,'status'=>1])->value('group');
        if($group){
            $event['g_id']=$group;
            $g_id=explode(',',$group);
            $g_name=Group::where(['id'=>['in',$g_id]])->field('name')->select()->toArray();
    ...
    public function save_excel(){
        ...
        PHPExcelService::setExcelHeader($header)
            ->setExcelTile('约酒报名表导出',' ',' 生成时间：'.date('Y-m-d H:i:s',time()))
