...
//        dump($data);exit;
    ...
    public function edit_forum_power(){
        ...
        if($data['audit_id']&&$data['audit']==''){
            $data['audit']=1;
        }
        if($data['audit']==1&&mb_strlen($data['audit_id'],'utf-8')>25){
            return JsonService::fail('输入框内限制25个字');
        }
        foreach ($data as $key=>$vo){
            if(empty($vo)&&$vo!=="0"&&$key!='audit_id'){
                unset($data[$key]);
            }
        }
...
//        return $this->fetch('public/common_form');
        $field = [
            ...
            Form::radio('is_audit','是否开启版块内容人工审核',0)->options([
                ['value'=>1,'label'=>'开启'],
                ['value'=>0,'label'=>'关闭'],
            ]),
            ...
            Form::frameImageOne('background','版块背景图(400*160px)',Url::build('admin/widget.images/index',array('fodder'=>'background')))->icon('image')->width('100%')->height('500px'),
...
//        $data['admin_uid'] = $this->adminId;
        ...
        //清除权限缓存
        ForumPower::clear_cache();
        db('cache_flush')->where('name','cacheFlush_forum_firstClassList')->update(['update_time'=>time()]);
        db('cache_flush')->where('name','cacheFlush_forum_sendList')->update(['update_time'=>time()]);
...
//                //['value'=>7,'label'=>'视频（竖屏）'],
            ...
            Form::radio('is_audit','是否开启版块内容人工审核',$forum['is_audit'])->options([
                ['value'=>0,'label'=>'关闭'],
                ['value'=>1,'label'=>'开启'],
            ]),
...
//            })->filterable(1),
            ...
            Form::frameImageOne('background','版块背景图(400*160px)',Url::build('admin/widget.images/index',array('fodder'=>'background')),$forum->getData('background'))->icon('image')->width('100%')->height('500px'),
...
//            Form::input('false_num','虚拟关注人数',$forum->getData('false_num')),
    ...
    public function edit_content(){
        ...
        $field = [
            ...
            Form::frameImageOne('background','版块背景图(800*320)',Url::build('admin/widget.images/index',array('fodder'=>'background')), $forum['background'])->icon('image')->width('100%')->height('500px'),
    ...
    public function get_platform_config(){
        $field='support_platform,platform_h5_url,platform_xcx_url,platform_android_url,platform_ios_url';
        $field=explode(',',$field);
        ...
        $thumb_dir_path=UPLOAD_PATH.'/caogao/houtai/';
        
        if (!is_dir($thumb_dir_path)){
            mkdir($thumb_dir_path,0777,true);
        }

        self::deldir($thumb_dir_path);

        $code_width=$code_height=200;

        $support_platform=$data['support_platform'];

        if(!is_array($support_platform)){
            $temporary=$support_platform;
            $support_platform=[];
            $support_platform[0]=$temporary;
        }
        if(!in_array(1,$support_platform)){
            unset($data['platform_h5_url']);
        }else{
            $url=$data['platform_h5_url'];
            $qrcode_file_name=$thumb_dir_path.'thumb_qr_code_platform_h5_url'.time().'.png';
            ob_start();
            ...
            $data['platform_h5_url_image']=get_domain().'//'.$qrcode_image['src'];
        }
        if(!in_array(2,$support_platform)){
            unset($data['platform_xcx_url']);
        }
        if(!in_array(3,$support_platform)){
            unset($data['platform_android_url']);
        }else{
            $url=$data['platform_android_url'];
            $qrcode_file_name=$thumb_dir_path.'thumb_qr_code_platform_android_url'.time().'.png';
            ob_start();
            $qrcode::png($url,$qrcode_file_name , $errorCorrectionLevel, $matrixPointSize, 2);
            ob_end_clean();//关闭缓冲区
            $qrcode_image = getThumbImage($qrcode_file_name,$code_width,$code_height);
            $data['platform_android_url_image']=get_domain().'//'.$qrcode_image['src'];
        }
        if(!in_array(4,$support_platform)){
            unset($data['platform_ios_url']);
        }else{
            $url=$data['platform_ios_url'];
            $qrcode_file_name=$thumb_dir_path.'thumb_qr_code_platform_ios_url'.time().'.png';
            ob_start();
            $qrcode::png($url,$qrcode_file_name , $errorCorrectionLevel, $matrixPointSize, 2);
            ob_end_clean();//关闭缓冲区
            $qrcode_image = getThumbImage($qrcode_file_name,$code_width,$code_height);
            $data['platform_ios_url_image']=get_domain().'//'.$qrcode_image['src'];
        }
