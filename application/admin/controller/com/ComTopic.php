...
class ComTopic extends AuthController
{
    ...
    public function create()
    {
        $field = [
            ...
            Form::select('fid','默认板块')->setOptions(function(){
                $list = ForumModel::getCatTierList();
                // $list2=ForumModel::getSelectList();
                foreach ($list as $menu){
                    $menus[] = [
                            'value'=>$menu['id'],
            ...
            Form::frameImageOne('image','封面(256*256)',Url::build('admin/widget.images/index',array('fodder'=>'image')))->icon('image'),
    ...
    public function edit()
    {
        ...
        $field = [
            ...
            Form::select('fid','默认板块',(string)$topic->getData('fid'))->setOptions(function(){
                $list = ForumModel::getCatTierList();
                // $list2=ForumModel::getSelectList();
                foreach ($list as $menu){
                    $menus[] = [
            ...
            Form::frameImageOne('image','封面(256*256)',Url::build('admin/widget.images/index',array('fodder'=>'image')),$topic->getData('image'))->icon('image'),
	...
	public function search_topic(){
	    $search=osx_input('search','','text');
	    $page=osx_input('page',1,'text');
	    $limit=osx_input('limit',10,'intval');
	    $data=db('com_topic')->where(['title'=>['like','%'.$search.'%'],'status'=>1])->page($page,$limit)->select();
	    JsonService::successful('获取成功',$data);
	}
