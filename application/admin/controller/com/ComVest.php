...
class ComVest extends AuthController
{
    ...
    public function edit(){
        ...
        if($params['is_post']==1){
            ...
            if($params['phone']!==''){
                preg_match("/^1[1][0-9]{9}$/", $params['phone'], $match_phone);
                if (!$match_phone) {
                    return Json::fail('请输入以111开头的虚拟手机号码');
                }
            ...
            if($params['id']>0){
                ...
                $version['bind_uid']=$version['nickname']=$version['avatar']=$version['phone']=$version['sex']=$version['signature']= $version['remark']=$version['password']='';
            }
            $field = [
                ...
                Form::input('password','密码',$version['password']),
