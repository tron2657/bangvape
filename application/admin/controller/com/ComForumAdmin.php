...
class ComForumAdmin extends AuthController
{
    ...
    public function del(){
        ...
        if($res!==false){
            //删除版主绑定
            $admin=ForumAdminModel::where('id',$id)->find();
            if(!ForumAdminModel::where(['uid'=>$admin['uid'],'level'=>$admin['level'],'status'=>1])->count()){
    ...
    public function approved(){
        ...
        if($res){
            ...
            if($set['status']==1){
                $message_id=Message::sendMessage($uid,0,$template,1,$set['title'],1,'index');
    ...
    public function score_set()
    {
        ...
        $info=db('com_forum_admin_score')->select();
        foreach($info as &$value){
            $value['info']=json_decode($value['info'],true);
            if($value['info']){
                foreach ($value['info'] as &$val){
                    $val['score']=db('system_rule')->where('flag',$val['flag'])->find();
                }
            }
            unset($val);
        }
        unset($value);
        $reward_points_rules=SystemConfig::getValue('reward_points_rules');
        $this->assign('reward_points_rules',$reward_points_rules);
    ...
    public function saveXieyi(){
        ...
        if($request->isPost()){
            $reward_points_rules = osx_input('post.reward_points_rules','','html');
            SystemConfig::edit(['value' => json_encode($reward_points_rules)],'reward_points_rules','menu_name');
            db('all_agreement')->where('id',9)->update(['update_time'=>time()]);
    ...
    public function edit_score(Request $request){
        ...
        $result = db('com_forum_admin_score')->where('id',$data['id'])->update($data);
        if ($result!==false) {
