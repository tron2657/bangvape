...
class ComReport extends AuthController
{
    ...
    public function delete_time(){
        $id=osx_input('id',0,'intval');
        $res = db('report_prohibit')->where(['id'=>$id])->update(['status'=>-1]);
        if($res) {
            return Json::successful('删除成功!');
        }else{
            return Json::fail('删除失败!');
    ...
    public function delete_content(){
        ...
        if($res) {
            $info=db('report')->where('id',$where['id'])->find();
            RoutineTemplateService::sendTemplate(WechatUser::getMiniOpenId($info['uid']),RoutineTemplateService::REPORT_RESULT, [
                'name1'=>['value'=>'已处理完成'],
                'thing2'=>['value'=>'内容已被删除'],
            ],'','/pages/index/index');
            $thread=db('com_thread')->where('id',$info['content'])->field('title,content')->find();
            $thread['title']=$thread['title']?$thread['title']:mb_substr(json_decode($thread['content']),0,10).'...';
            if($info['reason']){
                $reason=db('report_reason')->where('id',$info['reason'])->value('name');
            }else{
                $reason=$info['other_reason'];
            }
            RoutineTemplateService::sendTemplate(WechatUser::getMiniOpenId($info['to_uid']),RoutineTemplateService::HAVE_REPORT, [
    ...
    public function prohibit_create()
    {
        ...
        if($params['num']){
            if(intval($params['num'])<=0){
                return Json::fail('请正确上传禁言时长!');
            }
            if(intval($params['sort'])<=0){
                return Json::fail('排序值必须大于0');
            }
            if($params['id']){
            ...
            $field = [
                ...
                Form::select('time_type', '时间类型', (string)$data['time_type'])->setOptions([
                    ['label' => '小时', 'value' => 1],
                    ['label' => '天', 'value' => 2],
                ]),
    ...
    public function choose_prohibit(){
        ...
        if($params['choose_data']!=''){
            ...
            if($res){
                $info=db('report')->where('id',$params['id'])->find();
                RoutineTemplateService::sendTemplate(WechatUser::getMiniOpenId($info['uid']),RoutineTemplateService::REPORT_RESULT, [
                    'name1'=>['value'=>'已处理完成'],
                    'thing2'=>['value'=>'账号已被禁言'],
                ],'','/pages/index/index');
    ...
    public function report(){
        ...
        switch($where['type']){
            case 'delete_content':
                ...
                if($res) {
                    if($score){
                        $uid = ComThread::where(['id' => $where['id']])->value('author_uid');
                        $score_log=array();
                        foreach($score as &$value){
                            $score_res=User::where('uid',$uid)->setDec($value['flag'],$value['num']);
                            if($score_res!==false){
                                $score_log[$value['flag']]=$value['num'];
                            }
                        }
                        unset($value);
                        Support::jiafenlog($uid,'被举报扣分',$score_log,1,'行为') ;
                    }
                    $info=db('report')->where('id',$where['id'])->find();
                    RoutineTemplateService::sendTemplate(WechatUser::getMiniOpenId($info['uid']),RoutineTemplateService::REPORT_RESULT, [
                        'name1'=>['value'=>'已处理完成'],
            ...
            case 'no_deal':
                ...
                break;
            case 'choose_prohibit':
                $map['id']=$where['id'];
                $data['operation_uid']=SystemAdmin::activeAdminIdOrFail();
                $data['operation_identity']=1;
                $res=Report::user_prohibit($map,$where['choose_data'],$data);
                if($res){
                    if($score){
                        $uid = ComThread::where(['id' => $where['id']])->value('author_uid');
                        $score_log=array();
