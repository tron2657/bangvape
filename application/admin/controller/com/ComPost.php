...
class ComPost extends AuthController
{
    ...
    //qxh,评论设置页
    public function commen_test(){
        $comment_photo=osx_input('value','');
        // $comment_photo = 1;
        if($comment_photo == ''){
            $comment_photo_set = SystemConfig::getValue('comment_photo');
            $this->assign('comment_photo_set',$comment_photo_set);
            return $this->fetch();
        }else{
            ...
            $res = SystemConfig::updateSms($data);
                if($res == 1){
...
//        $form->hiddenResetBtn(true);
        $data['content']=content_show($data['content']);
        $data['content'] =  content_show(str_replace('&nbsp;','',$data['content']));
    ...
    public function quick_edit(){
        ...
        if(PostModel::where(['id'=>$id])->update([$field=>$value]) !== false){
            if($value==-1 && $field=='status'){
                ...
                if($set['status']==1){
                    $message_id=Message::sendMessage($post['author_uid'],0,$template,1,$set['title'],1,'','thread',$post['tid']);
                ...
                RoutineTemplateService::sendTemplate(WechatUser::getMiniOpenId($post['author_uid']),RoutineTemplateService::POST_DEL, [
                    'thing1'=>['value'=>$post['content']],
                    'thing2'=>['value'=>'被管理员删除'],
                ],'','/pages/index/index');
                //减分
                Gong::delaction('pinglun', $post['author_uid'],'评论被删除');
    ...
    public function delete(){
        ...
        if(empty($post['ids'])){
            ...
            foreach($post as &$value){
                ...
                Gong::delaction('pinglun', $value['author_uid'],'评论被删除');
                $time=time_format(time());
                ...
                $template=str_replace('{帖子标题}', $title, $template);
                if($set['status']==1){
                    $message_id=Message::sendMessage($value['author_uid'],0,$template,1,$set['title'],1,'','thread',$value['tid']);
                ...
                RoutineTemplateService::sendTemplate(WechatUser::getMiniOpenId($value['author_uid']),RoutineTemplateService::POST_DEL, [
                    'thing1'=>['value'=>$value['content']],
                    'thing2'=>['value'=>'被管理员删除'],
                ],'','/pages/index/index');
            }
            unset($value);
