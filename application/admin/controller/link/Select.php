...
use app\admin\model\trial\TrialActive;
...
class Select extends AuthController
{
    ...
    public function forumSelect(){
        ...
        // $forumList=ComForum::where('status',1)->where('name','like','%'.$keyword.'%')->order('sort desc,create_time desc')->limit(20)->select()->toArray();
    ...
    public function goodsSelect(){
        ...
        if(in_array('zg',$goods_open_list)) {
            $zgGoodsList = [];
            $zgColumnList = [];
            $knowledgeList = ColumnText::where('is_show', 1)->where('status',1)->where(function ($queryOr) use ($keyword) {
                $queryOr->where('id', $keyword)->whereOr('name', 'like', '%' . $keyword . '%')->whereOr('keyword', 'like', '%' . $keyword . '%');
            })->order('sort desc,create_time desc')->field('id,pid,is_column,image,name,price,sales,keyword')->limit(20)->select()->toArray();
            foreach ($knowledgeList as &$item) {
                ...
                if ($item['is_column']==1) {
                    $item['link_url'] .= '&type=0';
                    $zgColumnList[] = $item;
                }
                if ($item['is_column']==0&&$item['pid']=='0') $zgGoodsList[] = $item;
            }
            unset($item);
        }else{
        ...
        JsonService::successful([
            'eb_goods'=>['count'=>count($ebGoodsList),'list'=>$ebGoodsList,'tab_title'=>'商城'],
            'zg_goods'=>['count'=>count($zgGoodsList),'list'=>$zgGoodsList,'tab_title'=>'知识商城单品'],
            'zg_column_goods'=>['count'=>count($zgColumnList),'list'=>$zgColumnList,'tab_title'=>'知识商城专栏'],
            'shop_goods'=>['count'=>count($shopGoodsList),'list'=>$shopGoodsList,'tab_title'=>'积分商城']
        ]);
    }
    ...
    public function topicSelect(){
        ...
        unset($item);
//        $tabList=$this->_getTabList();
        JsonService::successful(['count'=>count($topicList),'list'=>$topicList,'tab_title'=>'话题页面']);
    }

    //精彩话题选择
    public function topicJcSelect(){
        $param=UtilService::getMore(['keyword']);
        $keyword=trim(text($param['keyword']));
        if($keyword==''){
            JsonService::success(['list'=>[],'count'=>0]);
        ...
        foreach ($topicList as &$item){
            ...
            $title=str_replace('||','|',$item['title']);
            $item['link_title']='['.$item['id'].']'.(iconv_strlen($title)>8?(mb_substr($title,0,8).'...'):$title);
            $item['link_url']='/packageA/topic/topic-detail-normal?id='.$item['id'];
        }
        unset($item);
//        $tabList=$this->_getTabList();
        JsonService::successful(['count'=>count($topicList),'list'=>$topicList,'tab_title'=>'话题页面']);
    ...
    public function channelSelect(){
        ...
        $topicList=Channel::where('status',1)->where('title|id','like','%'.$keyword.'%')->order('update_time desc,create_time desc')->limit(20)->select()->toArray();
        ...
        foreach ($topicList as &$item){
            ...
            $title=str_replace('||','|',$item['title']);
            $item['link_title']='['.$item['id'].']'.(iconv_strlen($title)>8?(mb_substr($title,0,8).'...'):$title);
            $item['link_url']='/packageA/channel/detail?id='.$item['id'];
        }
        unset($item);
//        $tabList=$this->_getTabList();
        JsonService::successful(['count'=>count($topicList),'list'=>$topicList,'tab_title'=>'频道页面']);
    }
    ...
    public function EventSelect(){
        ...
        $topicList=Event::where(['status'=>1,'id|title'=>['like','%'.$keyword.'%']])->order('create_time desc')->limit(20)->select()->toArray();

        //版块列表
        foreach ($topicList as &$item){
            $item['create_time']=time_format($item['create_time']);
            $item['nickname']=User::where('uid',$item['uid'])->value('nickname');
            $item['link_title']='['.$item['id'].']'.(iconv_strlen($item['title'])>8?(mb_substr($item['title'],0,8).'...'):$item['title']);
            $item['link_url']='/packageA/activity/detail?id='.$item['id'];
        }
        unset($item);
        $tabList=$this->_getTabList();
     ...
     * 试用活动筛选
     */
    public function TrialSelect(){
        $param=UtilService::getMore(['keyword']);
        $keyword=trim(text($param['keyword']));
        if($keyword==''){
            JsonService::success(['list'=>[],'count'=>0]);
        }
        $topicList=TrialActive::where(['status'=>1,'id|title'=>['like','%'.$keyword.'%']])->order('create_time desc')->limit(20)->select()->toArray();
        ...
        foreach ($topicList as &$item){
            ...
            $item['nickname']=User::where('uid',$item['uid'])->value('nickname');
            $item['link_title']='['.$item['id'].']'.(iconv_strlen($item['title'])>8?(mb_substr($item['title'],0,8).'...'):$item['title']);
            $item['link_url']='/pages/slogan/trail/index?id='.$item['id'];
        }
        unset($item);
        $tabList=$this->_getTabList();
        JsonService::successful(['count'=>count($topicList),'list'=>$topicList,'tab_title'=>$tabList['forum']['title']]);
            ...
            'os'=>[
                ...
                'tab_list'=>[
                    ...
                    'tab4'=>[
                        'tab_title'=>'Slogan',
                        'link_list'=>[
                            // ['title'=>'小说告示牌','link_url'=>'/pages/slogan/slogan-fiction'],
                            ['title'=>'有奖征集','link_url'=>'/packageA/topic/topic-select'],
                            // ['title'=>'精彩花贴','link_url'=>'/packageA/topic/topic-detail-normal?id=1'],
                ...
                'tab_list'=>[
                    'tab1'=>[
                        ...
                        'link_list'=>[
                            ...
                            ['title'=>'礼品卡','link_url'=>'/packageC/cart-mall/index'],
                            // ['title'=>'会员卡','link_url'=>'/pages/user/member/index'],
            ...
            'my'=>[
                ...
                'tab_list'=>[
                    ...
                    'tab6'=>[
                        ...
                        'tab_title'=>'其他',
                        'link_list'=>[
                            ['title'=>'邀请好友','link_url'=>'/packageB/promotion-center/reward-friend'],
                            ['title'=>'会员权益','link_url'=>'/pages/user/member/index'],
                        ],
                    ],
                ],
            ],
...
//                    ['title'=>'互动消息','link_url'=>'/packageA/message/alternate'],
            ...
            'renwu'=>[
                ...
                'link_list'=>[
                    ...
                    ['title'=>'会员等级','link_url'=>'/packageA/article/article?id=1'],
                    ['title'=>'积分规则','link_url'=>'/packageA/article/article?id=3'],
            ...
            'event'=>[
                'name'=>'event',//鉴权唯一标识，不可更改，不可重复
                'title'=>'活动中心',//分销
                'type'=>'link_list',//链接列表
                'level'=>1,//层级数（1层）
                'link_list'=>[
                    ['title'=>'活动列表','link_url'=>'/packageA/activity/index'],
                ],
            ],
            'shop'=>[
            ...
            'topic'=>[
                ...
                'select_url'=>url('link.select/topicSelect'),//请求地址topicJcSelect
            ],
            'jc_topic'=>[
                'name'=>'jc_topic',//鉴权唯一标识，不可更改，不可重复
                'title'=>'精彩话题',
                'type'=>'input_select',//输入筛选
                'tip'=>'请输入话题名称进行搜索',//搜索框提示文字
                'select_url'=>url('link.select/topicJcSelect'),//请求地址topicJcSelect
            ],
            'channel'=>[
                'name'=>'channel',//鉴权唯一标识，不可更改，不可重复
                'title'=>'频道',
                'type'=>'input_select',//输入筛选
                'tip'=>'请输入频道ID或名称进行搜索',//搜索框提示文字
                'select_url'=>url('link.select/channelSelect'),//请求地址
            ],
            'goods'=>[
            ...
            'activity'=>[
                'name'=>'activity',//鉴权唯一标识，不可更改，与社区同一鉴权标识
                'title'=>'活动页面',
                'type'=>'input_select',//输入筛选
                'tip'=>'请输入活动ID、标题进行搜索',//搜索框提示文字
                'select_url'=>url('link.select/eventSelect'),//请求地址
            ],
            'trial'=>[
                'name'=>'trial',//鉴权唯一标识，不可更改，与社区同一鉴权标识
                'title'=>'试用页面',
                'type'=>'input_select',//输入筛选
                'tip'=>'请输入活动ID、标题进行搜索',//搜索框提示文字
                'select_url'=>url('link.select/trialSelect'),//请求地址
            ],
            'defined'=>[
    ...
    private function _getZgCategoryList(){
        ...
        if($return_list===false){
            $return_list=[];
            $category_ids=ColumnCategory::where('is_show',1)->column('id');
            $node_ids=ColumnCategory::where('pid','>',0)->where('is_show',1)->column('pid');
            if(count($node_ids)>0){
                $category_ids=array_diff($category_ids,$node_ids);//获取叶子结点id
            }
            $category_list=ColumnCategory::whereIn('id',$category_ids)->field('id,cate_name')->order('id asc')->select();
            foreach ($category_list as $val){
