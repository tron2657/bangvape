...
//                $list = CategoryModel::getTierList();
                $list = CategoryModel::where(['pid'=>0])->field('id,cate_name')->select();
            ...
            Form::frameImageOne('pic','分类图标(175*175)',Url::build('admin/widget.images/index',array('fodder'=>'pic')))->icon('image'),
            Form::number('sort','排序'),
    ...
    public function save(Request $request)
    {
        ...
        if($data['pid'] == '') return Json::fail('请先在分类管理中添加分类');
...
//                $list = (Util::sortListTier(CategoryModel::where('id','<>',$id)->select()->toArray(),'顶级','pid','cate_name'));
            ...
            Form::frameImageOne('pic','分类图标(175*175)',Url::build('admin/widget.images/index',array('fodder'=>'pic')),$c->getData('pic'))->icon('image'),
