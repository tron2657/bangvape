    ...
    public function approve()
    {
        ...
        switch ($params['status']) {
            case '1':
                ...
                RoutineTemplateService::sendTemplate(WechatUser::getMiniOpenId($entity['uid']),RoutineTemplateService::CERTIFICATION_ENTITY, [
                    'phrase1'=>['value'=>'通过'],
                    'thing2'=>['value'=>$name],
                    'date3'=>['value'=>date('Y/m/d H:i',$entity['create_time'])],
                    'thing4'=>['value'=>'无'],
                ],'','/packageC/renzheng/detail?id='.$entity['cate_id']);
                Group::add_bind_group_uid($entity['uid'],$entity['id'],5);
            ...
            case '-2':
                ...
                if($set['status']==1){
                    $message_id=Message::sendMessage($entity['uid'],0,$set['template'],1,$set['title'],1,'','my',$entity['cate_id']);
                    $read_id=MessageRead::createMessageRead($entity['uid'],$message_id,$set['popup'],1);
                }
                if($set['sms']==1&&$set['status']==1){
                    $account=db('user')->where('uid',$entity['uid'])->value('phone');
                ...
                Group::delete_bind_group_uid($entity['uid'],$entity['id'],5);
