...
class StoreCombination extends AuthController
{
    ...
    public function get_combination_list(Request $request){
        ...
        foreach ($data as $k=>$v){
            $data[$k]['_stop_time'] = date('m/d/Y H:i:s',$v['stop_time']);
        }
...
//        })->filterable(1);
    ...
    public function save(Request $request,$id=0)
    {
        ...
        if($id){
            $count=StorePink::where('status',1)->where('cid',$id)->count();
            if($count>0){
                return Json::fail('当前还有拼团进行中，无法编辑!');
            }
...
//        return $this->fetch('public/common_form');
    ...
    public function order_pink($id){
        ...
        $list[] = $StorePink;
        foreach ($list as &$vo){
            $status=db('store_order')->where(['order_id'=>$vo['order_id']])->field('status')->find();
            if($status){
