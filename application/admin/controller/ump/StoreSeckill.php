...
//        var_dump($this->request->post());
        ...
        if(!$data['time_type']) return Json::fail('请选择秒杀时间段');
        $data['start_time'] = strtotime($data['section_time'][0].'00:00:00');
        $data['stop_time'] = strtotime($data['section_time'][1].'23:59:59');
        foreach ($data['time_type'] as &$v){
            $v='|'.$v.'|';
        }
        $data['time_type']=implode(',',$data['time_type']);

        if(!$data['start_time']||!$data['stop_time']||$data['start_time']>=$data['stop_time']) return Json::fail('请正确设置活动时间，结束时间必须大于开始时间');
    ...
    public function seckill($id){
        ...
        if(!$product) return Json::fail('数据不存在!');
        $time_type=\app\admin\model\ump\StoreSeckill::get_time_list();
        $f = array();
        ...
        $f[] = Form::dateRange('section_time','活动时间');
        $f[] = Form::checkbox('time_type','秒杀时间区',[])->options($time_type);
    ...
    public function edit($id)
    {
        ...
        $product = StoreSeckillModel::get($id);
        if(!$product) return Json::fail('数据不存在!');
        $product['time_type']=str_replace('|','', $product['time_type']);
        $time_type=\app\admin\model\ump\StoreSeckill::get_time_list();
        $f = array();
        ...
        $f[] = Form::input('unit_name','单位',$product->getData('unit_name'))->placeholder('个、位');
        $f[] = Form::dateRange('section_time','活动时间',$product->getData('start_time'),$product->getData('stop_time'));
        $f[] = Form::checkbox('time_type','秒杀时间区',explode(',' ,$product['time_type']))->options($time_type);
        $f[] = Form::frameImageOne('image','产品主图片(305*305px)',Url::build('admin/widget.images/index',array('fodder'=>'image')),$product->getData('image'))->icon('image');
    ...
    public function edit_content($id){
        ...
        if(empty($content)){
            $product=StoreSeckillModel::where('id',$id)->value('product_id');
            $content=db('store_product')->where('id',$product)->value('description');
        }
        $this->assign([
            'content'=>$content,
            'field'=>'description',
