...
class SystemConfigTab extends ModelBasic {
    ...
    public static function getRadioOrCheckboxValueInfo($menu_name,$value){
        ...
        if(is_array($value)){
            ...
            $str .= !empty($value)?$option[$value]:$config_one['parameter'];
        }
        return $str;
    }
    /**
    ...
    public static function getAllByEngTitle($eng_title){
        $where['status'] = 1;
        $where['eng_title'] = $eng_title;
        return Db::name('SystemConfigTab')->where($where)->find();
    }
