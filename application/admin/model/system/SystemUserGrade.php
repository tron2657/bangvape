 ...
 */
class SystemUserGrade extends ModelBasic
{
    use ModelTrait;
     ...
     * 查找系统设置的会员等级列表
     * */
    public static function getSytemList($where,$type)
    {
        $data=self::where('type',$type)->where('is_del',0)->order('id desc')->page((int)$where['page'],(int)$where['limit'])->select();
        $data=count($data) ? $data->toArray() : [];
        $count=self::where('type',$type)->where('is_del',0)->count();
