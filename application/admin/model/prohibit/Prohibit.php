...
class Prohibit extends ModelBasic
{
    ...
    public static function ProhibitAllList($where){
        $data=($data=db('report')->where('status',1)->where('prohibit_time','>',time())->order('id desc')->page((int)$where['page'],(int)$where['limit'])->select()) && count($data) ? $data : [];
        foreach ($data as &$item){
            ...
            $item['reason']=db('report_reason')->where('id',$item['reason'])->value('name');
            $time_type=db('report_prohibit')->where('id',$item['prohibit'])->find();
            switch($time_type['time_type']){
                case 1:
                    $time_type['time_type']='小时';
                    break;
