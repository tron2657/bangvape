   ...
   /**
    ...
    public static function getbadge($where,$type){
        ...
        return [
            [
                ...
                'sum'=>self::where('status',1)->where('add_time','>=',strtotime(date('Y-m-d 00:00')))->count(),
            ...
            [
                ...
                'sum'=>self::where('status',1)->where('add_time','>=',strtotime(date('Y-m-d 00:00')))->sum('stock'),
            ...
            [
                'name'=>'缺货商品种类',
                'field'=>'种',
                'count'=>self::where(['stock'=>['lt',20],'status'=>1])->count(),
                'content'=>'总商品种类数量',
                'background_color'=>'layui-bg-orange',
                'sum'=>self::where('status',1)->count(),
                'class'=>'fa fa-cube',
            ],
        ];
    }
    ...
    public static function ProfityTop10($where){
        ...
        $list=self::getModelTime($where,$model,'a.add_time')->group('a.seckill_id')->order('profity desc')->limit(10)
            ->field(['count(a.seckill_id) as p_count','b.title as store_name','sum(b.price) as sum_price','sum(b.price-b.cost) as profity'])
            ->select();
        if(count($list)) $list=$list->toArray();
        $maxList=[];
        $sum_count=0;
...
//        $model = $model->join('StoreProduct p','p.id=s.product_id');
    ...
    public static function get_time_list(){
        ...
        for ($i=0;$i<24;$i++){
            $time['value']=$i;
            $time['label']=date('H:i',16*60*60+$i*60*60).'~'.date('H:i',16*60*60+($i+1)*60*60-1);
            $time['label_name']=date('H:i',16*60*60+$i*60*60);
            $data[$i]=$time;
        }
        return $data;
    }
