...
class StoreStatistics extends ModelBasic
{
    ...
    {
        $is_type = 0;
        if (!empty($where['is_type'])) $is_type = $where['is_type'];
        return self::getTime($where)->where('paid', 1)->where('refund_status', 0)->where('is_zg', $is_type);
    }
    /**
     * 获取时间区间
     */
    ...
    public static function getConsumption($where)
    {
        ...
        if ($where['is_type']==1) {
            $consumption=$consumption->where('a.type','pay_zg');
        }else{
            $consumption=$consumption->where('a.type','pay_product');
        }
        $consumption=$consumption->find()->toArray();
        return $consumption;
    }
    /**
     * 获取拼团商品
    ...
    {
        $extension = self::getTime($where,new UserBill);
        if ($where['is_type']==1) {
            $extension=$extension->where('type','pay_zg');
        }else{
            $extension=$extension->where('type','brokerage');
        }
        $extension = $extension->where('category','now_money')->sum('number');
        return $extension;
    }
     ...
     * 最近交易
     */
    public static function trans($is_type = 0)
    {
        $trans = self::alias('a')
            ->join('__USER__ b', 'a.uid=b.uid','left')
            ->join('__STORE_ORDER_CART_INFO__ c', 'a.id=c.oid')
            ->join('__STORE_PRODUCT__ d', 'c.product_id=d.id');
        if ($is_type == 1) $trans = $trans->where('d.is_type',1);
        $trans = $trans->field('b.nickname,a.pay_price,d.store_name')
