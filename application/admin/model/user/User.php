...
class User extends ModelBasic
{
    ...
    public static function setWhere($where)
    {
        ...
        if($where['is_vip']!=''){//目前暂不支持永久会员
            if($where['is_vip']==1)
            {
                $model->where( function($query){
                        $query->where('u.overdue_time','>',0)->where('u.overdue_time','>=',time())->whereor('is_permanent',1);
                });
          
            }
            if($where['is_vip']==0)
            {
                $model->where('u.overdue_time','=',0);
 
            }
        }
        if($where['is_vest']===0){
            $model->where('u.is_vest',0);
        }
        if($where['uid']!=''){
            $model->where('u.uid',$where['uid']);
        }
        if($where['g_id']!=''){
            $model= Group::get_group_uid($where['g_id'],$model);
        }
        if($where['mail']!=''){
            $model->where('u.email','like','%'.$where['mail'].'%');
        }
        ...
        if(isset($where['nickname']) && $where['nickname'] != '') $model->where('u.nickname|u.phone|u.uid','LIKE',"%".$where['nickname']."%");
        if(isset($where['phone']) && $where['phone'] != '') $model->where('u.phone','LIKE',"%".$where['phone']."%");
        $model->where(function ($query) use($where){
            switch ($where['pay_count']) {
                case '':
                    ;
                    break;
                case 0:
                case 1;
                    $query->where('pay_count', $where['pay_count']);
                    break;
                case 2:
                    $query->where('pay_count', '>=', 2)->where('pay_count', '<=', 5);
                    break;
                case 3:
                    $query->where('pay_count', '>', 5)->where('pay_count', '<=', 10);
                    break;
                case 4:
                    $query->where('pay_count', '>', 10)->where('pay_count', '<=', 20);
                    break;
                case 5:
                    $query->where('pay_count', '>', 20);
                    break;
                default:
                    break;
            }
            trace($where);
            switch ($where['post_count']) {
                case '':
                    ;
                    break;
                case 0:
                case 1;
                    $query->where('post_count', $where['post_count']);
                    break;
                case 2:
                    $query->where('post_count', '>=', 2)->where('post_count', '<=', 5);
                    break;
                case 3:
                    $query->where('post_count', '>', 5)->where('post_count', '<=', 10);
                    break;
                case 4:
                    $query->where('post_count', '>', 10)->where('post_count', '<=', 20);
                    break;
                case 5:
                    $query->where('post_count', '>', 20);
                    break;
                default:
                    // code...
                    break;
    ...
    public static function getUserList($where){
        $model = self::setWherePage(self::setWhere($where),$where,['u.sex','w.province','w.city','u.status','u.is_promoter'],['u.nickname','u.uid']);
        $list = $model->alias('u')
            ->join('WechatUser w','u.uid=w.uid','LEFT')
            ->join('UserWallet x','u.uid=x.uid','LEFT')
            ->field('u.*,w.country,w.province,w.city,w.unionid,w.openid,w.routine_openid,w.groupid,w.tagid_list,w.subscribe,w.subscribe_time,x.enable_money,x.disable_money')
            ->page((int)$where['page'],(int)$where['limit'])
            ->select()
            ->each(function ($item){

                // $item['refund_operation_name']=SystemAdmin::get($item['refund_operation_uid']);
                // $item['refund_operation_show_time']=date('Y-m-d H:i',$item['refund_operation_time']);

                $item['isVip']=$item['overdue_time']>0 && $item['overdue_time']>=time() ||  $item['is_permanent']==1;
                $item['overdue_time_show']=date('Y-m-d H:i:s',$item['overdue_time']);
                // $item['isVip']=$item['isVip']?'是':'否';
                $item['add_time']=date('Y-m-d H:i:s',$item['add_time']);
                unset($item['pwd']);
                $item['is_recommend']=db('user_recommend')->where('uid',$item['uid'])->where('status',1)->count();
                ...
                $item['enable_money']=$item['enable_money']>0?$item['enable_money']:'0.00';
                $item['disable_money']=$item['disable_money']>0?$item['disable_money']:'0.00';
                $item['wallet']='可用余额:'.$item['enable_money'].'</br>冻结余额:'.$item['disable_money'];
    ...
    */
    ...
    public static function getUserDetailed($uid){
        $userInfo=self::where('uid',$uid)->find();
        $userInfo['user_grade']=self::cacugrade($userInfo['exp']);
        $userInfo['collect_count']=db('collect')->where('uid',$uid)->where('status',1)->count();
        $userInfo['post_count']=ComThread::where('author_uid',$uid)->where('status',1)->where('type',1)->count();
        $userInfo['video_count']=ComThread::where('author_uid',$uid)->where('status',1)->where('type',6)->count();
        $userInfo['news_count']=ComThread::where('author_uid',$uid)->where('status',1)->where('type',4)->count();
        $entity=db('certification_entity')->where('uid',$uid)->where('status',1)->column('cate_id');
        $userInfo['certification']=db('certification_cate')->where('id','in',$entity)->column('name');
        $userInfo['is_seller']=is_seller($uid);
        $userInfo['add_time']=time_format($userInfo['add_time']);
        $userInfo['last_time']=time_format($userInfo['last_time']);
        //经验值名字
        $rule=array('exp','fly','buy','gong');
        $rule_name=db('system_rule')->where(['flag'=>['in',$rule]])->field('flag,name')->select();
        $userInfo['exp_name']= $userInfo['fly_name']= $userInfo['buy_name']= $userInfo['gong_name']='';
        foreach ($rule_name as $v){
            $userInfo[$v['flag'].'_name']=$v['name'];
        }
        unset($v);
    ...
    private static function cacugrade($exp){
