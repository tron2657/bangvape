...
class ShopOrder extends ModelBasic
{
    ...
    public static function orderCount(){
        ...
        $data['jy']=self::statusByWhere(3,new self())->count();
...
//        $data=($data=$model->page((int)$where['page'],(int)$where['limit'])->select()) && count($data) ? $data->toArray() : [];
        foreach ($data as &$item){
            ...
            $item['product_info']=ShopProduct::where('id',$item['product_id'])->find();
            if(mb_strlen($item['product_info']['store_name'])>30){
                $item['product_info']['store_name']=mb_substr($item['product_info']['store_name'],0,30, 'utf-8').'...';
            }
            if($item['paid']==1){
            ...
            if($item['paid']==1 && $item['status']==1){
                $item['status_name']='待发货';
            }else if($item['paid']==1 && $item['status']==2){
                $item['status_name']='待收货';
            }else if($item['paid']==1 && $item['status']==3){
                $item['status_name']='已完成';
            }else if($item['paid']==1 && $item['status']==-2){
                $item['status_name']='已退款';
            }else if($item['paid']==0 && $item['status']==0){
                $item['status_name']='未支付';
            } else if($item['paid']==0 && $item['status']==-1){
    ...
    public static function SaveExcel($list){
        ...
        foreach ($list as $index=>$item){
            $_info =ShopProduct::where('id',$item['product_id'])->find();
            $item['cartInfo'] = $_info;
            $export[] = [
                $item['order_id'],$item['pay_type_name'],
                $item['total_num'],$item['pay_price'],$item['pay_postage'],$item['pay_price'],$item['refund_price'],
    ...
    public static function statusByWhere($status,$model = null,$alert='')
    {
        ...
        else if($status == 3)// 交易完成
            return $model->where($alert.'paid',1)->where($alert.'status',3);
        else if($status == -2)//已退款
            return $model->where($alert.'paid',1)->where($alert.'status',-2);
        else if($status == 0)//未支付
            return $model->where($alert.'paid',0)->where($alert.'status',0);
        else if($status == -1)//已取消
            return $model->where($alert.'paid',0)->where($alert.'status',-1);
        else
            return $model;
    }
    ...
    public static function getOrderPrice($where){
        ...
        $price = array();
        $price['order_num'] = 0;
        $price['pay_cash'] = 0;//支付金额
        $price['pay_score'] = 0;//支付积分
        $price['pay_postage'] = 0;//支付运费
        $price['total_num'] = 0; //商品总数
        $model = self::getOrderWhere($where,$model);
        $list = $model->where('status','>',0)->select()->toArray();
        foreach ($list as $v){
            $price['order_num']++;
            $price['total_num'] = bcadd($price['total_num'],$v['total_num'],0);
            $price['pay_cash'] = bcadd($price['pay_cash'],$v['pay_cash'],2);
            $price['pay_score'] = bcadd($price['pay_score'],$v['pay_score'],0);
            $price['pay_postage'] = bcadd($price['pay_postage'],$v['pay_postage'],2);
        }
        return $price;
    }
    ...
    public static function getOrderBadgeZg($where){
        return [
            ...
            [
                'name'=>'积分消耗数',
                'field'=>'个',
                'count'=>self::setEchatWhereZg($where)->where('is_zg',1)->where('paid',1)->sum('use_integral'),
                'content'=>'积分消耗总数',
                'background_color'=>'layui-bg-cyan',
                'sum'=>self::setEchatWhereZg($where,null,true)->where('is_zg',1)->where('paid',1)->sum('use_integral'),
                'class'=>'fa fa-line-chart',
                'col'=>2
            ],
            [
                'name'=>'积分抵扣金额',
                'field'=>'个',
                'count'=>self::setEchatWhereZg($where)->where('is_zg',1)->where('paid',1)->sum('deduction_price'),
                'content'=>'积分抵扣总金额',
                'background_color'=>'layui-bg-cyan',
                'sum'=>self::setEchatWhereZg($where,null,true)->where('is_zg',1)->where('paid',1)->sum('deduction_price'),
                'class'=>'fa fa-money',
                'col'=>2
            ],
            [
...
//        ],Url::build('My/order',['uni'=>$order['order_id']],true,true));
    ...
    public static function get_delivery_code($order_name){
        switch ($order_name){
            case '速通物流':
     ...
     * 获取物流信息
     * @param $order
     * @return url响应返回的html|mixed
     */
    public static function get_delivery($order){
        $ShipperCode=self::get_delivery_code($order['delivery_name']);
        $order_sn=$order['delivery_id'];//运单号
        $phone=$order['delivery_id'];//手机号
        ...
        //把$requestData进行加密处理
        $datas['DataSign'] = self::encrypt($requestData,$config['kdn_my']);
