...
class Group extends ModelBasic
{
    ...
    public static function bind_group_uid($ids,$group,$time,$is_deal=false){
        ...
        if($is_deal){
            //重新设置
            db('bind_group_uid')->where(['uid'=>$ids])->delete();
            $uids[0]=$ids;
        }else{
            //删除重复的删除
            db('bind_group_uid')->where(['uid'=>['in',$uids],'g_id'=>['in',$group]])->delete();
        }
        $data=[];
        foreach ($uids as $v){
            $value['create_time']=time();
    ...
    public static function get_bind_group_uid($uid){
        $list = db('bind_group_uid')->where(['uid'=>$uid,'status'=>1,'type'=>0])->group('g_id')->select();
        foreach ($list as &$vo){
...
//        $value=UserModel::cacugrade($exp);
    ...
    public static function get_group_by_uid($uid){
        ...
        if(!$group){
            ...
            Cache::tag('group_by_uid')->set('_group_'.$uid.'_power',$group,60*1);
        }
     ...
     * 绑定用户和用户组
     */
    public static function add_bind_group_uid($uid,$id,$type){
        $group_id='';
        switch ($type){
            case 5:
                $group_id=db('certification_entity')->where(['cate_id'=>$id,'status'=>1])->value('id');
                break;
        }
        if($group_id){
            return self::bind_group_uid($uid,$group_id,0,false);
        }else{
            return '';
        }
    }
     ...
     * 删除用户和用户组
     */
    public static function delete_bind_group_uid($uid,$id,$type){
        $group_id='';
        switch ($type){
            case 5:
                $group_id=db('certification_entity')->where(['cate_id'=>$id,'status'=>1])->value('id');
        ...
        if($group_id){
            return db('bind_group_uid') ->where(['uid'=>$uid,'g_id'=>$group_id,'type'=>$type])->delete();
        }else{
            return '';
        }
    }
