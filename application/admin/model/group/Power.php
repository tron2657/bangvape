...
class Power extends ModelBasic
{
     ...
     *时间；2020.3.27
     */
    public static function getAllPower($map,$g_id,$uid,$no=0){
        $power=db('power')->where($map)->select();
        //用户管理权限
        $no_power=['forum_prohibit','group_manage','audit_visit','audit_content','audit_admin'];
        //获取选择的power;
        if($uid){
        ...
        foreach($power as $k=>&$vo){
            if($vo['input_type']=='radio'){
                //管理权限判断
                if(in_array($vo['sign'],$no_power)&&$no==1){
                    unset($power[$k]);
                }else {
                    ...
                    foreach ($value as $key => $v) {
                        $list = explode('=>', $v);
                        $valueList = ['key' => $list[0], 'label' => $list[1], 'checked' => '','readonly'=>0];
