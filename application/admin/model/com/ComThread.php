...
class ComThread extends ModelBasic
{
    ...
    public static function ThreadList($where)
    {
        ...
        foreach ($data as &$item){
            ...
            if($item['status']==1){
                $already_in_channel_post_pool=ChannelPostPool::where('post_id',$item['id'])->where('status',1)->count();
                if($already_in_channel_post_pool){
                    //备选池已存在
                    $item['already_in_channel_post_pool']=1;
                }else{
                    $item['already_in_channel_post_pool']=0;
                }
            }
            //帖子热度标识
            $counts=SystemConfig::getValue('forum_hot_count');
            $counts=explode(',',$counts);
            $item['hot_count']=0;
            $item['reward_count']=0;
            if($item['support_count']>=$counts[0]){
                $item['hot_count']=$counts[0];
                if($item['support_count']>=$counts[1]){
                        $item['hot_count']=$item['hot_count'].'/'.$counts[1];
                }
                if($item['support_count']>=$counts[2]){
                        $item['hot_count']=$item['hot_count'].'/'.$counts[2];
                }
                if($item['support_count']>=$counts[3]){
                        $item['hot_count']=$item['hot_count'].'/'.$counts[3];
                }
                //奖励次数
                $item['reward_count']=ComForumAdminScoreLog::getRewardCount($item['id']);
            }
...
//                $model = $model->whereOr('p.cate_id','LIKE',["%$catid%",$catidab]);
            ...
            if(isset($where['is_recommend']) && $where['is_recommend']!=''){
                $model->where('is_recommend', $where['is_recommend']);
            }
            if($where['fid']){
            ...
            if($where['hot_count']!=''){
                $counts=SystemConfig::getValue('forum_hot_count');
                $counts=explode(',',$counts);
                $key=array_search($where['hot_count'],$counts);
                if($key<count($counts)-1){
                    $model->where('support_count','egt',$where['hot_count'])->where('support_count','lt',$counts[$key+1]);
                }else{
                    $model->where('support_count','egt',$where['hot_count']);
                }
    ...
    public static function createThread($data)
    {
        ...
        if($data['image']==''&&$data['is_auto_image']==1&&$data['from']=='HouTai'){
            $data['image']=self::_contentToImage($data['contenComt']);
            if(!$data['image']){
                $data['image']='';
            }else{
     ...
     * @return string
     */
    public static function _videoKey($video_id){
        $getYunConfig = TencentFile::ifYunUpload();
        //Header加密
        $Header['alg']='HS256';
        $Header['typ']='JWT';
     ...
     * @return string
     */
    public static function getAnimatedGraphic($video_id) {
        $template_id = SystemConfig::getValue('tencent_animated_template_id');
        if ($template_id == '') return '';
        try {
            $getYunConfig = TencentFile::ifYunUpload();
            $cred = new Credential($getYunConfig['sid'], $getYunConfig['skey']);
            ...
            for ($i = 0; $i < 60; $i++) {
                try {
                    $req2 = new DescribeTaskDetailRequest();
                    $params = array(
                        'TaskId' => $taskId
                    );
                    $req2->fromJsonString(json_encode($params));
                    $resp2 = $client->DescribeTaskDetail($req2);

                    if ($resp2->getStatus() == 'FINISH') {
                        $procedureTask = $resp2->getProcedureTask();
                        if (is_null($procedureTask)) return '';
                        $mediaProcessResultSet = $procedureTask->getMediaProcessResultSet();
                        if (is_null($mediaProcessResultSet) || empty($mediaProcessResultSet)) return '';
                        $animatedGraphicTask = $mediaProcessResultSet[0]->getAnimatedGraphicTask();
                        if (is_null($animatedGraphicTask)) return '';
                        $output = $animatedGraphicTask->getOutput();
                        if (is_null($output)) return '';
                        $url = $output->getUrl();
                        return $url;
                    }
