...
class ComThreadClass extends ModelBasic
{
    ...
    private static function get_extend($id)
    {
        $data=ComThreadClassExtend::get(['class_id'=>$id]);
        if($data)
        {
            $data['tg_enable']=$data['tg_enable']==0?'关闭':'开启';
            $data['tg_end_time']=date('Y-m-d H:i',$data['tg_end_time']);
            $data['tg_start_time']=date('Y-m-d H:i',$data['tg_start_time']);
            return $data;
        }
    ...
    public static function ClassList($where){
        ...
        foreach ($data as &$item){
            ...
            $item['extend']=self::get_extend($item['id']);
        }
