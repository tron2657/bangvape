...
class VisitAudit extends ModelBasic
{
    ...
    public static function send_message($map,$message_type,$reason=''){
        ...
        foreach ($audit as $v){
            ...
            if($set['status']==1){
                if($message_type==49){
                    $message_id=Message::sendMessage($v['uid'],$now_uid,$template,1,$set['title'],1,'','forum',$v['fid']);
                }else{
                    $message_id=Message::sendMessage($v['uid'],$now_uid,$template,1,$set['title'],1,'','index','');
                }
            ...
            if($message_type==49){
                RoutineTemplateService::sendTemplate(WechatUser::getMiniOpenId($v['uid']),RoutineTemplateService::FORUM_ADD, [
                    'date1'=>['value'=>date('Y/m/d H:i',time())],
                    'thing2'=>['value'=>$forum_name],
                    'thing3'=>['value'=>'无'],
                ],'','/packageA/forum-detail/normal?fid='.$v['fid']);
            }
            if($message_type==50){
                $nickname=db('user')->where('uid',$v['uid'])->value('nickname');
                RoutineTemplateService::sendTemplate(WechatUser::getMiniOpenId($v['uid']),RoutineTemplateService::FORUM_REFUSE, [
                    'name1'=>['value'=>$nickname],
                    'date2'=>['value'=>date('Y/m/d H:i',$v['create_time'])],
                    'thing3'=>['value'=>$forum_name],
                    'thing4 '=>['value'=>$reason],
                ],'','/packageA/forum-detail/normal?fid='.$v['fid']);
            }
        }
