...
class ComForum extends ModelBasic
{
     ...
     * @return array
     */
    public static function get_check_radio($map){
        $com_forum=db('com_forum')->where($map)->field('id,name')->select();
        $data=[];
        foreach ($com_forum as $v){
            $data[$v['id']]=['label'=>$v['name'],'value'=>$v['id']];
        }
        unset($v);
        return  $data;
    }
