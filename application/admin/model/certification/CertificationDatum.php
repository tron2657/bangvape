...
class CertificationDatum extends ModelBasic
{
    ...
    {
        $model = new self;
        $list = $model->where('status',1)->where($where)->order('id asc')->select();
        return $list;
    }
    /**
     * 获取指定列表
    ...
    public static function getAdminPage($params,$ajax)
    {
        ...
        if ($ajax) {
            ...
            $data=($data=$model->order('sort DESC')->select()) && count($data) ? $data->toArray():[];
            foreach ($data as $key => $value) {
//                $data[$key]['type_id']= getCertificationType($value['type_id']);
                $use_range=explode(',',$value['use_range']);
                $data[$key]['use_range']=self::get_use_range($use_range);
            }
     ...
     * @return string
     */
    public static function get_use_range($value){
        $range='';
        foreach ($value as $v){
            switch ($v){
                case 0:$range.='用户资料项<br/>';break;
                case 1:$range.='认证<br/>';break;
                case 2;$range.='活动<br/>';break;
                case 4;$range.='产品试用<br/>';break;
                default:;
            }
        }
        return $range;
