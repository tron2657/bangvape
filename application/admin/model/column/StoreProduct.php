...
//        return $res;
    ...
    public static function getModelObject($where=[]){
        ...
        if(!empty($where)){
            ...
            $model = $model->where('p.is_type',$where['is_type']);
            $model = $model->where('p.is_column',$where['is_column']);
...
//                $model = $model->whereOr('p.cate_id','LIKE',["%$catid%",$catidab]);
            ...
            if(isset($where['recommend_sell']) && $where['recommend_sell']!=2){
                $model = $model->where('recommend_sell',$where['recommend_sell']);
            }
        }
        return $model;
    }
    ...
    public static function ProductList($where){
        ...
        foreach ($data as &$item){
            $cateName = db('store_category_column')->where('id', 'IN', $item['cate_id'])->column('cate_name', 'id');
        ...
        if($where['excel']==1){
            ...
            foreach ($data as $index=>$item){
                $export[] = [
                    ...
                    $item['like'],
                    $item['collect'],
                    $item['recommend_sell']?"是":"否",
                    $item['buy_num'],
                    $item['platform_get'],
                    $item['strip_num']
                ];
            }
            PHPExcelService::setExcelHeader(['产品名称','产品简介','产品分类','价格','库存','销量','点赞人数','收藏人数','是否推荐分销','限购数','平台抽取','剥比'])
    ...
    public static function setWhereType($model,$type,$is_type=0){
        switch ($type){
            case 1:
                $data = ['is_show'=>1,'is_del'=>0,'is_type'=>$is_type];
                break;
            case 2:
                $data = ['is_show'=>0,'is_del'=>0,'is_type'=>$is_type];
                break;
            case 3:
                $data = ['is_del'=>0,'is_type'=>$is_type];
                break;
            case 4:
                $data = ['is_show'=>1,'is_del'=>0,'stock'=>0,'is_type'=>$is_type];
                break;
            case 5:
                $data = ['is_show'=>1,'is_del'=>0,'stock'=>['elt',1],'is_type'=>$is_type];
                break;
            case 6:
                $data = ['is_del'=>1,'is_type'=>$is_type];
                break;
        }
