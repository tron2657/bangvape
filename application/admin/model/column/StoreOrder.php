...
//        $data=($data=$model->page((int)$where['page'],(int)$where['limit'])->select()) && count($data) ? $data->toArray() : [];
        foreach ($data as &$item){
            ...
            foreach ($_info as $k=>$v){
                ...
                if (isset($cart_info['productInfo']) && !isset($cart_info['productInfo']['store_name'])) {
                    $cart_info['productInfo']['store_name'] = isset($cart_info['productInfo']['name']) ? $cart_info['productInfo']['name'] : '';
                }
    ...
    public static function SaveExcel($list){
        ...
        foreach ($list as $index=>$item){
            ...
            foreach ($_info as $k=>$v){
                ...
                $goodsName[] = implode(
                    [isset($v['productInfo']['name'])?$v['productInfo']['name']:'',
...
//        if($where['is_del'] != '' && $where['is_del'] != -1) $model = $model->where('is_del',$where['is_del']);
    ...
    public static function setEchatWhereZg($where,$status=null,$time=null){
        ...
        switch ($where['type']){
            case 1:
                //专栏
                $model=$model->where('combination_id',0)->where('is_column',1)->where('seckill_id',0)->where('bargain_id',0)->where('is_zg',1);
                break;
            case 2:
                //单品
                $model=$model->where('combination_id',">",0)->where('is_column',0)->where('pink_id',">",0)->where('is_zg',1);
                break;
        }
        return self::getModelTime($where,$model);
    }
