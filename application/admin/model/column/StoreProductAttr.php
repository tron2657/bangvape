...
class StoreProductAttr extends ModelBasic
{
    ...
    public static function createProductAttr($attrList,$valueList,$productId)
    {
        ...
        foreach ($valueList as $index=>$value){
            ...
            if(!isset($value['strip_num']) || !is_numeric($value['strip_num']) || floatval($value['strip_num']) != $value['strip_num'])
                return self::setErrorInfo('请填写正确的商品剥比值');
            if(bccomp($value['price'],$value['strip_num'])<0){
                return self::setErrorInfo('剥比不能超过售价');
            }
