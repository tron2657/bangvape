...
<div class="layui-fluid" style="background: #fff;margin-top: -10px;">
                    ...
                    <div class="layui-form-item">
                        <div class="layui-inline">
                            ...
                            <div class="layui-input-block">
                                <input type="text" style="width:250px" name="store_name" class="layui-input" placeholder="搜索商品名称">
                            </div>
                        </div>
                        <div class="layui-inline">
    ...
    <div class="layui-row layui-col-space15"  id="app">
        ...
        <div class="layui-col-md12">
            <div class="layui-card">
                <div class="layui-card-header">专栏/内容管理</div>
                <div class="layui-card-body">
                    <div class="layui-btn-container">
                        <button type="button" class="layui-btn layui-btn-xs" style="height: 32px;padding: 5px 10px" onclick="dropdown(this)">添加单品 <span class="caret"></span></button>
                        <ul class="layui-nav-child layui-anim layui-anim-upbit" style="font-size: 14px">
                            <li>
                                <a href="javascript:void(0);" onclick="$eb.createModalFrame(this.innerText,'{:Url('create_text')}?column_id={$pid}',{h:700,w:650})">
                                    图文单品
                                </a>
                            </li>
                            <li>
                                <a href="javascript:void(0);" onclick="$eb.createModalFrame(this.innerText,'{:Url('create_audio')}?column_id={$pid}',{h:700,w:650})">
                                    音频单品
                                </a>
                            </li>
                            <li>
                                <a href="javascript:void(0);" onclick="$eb.createModalFrame(this.innerText,'{:Url('create_video')}?column_id={$pid}',{h:700,w:650})">
                                    视频单品
                                </a>
                            </li>
                        </ul>
                    ...
                    <script type="text/html" id="readcheckboxstatus">
                        <input type='checkbox' name='id' lay-skin='switch' value="{{d.id}}" lay-filter='is_show' lay-text='开启|关闭'  {{ d.is_trial == 1 ? 'checked' : '' }}>
                    </script>
...
<script>
    ...
    layList.form.render();
    //加载列表
    layList.tableList('List',"{:Url('column.column_list/product_list',['pid'=>$pid])}",function (){
        return join=[
            {field: 'id', title: 'ID', sort: true,event:'id',width:'6%'},
            {field: 'img', title: '商品图片',templet:'#image',width:'10%'},
            {field: 'store_name', title: '商品信息',templet:'#store_name'},
            {field: 'is_free', title: '类型',width:'6%'},
            {field: 'nickname', title: '作者',width:'6%'},
            {field: 'sort', title: '专栏内排序',width:'8%'},
            {field: 'sales', title: '状态',event:'sales',width:'8%',templet:"#checkboxstatus"},
            {field: 'sort', title: '试读',width:'6%',templet:"#readcheckboxstatus"},
    ...
    function dropdown(that){
        ...
        if($(document).height() < top+$(that).next('ul').height()){
            ...
            $(that).next('ul').css({
                'padding': 10,
                'left': 11,
                'top':$(that).parent('td').height() / 2 + $(that).height() + 20,
                'min-width': 'inherit',
                'position': 'absolute'
            }).toggle();
        }
    ...
    layList.switch('is_show',function (odj,value) {
        if(odj.elem.checked==true){
            layList.baseGet(layList.Url({c:'column.column_list',a:'set_trial',p:{is_trial:1,id:value}}),function (res) {
                layList.msg(res.msg);
            });
        }else{
            layList.baseGet(layList.Url({c:'column.column_list',a:'set_trial',p:{is_trial:0,id:value}}),function (res) {
                layList.msg(res.msg);
            });
        }
    });
