...
<!--                        <div class="hr-line-dashed"></div>-->
                        ...
                        <div class="form-group" style="text-align: center;">
                            <div class="col-sm-4 col-sm-offset-2">
                                {if condition="$tab_id eq '34'"}
                                    {if condition="in_array('osapi_video',$open_list)"}
                                    <button class="btn btn-primary" type="submit">提交</button>
                                    {else/}
                                    <button class="btn btn-primary" id="unable" type="submit">提交</button>
                                    {/if}
                                {elseif condition="$tab_id eq '40'"/}
                                    {if condition="in_array('user_invite_rewards',$open_list)"}
                                    <button class="btn btn-primary" type="submit">提交</button>
                                    {else/}
                                    <button class="btn btn-primary" id="unable" type="submit">提交</button>
                                    {/if}
                                {elseif condition="$tab_id eq '90'"/}
                                    {if condition="in_array('website_connect',$open_list)"}
                                    <button class="btn btn-primary" type="submit">提交</button>
                                    {else/}
                                    <button class="btn btn-primary" id="unable" type="submit">提交</button>
                                    {/if}
                                {elseif condition="$tab_id eq '70'"/}
                                    {if condition="in_array('right_copy',$open_list)"}
                                    <button class="btn btn-primary" type="submit">提交</button>
                                    {else/}
                                    <button class="btn btn-primary" id="unable" type="submit">提交</button>
                                    {/if}
...
<script>
    ...
    * */
        ...
        $.ajaxFileUpload({
            ...
            success: function(data, status, e){
                ...
                if(data.code == 200){
                    ...
                    // }else if(bool_upload_num == 0){
                        // if (time == 1) {
                        // console.log(222)

                        //     let className = $(is).data('name');
                        //     let tag = '<div class="file-box"><div class="'+ className +'"></div></div>';
                        //     $('.show-box').append(tag);
                        // }
                        // $('.show-box').css('display','block')
                        // let className = $(is).data('name');
                        // let tag = '<div class="'+ className +'"></div>'
                        // $('.show-box').append(tag);
                        // getHtml($(is).data('name'),data.data.url);
                        // $('.'+$(is).data('name')).empty();
                        // $('.'+$(is).data('name')).append(html_one);
    ...
    $(document).on('click','.open_image',function (e) {
        var image = $(this).children().data('image');
        $eb.openImage(image);
    })
    window.onload = function () {
        if ($("input[name='video_cover_type']").length > 0) {
            var video_cover_type = $("input[name='video_cover_type']:checked").val();
            if (video_cover_type == '1') {
                $('#video_default_cover').parents('.form-group').hide();
            }
        }
    }
</script>
