...
<div class="main">
    ...
    <div class="left">
        <div class="left-top">
            <button class="layui-btn layui-btn-primary layui-btn-xs" id="addcate" title="添加分类"><i
                        class="layui-icon layui-icon-add-circle-fine"></i></button>
            <button class="layui-btn layui-btn-primary layui-btn-xs" id="editcate" title="编辑当前分类"><i
                        class="layui-icon layui-icon-edit"></i></button>
            <button class="layui-btn layui-btn-primary layui-btn-xs" id="deletecate" title="删除当前分类"><i
                        class="layui-icon layui-icon-delete"></i></button>
        </div>
        <div class="tabs-left">
            <ul class="nav nav-tabs">
                <li {if condition="$pid eq 0" } class="active" {
                /if}><a href="{:Url('index',array('pid'=>0,'fodder'=>$Request.param.fodder,'big'=>$Request.param.big))}">所有分类</a></li>
                {volist name="$typearray" id="vo" key="k"}
                {if condition="$vo.id eq $pid"}
                <li class="active"><a
                            href="{:Url('index',array('pid'=>$vo.id,'fodder'=>$Request.param.fodder,'big'=>$Request.param.big))}">{$vo.name}</a>
                </li>
                {else}
                <li>
                    <a href="{:Url('index',array('pid'=>$vo.id,'fodder'=>$Request.param.fodder,'big'=>$Request.param.big))}">{$vo.name}</a>
                </li>
                {/if}
                {volist name="$vo.child" id="voo" key="kk"}
                <li class="child {if condition=" $voo.id eq $pid
                "} active{/if}">
                <a href="{:Url('index',array('pid'=>$voo.id,'fodder'=>$Request.param.fodder,'big'=>$Request.param.big))}">{if
                    condition="$kk eq count($vo.child)"}└{else/}├{/if}{$voo.name}</a>
                </li>
                {/volist}
                {/volist}
            </ul>
