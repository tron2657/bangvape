            ...
            <div class="layui-card">
                <div class="layui-card">
                    ...
                    <div class="layui-card-body">
                        ...
                        <script type="text/html" id="is_end">
                            <input type='checkbox' name='id' lay-skin='switch' value="{{d.id}}" lay-filter='is_end' lay-text='强制|未强制' {{ d.is_end == 1 ? 'checked' : '' }}>
                        </script>
...
<script>
    ...
    layList.switch('is_end', function(odj, value) {
        if (odj.elem.checked == true) {
            layList.baseGet(layList.Url({
                c: 'trial.trial_active',
                a: 'set_end',
    ...
    layList.tool(function(event, data, obj) {
        switch (event) {
            case 'see':
                $eb.createModalFrame(data.nickname + '-会员详情', layList.Url({
                    c: 'user.user',
                    a: 'see',
                    p: {
                        uid: data.uid
                ...
                $eb.$swal('delete', function() {
                    $eb.axios.get(url).then(function(res) {
                        if (res.status == 200 && res.data.code == 200) {
                            ...
                            return Promise.reject(res.data.msg || '结束失败');
                        }
                    }).catch(function(err) {
                        $eb.$swal('error', err);
                    });
