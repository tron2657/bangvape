...
<body>
<form autocomplete="off" id="signupForm" class="ivu-form ivu-form-label-right form-create" action="{:Url('edit_manage_power')}" method="post">
    <div class="ivu-row">
        <input type="hidden" class="form-control" name="is_post" value="1" validate="" style="width:100%"/>
        <input type="hidden" class="form-control" name="g_id" value="{$g_id}" validate="" style="width:100%"/>
        <input type="hidden" class="form-control" name="manage_type" value="{$manage_type}" validate="" style="width:100%"/>
        <div class="ivu-col ivu-col-span-24">
            <div style="font-size:16px;height: 22px;line-height:22px;font-weight: bold;border-bottom:1px solid #ccc;margin-bottom: 10px">社区管理权限</div>
        </div>
            ...
            <div class="ivu-form-item">
                ...
                <div class="ivu-form-item-content" style="margin-left: 125px;">
                    <div class="ivu-input-wrapper ivu-input-type">
                        ...
                        <input id="fc_{$v['sign']}" name="{$v['sign']}" autocomplete="off" spellcheck="false" type="text" placeholder="请输入{$v.name}"  class="ivu-input" value="{$v['checked']}"  {notempty name='vo.checked'}checked="checked"{/notempty}>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div  class="ivu-radio-group">
                        ...
                        <label class="ivu-radio-wrapper ivu-radio-group-item {notempty name='vo.checked'}ivu-radio-wrapper-checked{/notempty}">
                                <span class="ivu-radio {notempty name='vo.checked'}ivu-radio-checked{/notempty}">
                                    ...
                                    <input type="radio" class="ivu-radio-input" name="{$v['sign']}" value="{$vo['key']}"  {notempty name='vo.checked'}checked="checked"{/notempty}>
                                </span>
                            {$vo['label']}
                        </label>
                        {if condition="count($v['value_show'])>3"}</br>{/if}
                        {/volist}
                    </div>
                </div>
            </div>
        ...
        {/if}
        {/volist}
        <div class="ivu-col ivu-col-span-24" data-role="submit">
            <div class="ivu-col ivu-col-span-24"><button type="button" class="ivu-btn ivu-btn-primary ivu-btn-long ivu-btn-large">
...
<script>
    ...
    $('[data-role="submit"]').click(function () {
        ...
        $.post("{:Url('edit_manage_power')}",data,function (res) {
            if(res.code==200){
