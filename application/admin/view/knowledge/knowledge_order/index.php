                ...
                <div class="layui-card-body">
                    ...
                    <script type="text/html" id="act">
                        ...
                        <ul class="layui-nav-child layui-anim layui-anim-upbit">
                            ...
                            {{# } ;}}
                            <li>
                                <a href="javascript:void(0);" onclick="$eb.createModalFrame('拒绝退款','{:Url('refund_n')}?id={{d.id}}',{w:document.body.clientWidth,h:document.body.clientHeight})">
...
<script>
    ...
    layList.tool(function (event,data,obj) {
        switch (event) {
            case 'marke':
                var url =layList.U({c:'knowledge.knowledge_order',a:'remark'}),
                    id=data.id,
                    make=data.remark;
                ...
                break;
            case 'danger':
                var url =layList.U({c:'knowledge.knowledge_order',a:'take_delivery',p:{id:data.id}});
    ...
    layList.sort(function (obj) {
        ...
        switch (obj.field){
            case 'order_id':
                layList.reload({order: layList.order(type,'a.order_id')},true,null,obj);
                break;
            case 'add_time':
                layList.reload({order: layList.order(type,'a.add_time')},true,null,obj);
                break;
        }
    })
    ...
    require(['vue'],function(Vue) {
        new Vue({
            ...
            methods: {
                ...
                getBadge:function() {
                    var that=this;
                    layList.basePost(layList.Url({c:'knowledge.knowledge_order',a:'getBadge'}),this.where,function (rem) {
                        that.badge=rem.data;
                    });
                },
                search:function () {
                ...
                excel:function () {
                    this.where.excel=1;
                    location.href=layList.U({c:'knowledge.knowledge_order',a:'order_list',q:this.where});
                }
            },
            mounted:function () {
                var that=this;
