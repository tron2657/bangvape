...
<div class="panel">
    <div class="panel-body">
        <form class="form-horizontal" id="signupForm">
            <input type="hidden" name="id" value="{$channel_post_pool_id}">
            ...
            <div class="form-group">
                <div class="col-md-12">
                    <div class="input-group">
                        ...
                        <select class="layui-select layui-input" name="channel_id" id="channel_id" value="{$channel_post.channel_id}">
                        ...
                        <div class="view_block_one">
                            <div class="v_top_user">
                                ...
                                <div class="v_user_info">
                                    <div class="v_nickname">{{post_info['user']['account']}}</div>
                    ...
                    <div v-if="is_news==1" class="view_block">
                        <div v-show="now_num==1" class="view_block_one v_news">
                            <div class="v_one_image_left">
                                ...
                                <div class="v_top_user">
                                    <span class="v_nickname">{{post_info['user']['account']}}</span>
                                    <span>{{post_info['post_time']}}</span>
                                </div>
                            </div>
            ...
            <div id="show_tip" class="tip_info" style="color: red;text-align: center;">该频道已有该内容,提交后将覆盖原有内容设置</div>
            <div id="show_audit_tip" class="tip_info" style="color: red;text-align: center;">该频道中该内容为待审核状态,提交后将覆盖原有内容设置并自动审核通过</div>
        </form>
...
{block name="script"}
<script>
    var already_in_ids="{$already_in_ids}";
    already_in_ids=JSON.parse(already_in_ids);
    var already_in_audit_ids="{$already_in_audit_ids}";
    already_in_audit_ids=JSON.parse(already_in_audit_ids);
    var check_show_tip=function () {
        var channel_id=parseInt($('#channel_id').val());
        if(already_in_ids.includes(channel_id)){
            $('#show_tip').show();
        }else{
            $('#show_tip').hide();
        }
        if(already_in_audit_ids.includes(channel_id)){
            $('#show_audit_tip').show();
        }else{
            $('#show_audit_tip').hide();
        }
    ...
    $(function () {
        $('#channel_id').val("{$channel_post['channel_id']}");
