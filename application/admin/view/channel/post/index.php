                        ...
                        <form class="layui-form layui-form-pane" action="" style="margin-top: 10px">
                            <div class="layui-form-item">
                                ...
                                <div class="layui-inline">
                                    ...
                                    <div class="layui-input-block">
                                        <select name="post_type" v-model="where.post_type" lay-filter="post_type">
                                ...
                                <div class="layui-inline">
                                    ...
                                    <div class="layui-input-block">
                                        <select name="is_hide" v-model="where.is_hide" lay-filter="is_hide">
                ...
                <div class="layui-card">
                    ...
                    <div class="layui-card-body">
                        <div class="layui-btn-container" style="margin-bottom: 2px;margin-top: 10px">
                            ...
                            {eq name="status" value="1"}
                            <button class="layui-btn layui-btn-sm" onclick="hide_post()">批量屏蔽</button>
                            <button class="layui-btn layui-btn-sm" onclick="move_post()">批量移动</button>
                            {/eq}
                            <button class="layui-btn layui-btn-sm" onclick="update_post()" data-type="update_post" style="float: right">立即更新</button>
                        </div>
                        ...
                        <script type="text/html" id="post_content">
                            <div class="c_one_post_content">
                                <div class="c_top_line" style="display: flex;align-items: center">
                                ...
                                <div class="c_content">
                                    <div class="c_left">
                                        ...
                                        <button style="color: #089bf1;
                                            ...
                                            border-radius: 5px;
                                            margin-bottom: 15px;
                                            background: white;" onclick="$eb.createModalFrame('查看视频','{:Url('com.com_thread/view_video')}?id={{d.post_id}}&type=1')">视频</button>
                                        {{# } }}
                        ...
                        <script type="text/html" id="post_status">
                            {{# if(d.is_hide == 1){ }}
                            <p style="color: #ffb64d"><i class="i_dian"></i> 已屏蔽</p>
                            {{# }else if(d.already_end == 1){ }}
                            <p style="color: #ffb64d"><i class="i_dian"></i> 已过期</p>
                            {{# }else{ }}
                            <p style="color: #0092dc"><i class="i_dian"></i> 已推送</p>
                            {{# } }}
                        </script>
                        <script type="text/html" id="act">
                            ...
                            <br/>
                                <button class="layui-btn layui-btn-xs" onclick="$eb.createModalFrame('移动内容','{:Url('reset_channel')}?post_id={{d.post_id}}&now_channel={{d.channel_id}}',{w:600,h:300})">
                                    <i class="fa fa-change"></i> 移动
                                </button>
                                {neq name="channel_id" value="2"}
                                    {{# if(d.is_top == 0){ }}
                                    <a class="layui-btn layui-btn-xs" lay-event='top' href="javascript:void(0);" style="background-color: #ffb64d;" >
                                        <i class="fa fa-top"></i> 置顶
                                    </a>
                                    {{# }else{ }}
                                    <a class="layui-btn layui-btn-xs" lay-event='not_top' href="javascript:void(0);" style="background-color: #ffb64d;" >
                                        <i class="fa fa-top"></i> 取消置顶
                                    </a>
                                    {{# } }}
                                <br/>
                                {/neq}
                            ...
                            <br/>
                            {{# }else{ }}
                            <a class="layui-btn layui-btn-xs" lay-event='hide' href="javascript:void(0);" style="background-color: red;" >
                                <i class="fa fa-warning"></i> 屏蔽
                            </a>
                            {{# } }}
...
<script>
    ...
    //加载列表
    layList.tableList('List',"{:Url('post_list',['channel_id'=>$channel_id])}",function (){
        switch(parseInt(status)){
            case 1:
                var join = [
                    {type:'checkbox'},
                    {field: 'id', title: 'ID', event:'id',width:'8%'},
                    {field: 'post_content', title: '内容信息',templet:'#post_content',width:'40%'},
                    {field: 'post_info', title: '推送信息',templet:'#post_info',width:'16%'},
                    {field: 'post_status', title: '状态',templet:'#post_status',width:'8%'},
    ...
    layList.tool(function (event,data,obj) {
        switch (event) {
            case 'del':
                var url=layList.U({c:'channel.post',a:'delete_post',q:{id:data.id}});
                var code = {title:"是否要删除该内容",text:"是否从当前频道中删除该内容？",confirm:'是的，我要删除'};
                ...
                break;
            case 'hide':
                var url=layList.U({c:'channel.post',a:'hide_post',q:{id:data.id}});
                var code = {title:"是否要屏蔽该内容",text:"屏蔽后可以取消屏蔽",confirm:'是的，我要屏蔽'};
                $eb.$swal('delete',function(){
                    $eb.axios.get(url).then(function(res){
                        if(res.status == 200 && res.data.code == 200) {
                            $eb.$swal('success', res.data.msg);
                ...
                break;
            case 'not_hide':
                var url=layList.U({c:'channel.post',a:'un_hide_post',q:{id:data.id}});
                ...
                break;
            case 'top':
                var url=layList.U({c:'channel.post',a:'top_post',q:{id:data.id}});
                var code = {title:"是否要置顶该内容",text:"置顶后可以取消置顶",confirm:'是的，我要置顶'};
                $eb.$swal('delete',function(){
                    $eb.axios.get(url).then(function(res){
                        if(res.status == 200 && res.data.code == 200) {
                            $eb.$swal('success', res.data.msg);
                ...
                break;
            case 'not_top':
                var url=layList.U({c:'channel.post',a:'un_top_post',q:{id:data.id}});
                var code = {title:"是否要取消置顶该内容",text:"取消置顶后可以再次置顶",confirm:'是的，我要取消置顶'};
                $eb.$swal('delete',function(){
                    $eb.axios.get(url).then(function(res){
                        if(res.status == 200 && res.data.code == 200) {
                            $eb.$swal('success', res.data.msg);
                            ...
                            return Promise.reject(res.data.msg || '取消置顶失败')
                    }).catch(function(err){
                        $eb.$swal('error',err);
                    });
                },code)
                break;
            case 'update_top':break;
            default:
        }
    })
    // 批量删除
    function delete_post(){
        ...
        if(ids.length){
            ...
            var url=layList.U({c:'channel.post',a:'delete_posts',q:{id:str}});
            var code = {title:"是否要批量删除内容",text:"是否从当前频道中批量删除内容？",confirm:'是的，我要删除'};
            $eb.$swal('delete',function(){
                $eb.axios.get(url).then(function(res){
                    if(res.status == 200 && res.data.code == 200) {
                        $eb.$swal('success', res.data.msg);
    ...
    function hide_post(){
        ...
        if(ids.length){
            ...
            var url=layList.U({c:'channel.post',a:'hide_posts',q:{id:str}});
            var code = {title:"是否要批量屏蔽内容",text:"是否从当前频道中批量屏蔽内容？",confirm:'是的，我要屏蔽'};
            $eb.$swal('delete',function(){
                $eb.axios.get(url).then(function(res){
                    if(res.status == 200 && res.data.code == 200) {
                        $eb.$swal('success', res.data.msg);
                        ...
                        return Promise.reject(res.data.msg || '批量屏蔽失败')
                }).catch(function(err){
                    $eb.$swal('error',err);
                });
            },code)
        }else{
            layList.msg('请选择要批量屏蔽的内容');
        }
    }
    ...
    function move_post(){
        ...
        if(ids.length){
            ...
            $eb.createModalFrame('移动内容','{:Url('reset_channel')}?post_id='+str+'&now_channel={$channel_id}&is_type=1',{w:600,h:300})
        }else{
            layList.msg('请选择要批量屏蔽的内容');
        }
    }
    function update_post(){
        parent.$("#loading_content").show();
       //process=setInterval(get_update_process,2000);
       $.post('update_post',{},function (res) {
           parent.$("#loading_content").hide();
           layList.msg(res.msg);
       })
    }
    ...
    require(['vue'],function(Vue) {
        new Vue({
            ...
            mounted:function () {
                ...
                layList.form.render();
                layList.form.on("select(post_type)", function (data) {
                    that.where.post_type = data.value;
                });
                ...
                layList.form.on("select(is_hide)", function (data) {
                    that.where.is_hide = data.value;
                });
                // 定义查看视频点击按钮事件
