...
<div class="row" style="width: 100%;margin-left: 0;">
  <div class="col-sm-12" style="background-color: #fff">
    <div class="layui-card-header">收益说明</div>
    <div class="form" style="margin: 20px">
      <div class="input-box" style="display: flex;align-items: flex-start;">
        <label for="" style="margin-bottom: 0;width: 78px;text-align: right">说明内容</label>
        <textarea type="text/plain" id="myEditor" style="width:700px;margin-left: 10px;">{$agent_income_config}</textarea>
      </div>
...
<script>
  ...
  $('#save_btn').on('click',function(){
    var list = {};
    list.agent_income_config = getContent();
    $.ajax({
      url:"{:Url('saveIncomeStatement')}",
      data:list,
      type:'post',
      dataType:'json',
      success:function(re){
