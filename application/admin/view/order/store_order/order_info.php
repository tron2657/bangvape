...
<div class="ibox-content order-info">
    ...
    {if condition="$orderInfo['paid'] eq 1 && $orderInfo['refund_status'] eq 1" }
        <div class="col-sm-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    退款信息
                ...
                <div class="panel-body">
                    <div class="row show-grid">
                        <div class="col-xs-12" >退款原因: {$orderInfo.refund_reason_wap}</div>
                        <div class="col-xs-12">退款时间: {$orderInfo.refund_reason_time_text}</div>
                        <div class="col-xs-12">退款说明: {$orderInfo.refund_reason_wap_explain}
                        {if condition="$orderInfo['refund_reason_wap_img'] && $orderInfo['refund_reason_wap_img']!=''"}
                    <br>
                             {volist name="orderInfo['refund_reason_wap_img_data']" id="vo"}
                                 <img width="120px" height="120px" src="{$vo}"/>
                            {/volist}
                        {/if}
                        </div>
                    ...
                    <div class="row show-grid">
                        ...
                        <div class="col-xs-6">支付方式:
                            {if condition="$orderInfo['paid'] eq 1"}
                               ...
                               余额支付
                               {elseif condition="$orderInfo['pay_type'] eq 'routine'"}
                                小程序支付
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <div class="row show-grid">
                        ...
                        <div class="col-xs-6">快递单号: {$orderInfo.delivery_id} |
                            {if condition="$orderInfo['no_delivery'] eq 0"}
                                <button class="btn btn-info btn-xs" type="button"  onclick="$eb.createModalFrame('物流查询','{:Url('express',array('oid'=>$orderInfo['id']))}',{w:document.body.clientWidth,h:document.body.clientHeight})">物流查询</button>
                            {/if}
                        </div>
                    </div>
                </div>
