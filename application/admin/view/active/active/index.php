    ...
    <div class="layui-row layui-col-space15" style="margin-top: -27px">
        <div class="layui-col-md12">
            <div class="layui-tab layui-tab-brief" lay-filter="tab">
                <ul class="layui-tab-title" style="background-color: white;top: 10px">
                    ...
                    <a href="{eq name='status' value='1'}javascript:;{else}{:Url('index',['status'=>1])}{/eq}">约酒列表</a>
                    </li>
                    <li lay-id="list" {eq name='status' value='-1'}class="layui-this" {/eq}>
                    <a href="{eq name='status' value='-1'}javascript:;{else}{:Url('index',['status'=>-1])}{/eq}">失效列表</a>
                    </li>
                </ul>
            </div>
        ...
        <div class="layui-col-md12" >
            <div class="layui-card">
                <div class="layui-card">
                    <div class="layui-card-header">约酒列表</div>
                    <div class="layui-card-body">
...
<script>
    ...
    layList.tableList('List',"{:Url('get_list',['status'=>$status])}",function (){
        if(status==-1){
            var join = [
                ...
                {field: 'event', title: '活动'},
                {field: 'store', title: '活动门店'},
                
                {field: 'invalid_time', title: '失效时间'},
            ...
            var join = [
                ...
                {field: 'id', title: 'ID', event:'id',width:'5%'},
                {field: 'event', title: '活动'},
                {field: 'store', title: '活动门店',width:'18%'},
                {field: 'event_time', title: '活动时间',width:'10%'},
                {field: 'enroll_time', title: '报名时间',width:'10%'},
                {field: 'condition', title: '条件',width:'6%'},
                // {field: 'pattern', title: '付费模式',width:'6%'},
                // {field: 'check_user', title: '核销',width:'10%'},
                // {field: '', title: '活动门店',width:'10%'},
                {field: 'invalid_time_show', title: '失效/组局/领取',width:'15%'},
                {field: 'record', title: '数据'},
    ...
    layList.switch('status',function (odj,value) {
        if(odj.elem.checked==true){
            layList.baseGet(layList.Url({c:'active.event',a:'set_event_status',p:{status:1,id:value}}),function (res) {
                layList.msg(res.msg);
            });
        }else{
            layList.baseGet(layList.Url({c:'active.event',a:'set_event_status',p:{status:0, id:value}}),function (res) {
                layList.msg(res.msg);
            });
        }
    });
    ...
    layList.tool(function (event,data,obj) {
        switch (event) {
            case 'see':
                ...
                break;
            case 'set_hot':
                var url=layList.U({c:'active.active',a:'set_recommend',q:{id:data.id, field:'is_recommend', value: data.is_recommend == 1?0:1}});
                ...
                break;
            case 'delete':
                var url=layList.U({c:'active.active',a:'set_event_status',q:{id:data.id, status:-1}});
                ...
                break;
            case 'open':
                layList.basePost(layList.Url({c:'active.active',a:'set_event_status'}),{id:data.id,'status':1},function (res) {
