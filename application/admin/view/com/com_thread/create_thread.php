                ...
                <div class="col-md-12">
                    <div class="input-group" style="display: flex">
                        <label style="display: flex;align-items: center">
                            ...
                            <input type="checkbox" id="show_upload" name="show" style="margin-top: 0;" checked>自动获取封面（从文章中获取前9张图片）,仅支持编辑器中手动上传的图片，不支持外部复制的图片
                            {elseif condition="$info.is_auto_image eq '0'"/}
                            <input type="checkbox" id="show_upload" name="show" style="margin-top: 0;">自动获取封面（从文章中获取前9张图片）,仅支持编辑器中手动上传的图片，不支持外部复制的图片
                            {/if}
                        </label>
                    </div>
                </div>
...
<script>
  ...
  window.addEventListener("storage", function (e) {
    if (e.key === "bind_username") {
      ...
      goodsArr.push(goodsVal.id);
      window.localStorage.removeItem("add_goods_val")
    } else if (e.key === "bind_userId") {
        console.log(e.newValue)
        console.log($(".now_bind_user"))
      $(".now_bind_user").data('id', e.newValue);
    ...
    $.ajax({
      ...
      success: function (re) {
        if (re.code == 200) {
          ...
          setTimeout(function (e) {
            //parent.$(".J_iframe:visible")[0].contentWindow.location.reload();
              parent.$(".J_iframe:visible")[0].contentWindow.$(".layui-laypage-skip").find("input").val("{$page}");
              parent.$(".J_iframe:visible")[0].contentWindow.$(".layui-laypage-btn").click();
            parent.layer.close(parent.layer.getFrameIndex(window.name));
          }, 600)
        } else {
          $eb.message('error', re.msg);
