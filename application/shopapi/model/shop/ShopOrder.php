...
class ShopOrder extends ModelBasic
{
    ...
    public static function cacheOrderInfo($uid,$cacheTime = 600)
    {
        $key = md5(time());
        Cache::set('shop_user_order_'.$uid.$key,$key,$cacheTime);
        return $key;
    }
    ...
    public static function cacheKeyCreateOrderNew($uid,$productInfo,$totalNum,$addressId,$mark = '')
    {
        ...
        if($productInfo['limit_num']>0){
            $user_nums=self::where('uid',$uid)->where('product_id',$productInfo['id'])->where('status','>',-1)->sum('total_num');
        ...
        $user_order=[
            ...
            'status'=>2,
            'create_time'=>time(),
            'bind_table'=>'shop_order',
            'order_type'=>3,
            'amount'=>$payPrice,
            'unique'=>md5($order_id),
            'info'=>'购买'.$productInfo['store_name'],
        ];
...
//        if($orderInfo['pay_type'] != 'yue') return self::setErrorInfo('该订单不能使用余额支付!');
    ...
    public static function MiniProgramJsPay($orderId,$field = 'order_id')
    {
        ...
        return MiniProgramService::jsPay($openid,$orderInfo['order_id'],$orderInfo['pay_price'],'shop',SystemConfigService::get('site_name'));
    }
    ...
    public static function wechatAppPay($orderId,$field = 'order_id')
    {
        ...
        if($orderInfo['paid']) exception('支付已支付!');
        if($orderInfo['pay_price'] <= 0) exception('该支付无需支付!');
        return WechatAppService::AppPay($orderInfo['order_id'],$orderInfo['pay_price'],'product_shop',SystemConfigService::get('website_name').'-微信APP支付');
    }
    /**
     * 微信支付 为 0元时
     * @param $order_id
    ...
    public static function paySuccess($orderId,$paytype='weixin',$formId = '')
    {
        ...
        //新的支付统计中的内容绑定
        $res4=db('user_order')->where(['order_id'=>$orderId])->update(['pay_type'=>$order['pay_type'],'status'=>1,'pay_time'=>time()]);
    ...
    public static function tidyOrder($order,$detail = false,$isPic=false)
    {
        ...
        if($order['paid']==1 && $order['status'] == 1){
            ...
            $status['_class'] = 'state-sqtk';
        }else if($order['paid']==0 && $order['status'] == 0){
            $status['_type'] = 0;
            $status['_title'] = '待支付';
            $status['_msg'] = '请支付订单';
            $status['_class'] = 'state-sqtk';
