...
class AuthApi extends AuthController
{
    ...
    public function confirm_order()
    {
        ...
        $data['orderKey'] = ShopOrder::cacheOrderInfo($this->userInfo['uid']);
    ...
    {
        list($midkey,$productId,$addressId, $totalNum, $mark,) = UtilService::postMore([
            'midkey','productId','addressId', 'totalNum', 'mark',
        ], Request::instance(), true);
        $iv = "1234567890123412";//16位 向量
        $key= '201707eggplant99';//16位 默认4秘钥
        $midkey=trim(openssl_decrypt(base64_decode($midkey),"AES-128-CBC",$key,OPENSSL_RAW_DATA,$iv));
        $oldkey=Cache::get('shop_user_order_'.$this->userInfo['uid'].$midkey);
        if($oldkey!=$midkey){
            JsonService::fail('订单信息不存在或已过期！');
        }
//        if (ShopOrder::be(['order_id|unique' => $midkey, 'uid' => $this->userInfo['uid'], 'status' => 1])){
            ...
            switch ($order['pay_type']) {
                ...
                case 'yue':
                    ...
                    if ($res = ShopOrder::yuePay($order['order_id'], $this->userInfo['uid'],'',$bill_type))
                        ...
                        $errorinfo = ShopOrder::getErrorInfo();
                ...
                case 'weixin_app':
                    ...
                    try {
                        $appConfig = ShopOrder::wechatAppPay($order); //订单列表发起支付
                        for($i=0;$i<3;$i++){
                            RoutineFormId::SetFormId($appConfig['prepayid'], $this->uid);//多个地方用到表单令牌
                        }
                        ShopOrder::where('id', $order['id'])->update(['pay_type'=>'weixin_app']);
                    } catch (\Exception $e) {
                        return JsonService::fail($e->getMessage());
                    }
                ...
                case 'routine':
                    ...
                    try {
                        $jsConfig = ShopOrder::MiniProgramJsPay($order); //订单列表发起支付
                        if(isset($jsConfig['package']) && $jsConfig['package']){
                            $jsConfig['package']=str_replace('prepay_id=','',$jsConfig['package']);
                            for($i=0;$i<3;$i++){
                                RoutineFormId::SetFormId($jsConfig['package'], $this->uid);
