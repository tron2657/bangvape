    ...
    public static function getListPage($map,$page=1,$row=10,$order='create_time desc')
    {
        ...
        foreach ($order_list as &$val){
            ...
            if(count($goods_ids)){
                ...
                foreach ($val['goods_info_list'] as $k => &$goods){
                    if (isset($goods_detail[$goods['product_id']])) {
                        ...
                        $column = db('column_text')->where('id',$goods['product_id'])->field('id,name,image')->find();
                        if (empty($column)) {
                            array_splice($val['goods_info_list'],$k,1);
                            continue;
                        }
                        $goods['store_name'] = $column['name'];
                        $goods['image'] = $column['image'];
                        $goods['image_150']=thumb_path($goods['image'],150,150);
                        $goods['image_350']=thumb_path($goods['image'],350,350);
                        $goods['image_750']=thumb_path($goods['image'],750,750);
                    }
