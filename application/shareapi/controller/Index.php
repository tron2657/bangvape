...
class Index extends ControllerBasic
{
            ...
            switch ($agent_config['agent_way']) {
                ...
                case 3://购买商品自动生效
                    ...
                    if($agent_config['goods_type']=='one_goods'){
                        ...
                        $how_to_seller['goods']=db('column_text')->find($agent_config['goods_id']);
                    }
                    break;
                default:
            }
    ...
    private function _delHaiBao($code_url)
    {
        ...
        $font_color=SystemConfig::getValue('invite_font_color');
        if($font_color==0){
            $color="255,255,255";
        }else{
            $color="0,0,0";
        ...
        $nickname_color=$color;//rgb的三元素，号拼接
...
//        $avatarImg =imagecreatefromstring(file_get_contents($avatar));
        ...
        $code_word_color=$color;//rgb的三元素，号拼接
        $code_word_color=explode(',',$code_word_color);

        //字体文件
        // $word_font_file= strtoupper(substr(PHP_OS,0,3))==='WIN'?realpath(PUBILC_PATH.'/static/font/SourceHanSansCN-Normal.ttf'):PUBILC_PATH.'/static/font/SourceHanSansCN-Normal.ttf';
        $word_font_file= strtoupper(substr(PHP_OS,0,3))==='WIN'?realpath('.\public\static\font\SourceHanSansCN-Normal.ttf'):PUBILC_PATH.'/static/font/SourceHanSansCN-Normal.ttf';
        //旋转角度
        $word_circleSize = 0;
        ...
        if($invite_show==1){
            ...
            $url= explode('#',$code_url);
            if(count($url)>0)
            {
                $url=$url[1];
                $code_url=ShareLink::gen_share_link($uid,$url,'haibao');
            }
        }
        ...
        foreach ($haiBaoList as &$val) {
            ...
            if ($is_seller || $invite_show == 1) {
                if($val['colour']==1){
                    ...
                    $code_word_color_pic = imagecolorallocate($bigImg,255,255,255);
                }
...
//                imagefttext($bigImg, $code_word_fontSize, $word_circleSize, $code_word_x, $code_word_y, $code_word_color_pic, $word_font_file, $invite_code);
            ...
            $val['url']=get_root_path($file_name,true).'?'.time();
            $i++;
        }
        unset($val);
        /*if(isset($file_name)){
    ...
    {
        //判断图片是否有效
        $re=get_headers($imgpath);
        if(preg_match('/200/',$re[0])){
            $uid=get_uid();
            if(file_exists($imgpath)){
                $imgpath='public/uploads/common/user/'.'avatar_'.$uid.'.png';
            }else{
                $imgpath=self::getImage($imgpath,'public/uploads/common/user/','avatar_'.$uid.'.png',1);
            }
...
//                }
    ...
    */
    ...
    public static function getImage($url,$save_dir='',$filename='',$type=0){
        ...
        fwrite($fp2,$img);
        fclose($fp2);
        (!file_exists($save_dir)&&!mkdir($save_dir.$filename,0777,true));
        unset($img,$url);
