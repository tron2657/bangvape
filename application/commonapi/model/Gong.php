...
class Gong extends ModelBasic
{
    ...
    public  static function finishtask($jifenflag,$tasktablename,$uidflag)
    {
        ...
        if($guanzhu['status'] == 1){
            ...
            if($jifenflag == 'dianzan'){
                ...
                $followcount = db($tasktablename)->whereTime('create_time',date('Y-m-d'))->where($uidflag,$uid)->where('is_thread',0)->count();
            }else{
    ...
    public static function actionadd($actionflag,$actiontable,$uidflag,$uid=0)
    {
        ...
        }elseif($actionflag == 'goumaishangpin'){
            $count = db($actiontable)->where($uidflag,$uid)->where('is_zg',0)->whereTime('add_time',date('Y-m-d'))->count();
        }elseif($actionflag == 'goumaizhishishangpin'){
            $count = db($actiontable)->where($uidflag,$uid)->where('is_zg',1)->whereTime('add_time',date('Y-m-d'))->count();
        }elseif($actionflag == 'fashangpinpingjia'||$actionflag == 'fazhishishangpinpingjia'){
            $count = db($actiontable)->where($uidflag,$uid)->whereTime('add_time',date('Y-m-d'))->count();
        }elseif($actionflag == 'shangpinshoucang'){
            $count = db($actiontable)->where($uidflag,$uid)->whereTime('add_time',date('Y-m-d'))->where('is_zg',0)->where('type','collect')->where('category','product')->count();
        }else{
            $count = db($actiontable)->whereTime('create_time',date('Y-m-d'))->where($uidflag,$uid)->count();
        }
        $res=Support::addjifen($guanzhu,$count,$uid) ;
        $info='';
        if($res===1){
            if($guanzhu['expone']!=0){
                $exp_name=db('system_rule')->where('flag','exp')->where('status',1)->where('is_del',0)->value('name');
                if($exp_name){
                    $info=$info.$exp_name.'+'.$guanzhu['expone'].' ';
                }
            }
            if($guanzhu['flyone']!=0){
                $exp_name=db('system_rule')->where('flag','fly')->where('status',1)->where('is_del',0)->value('name');
                if($exp_name){
                    $info=$info.$exp_name.'+'.$guanzhu['flyone'].' ';
                }
            }
            if($guanzhu['gongone']!=0){
                $exp_name=db('system_rule')->where('flag','gong')->where('status',1)->where('is_del',0)->value('name');
                if($exp_name){
                    $info=$info.$exp_name.'+'.$guanzhu['gongone'].' ';
                }
            }
            if($guanzhu['buyone']!=0){
                $exp_name=db('system_rule')->where('flag','buy')->where('status',1)->where('is_del',0)->value('name');
                if($exp_name){
                    $info=$info.$exp_name.'+'.$guanzhu['buyone'].' ';
                }
            }
            if($guanzhu['firstone']!=0){
                $exp_name=db('system_rule')->where('flag','first')->where('status',1)->where('is_del',0)->value('name');
                if($exp_name){
                    $info=$info.$exp_name.'+'.$guanzhu['firstone'].' ';
                }
            }
            if($guanzhu['twoone']!=0){
                $exp_name=db('system_rule')->where('flag','two')->where('status',1)->where('is_del',0)->value('name');
                if($exp_name){
                    $info=$info.$exp_name.'+'.$guanzhu['twoone'].' ';
                }
            }
            if($guanzhu['threeone']!=0){
                $exp_name=db('system_rule')->where('flag','three')->where('status',1)->where('is_del',0)->value('name');
                if($exp_name){
                    $info=$info.$exp_name.'+'.$guanzhu['threeone'].' ';
                }
            }
            if($guanzhu['fourone']!=0){
                $exp_name=db('system_rule')->where('flag','four')->where('status',1)->where('is_del',0)->value('name');
                if($exp_name){
                    $info=$info.$exp_name.'+'.$guanzhu['fourone'].' ';
                }
            }
            if($guanzhu['fiveone']!=0){
                $exp_name=db('system_rule')->where('flag','five')->where('status',1)->where('is_del',0)->value('name');
                if($exp_name){
                    $info=$info.$exp_name.'+'.$guanzhu['fiveone'].' ';
                }
            }
        }
        return $info;
    ...
    //完成行为加积分(点赞、评论、收藏、发帖)
    public static function actionAddAll($actionflag,$uid=0)
    {
        $uid=empty($uid) ? get_uid() : $uid;
        $guanzhu = db('system_rule_action')->where('actionflag',$actionflag)->find();

        $res=Support::addjifenAll($guanzhu,$uid) ;
        return $res;
    }
    ...
    //扣分
    public static function delaction($actionflag,$uid,$name)
    {

        $guanzhu = db('system_rule_action')->where('actionflag',$actionflag)->find();

        Support::delsubjifen($guanzhu,$uid,$name) ;
