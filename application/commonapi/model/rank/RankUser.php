...
class RankUser extends ModelBasic
{
    ...
    public static function getList($type,$uid,$page,$row){
        if($type=='all'){
            /*$gi_table = self::order('rank asc')->buildSql();//先排序
            $list = self::table($gi_table .'as gi')
                ->field('gi.*')
                ->where('status',1)
                ->page($page,$row)
                ->select()->toArray();*/
            $list=self::where('status',1)->where('fans','>',0)->order('rank asc')->page($page,$row)->select()->toArray();
        }else{
            /*$gi_table = self::order('week_rank asc')->buildSql();//先排序
            $list = self::table($gi_table .'as gi')
                ->field('gi.*')
                ->where('status',1)
                ->page($page,$row)
                ->select()->toArray();*/
            $list=self::where('status',1)->where('new_fans','>',0)->order('week_rank asc')->page($page,$row)->select()->toArray();
        }
