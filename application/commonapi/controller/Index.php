...
class Index extends ControllerBasic
{
        ...
        switch ($type){
            case 'all':
                ...
                foreach ($log as &$val){
                    foreach ($type as $key=>$value){
                        if($val[$value['flag']]!=0) {
                            $score_value['name']=$value['name'];
                            if ($val['type'] == 0) {
                                $score_value['value'] = -$val[$value['flag']];
                            } else {
                                $score_value['value'] = $val[$value['flag']];
                            }
     ...
     * 获取缓存结构信息
     */
    public function get_cache_flush_list(){
        $data=db('cache_flush')->select();
        $this->apiSuccess($data);
    }
    ...
    public function get_site_service(){
        $data['site_service_avatar']=get_root_path(SystemConfig::getValue('site_service_avatar'));
        $data['site_service_nickname']=SystemConfig::getValue('site_service_nickname');
        $data['site_service_identity']=SystemConfig::getValue('site_service_identity');
        $data['site_service_introduce']=SystemConfig::getValue('site_service_introduce');
        $data['shop_phone']=SystemConfig::getValue('shop_phone');
        $data['service_code']=get_root_path(SystemConfig::getValue('service_code'));
        $this->apiSuccess($data);
    }
