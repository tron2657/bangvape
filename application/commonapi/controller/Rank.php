...
class Rank extends ControllerBasic
{
            ...
            foreach($list as &$value){
                switch($value['id']){
                    ...
                    case 5;
                        ...
                        break;
                    case 6;
                        $user=RankUser::where('status',1)->where('new_fans','>',0)->order('rank asc')->limit(3)->column('nickname');
    ...
    public function UserRankList(){
        ...
        switch($type){
            case 'all':
                ...
                if(!$list){
                    $list=RankUser::getList($type,$uid,$page,$row);
        ...
        $data['rank']=$rank;
        $rank_one = UserModel::where('uid',$data['rank']['uid'])->find();
        if($rank_one['cate_id']){
            $datum_data=db('certification_entity')->where('uid',$rank_one['uid'])->where('status',1)->where('cate_id',$rank_one['cate_id'])->value('datum_data');
            $datum_data=unserialize($datum_data);
            if(!empty($datum_data['rztx'])){
                $data['rank']['rztx']=$datum_data['rztx'];
            }
        }
        else{
            $data['rank']['rztx']='';
        ...
        foreach($data['list'] as $k=>$v){
            $one = UserModel::where('uid',$v['uid'])->find();
            if($one['cate_id']){
                $datum_data=db('certification_entity')->where('uid',$v['uid'])->where('status',1)->where('cate_id',$one['cate_id'])->value('datum_data');
                $datum_data=unserialize($datum_data);
                if(!empty($datum_data['rztx'])){
                    $data['list'][$k]['rztx']=$datum_data['rztx'];
                }
            }
            else{
                $data['list'][$k]['rztx']='';
    ...
    public function ThreadRankList(){
        ...
        foreach ($list as &$val){
            ...
            $val['update_time']=time_to_show($val['update_time']);
