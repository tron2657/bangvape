...
use app\admin\controller\com\ComThreadClassExtend;
...
use app\osapi\model\com\ComThreadDraft;
...
use app\osapi\model\com\ComForumAdminScore;
use app\admin\model\com\ComThread as AdminThreadModel;
use think\Db;
                    ...
                    if(!$postListCache){
                        ...
                        if($user_recommend_list){
                            ...
                            if($count>99){
                                ...
                                $thr=ComThread::where($map)->field('*')->having('1')->order('create_time desc')->buildSql();
                                $thread = Db::table($thr.' a')->order('a.create_time desc')->group('a.author_uid')->select();
                ...
                if($uid){
                    ...
                    if(!$forum_ids){
                        ...
                        if($access[1] == '微信小程序' && $video_is_on==0){
                            ...
                            $forum_ids=ComForumMember::where('uid',$uid)->where('status',1)->where('fid','in',$ids)->where('fid','in',$fids)->where('fid','not in',$video_forum)->limit(200)->column('fid');
                        }else{
                            $forum_ids=ComForumMember::where('uid',$uid)->where('status',1)->where('fid','in',$ids)->where('fid','in',$fids)->limit(200)->column('fid');
                        }
...
//                ComThread::where('id','in',$weibo_ids)->setInc('view_count');
        ...
        $list=[];
        if(!empty($userList)){
            if(is_array($userList['post'])){
                foreach ($userList['post'] as $v){
                    $list[]=$v;
                }
            }
        }
        ...
        unset($v);
        if(!empty($forum_list)&&in_array('thread_list',$forum_list)){
            foreach ($forum_list['thread_list'] as $v){
                $list[]=$v;
            }
        }
        unset($v);
    ...
    public function threadList()
    {
        ...
        $sort = input('post.sort', 'send_time desc,create_time desc');
    ...
    public function sendThread()
    {
        ...
        if($from == 'PC'){
            ...
            //特殊情况处理结构
            $image_change=json_decode($image,true);
            if (!is_array($image_change)) $image_change = [];
            $new_image=[];
            foreach ($image_change as $v){
                if(is_array($v)){
                    if(isset($v['image'])){
                        $new_image[]=$v['image'];
                    }
            ...
            unset($v);
            $image=count($new_image)>0?json_encode($new_image):$image;
        }

        if($class_id>0)
        {
            $time=time();
            $class_extend=\app\admin\model\com\ComThreadClassExtend::where('class_id',$class_id)->find();
            if($class_extend && $class_extend['tg_start_time']<$time && $class_extend['tg_end_time']<=$time)
            {
                $this->apiError(['info'=>'当前投稿活动已结束']);
            }                   
        }
        ...
        $column_id = osx_input('column_id', 0);//知识付费商品分享
        $id = input('post.id', 0);
        ...
        $colum_count = explode(",", $column_id);
        $colum_count = count($colum_count);
        if($colum_count>0&&$column_id!=0){
            action_power('share_goods',$uid);
        }
        ...
        if($type==1&&$is_weibo==0){
            ...
            if (mb_strlen(text($title),'UTF-8') > $title_limit) {
                $this->apiError(['info'=>'标题过长，限制为'.$title_limit.'字']);
            }
            ...
            if($content_limit > 0){
                if (mb_strlen(text(htmlspecialchars_decode($content)),'UTF-8') > $content_limit) {
                    $this->apiError(['info'=>'内容过长，限制为'.$content_limit.'字']);
                }
            }
            if($product_limit > 0){
                if ($product_count > $product_limit){
                    $this->apiError(['info'=>'商品分享数量超过上限:'.$product_limit]);

                }
                if ($colum_count > $product_limit){
                    $this->apiError(['info'=>'知识商品分享数量超过上限:'.$product_limit]);

                }
            }
        }elseif($type==6&&$is_weibo==0){
            if($video_url==''||$video_id==''){
                $this->apiError(['info'=>'请先上传视频']);
            }
            //发视频权限
            ...
            if (mb_strlen(text($title),'UTF-8') > $title_video) {
                $this->apiError(['info'=>'标题过长，限制为'.$title_video.'字']);
            }
            ...
            if($content_video>0){
                if (mb_strlen(text(htmlspecialchars_decode($content)),'UTF-8') > $content_video) {
                    $this->apiError(['info'=>'内容过长，限制为'.$content_video.'字']);
                }
            }
            if($product_limit_video > 0){
                if ($product_count > $product_limit_video) {
                    $this->apiError(['info'=>'商品分享数量超过上限:'.$product_limit_video]);
                }
                if ($colum_count > $product_limit_video){
                    $this->apiError(['info'=>'知识商品分享数量超过上限:'.$product_limit_video]);

                }
            }
            if (is_array($cover_type) && in_array('1', $cover_type) && $video_cover == '' || $cover_type == '1') {
                $animatedGraphic = AdminThreadModel::getAnimatedGraphic($video_id);
                if ($animatedGraphic != '') {
                    $video_cover = $animatedGraphic;
                }
            }
        }elseif($is_weibo==1){
            //发微博权限
            ...
            if($limit_weibo_content>0){
                if (mb_strlen(text(htmlspecialchars_decode($content)),'UTF-8') > $limit_weibo_content) {
                    $this->apiError(['info'=>'内容过长，限制为'.$limit_weibo_content.'字']);
                }
            }
            if($limit_weibo_store > 0){
                if ($product_count > $limit_weibo_store) {
                    $this->apiError(['info'=>'商品分享数量超过上限:'.$limit_weibo_store]);
                }
                if ($colum_count > $limit_weibo_store){
                    $this->apiError(['info'=>'知识商品分享数量超过上限:'.$limit_weibo_store]);
        ...
        if($access[1] == '微信小程序'){
            ...
            $weixin['content']=$title;
            $url = "https://api.weixin.qq.com/wxa/msg_sec_check?access_token=".$token['access_token'];
            $info = $this->http_request($url,json_encode($weixin,JSON_UNESCAPED_UNICODE));
            $info =  json_decode($info,true);
            if($info['errcode']==87014){
                $this->apiError(['info'=>'标题含有违法违规内容，请重新发布']);
            }
        }
        ...
        if($oid>0){
            ...
            if($fid=='')
            {
                $tmpId=ComTopic::where('id',$oid)->where('status','>',0)->value('fid');
                if($tmpId!=null)
                {
                    $fid= $tmpId;
        ...
        if ($result) {
            ...
            if($id==0){
                ...
                if($set1['status']==1){
                    ...
                    $message_list1=Message::where($map1)->select()->toArray();//为什么这里还要查询一次数据库
                    // $message_list1=$message1;
                    $data3['is_read']=0;
                    if($set1['popup']==1){
                        $data3['is_popup']=0;
                    }else{
                ...
                if($is_admin['admin_one']==1 || $is_admin['admin_two']==1){
                    ...
                    if($set2['status']==1){
                        ...
                        //最新消息内容更新
                        unset($data4['to_uid']);
                        Message::update_message_census($forum_list,'message_new_send',$data4);
                        //最新消息内容更新end

                        //发送app消息
//                        foreach ($message2 as $v){
//                            app_send_message_one($v['to_uid'],$v['content'],$v['route'],$v['link_id'],$v['title']);
//                        }
//                        unset($v);
            ...
            if($id==0){
                ...
                $is_audit= ComForum::where('id',$fid)->value('is_audit');//是否开启人工审核 1开启 0关闭
                if($is_audit==0){
                    //增加行为积分
                    if($is_weibo==1){
                        $jf_info=Gong::actionadd('fadongtai','com_thread','author_uid') ;
                        $name_info='动态';
                    }elseif($type==6){
                        $jf_info=Gong::actionadd('fashipin','com_thread','author_uid') ;
                        $name_info='视频';
                    }else{
                        $jf_info=Gong::actionadd('fatie','com_thread','author_uid') ;
                        $name_info='帖子';
                    }
                }
            ...
            $res['info']=$name_info.'发布成功'.$info.' '.$jf_info;
            ...
            if($id){
                Cache::rm('thread_detail_'.$id);//编辑帖子时清除帖子缓存
                if($status==2){
                    Channel::delPost($id);
                }
            }
    ...
    public function forumIndex()
    {
        ...
        if( $other_info['detail_top'] ){
            $is_follow=db('user_follow')->where('uid',$uid)->where('follow_uid','in',$other_info['detail_top']['author_uid'])->cache('follow_uid_'.$uid.'_to_uid_'.$other_info['detail_top']['author_uid'],3600)->where('status',1)->count();
            $other_info['detail_top']['user']['is_follow']=$is_follow?1:0;
        }
     ...
     * 告示牌(论坛型)
     */
    public function forum_fiction_Index()
    {
        $uid = get_uid();
        $forumId=input('fid',0,'intval');
        $forum['forum'] = ComForum::getForumDetail($forumId, $uid);  //获取版块详情信息
 
        $childrenData=\app\osapi\model\com\ComForum::where('pid',$forumId)->field('id,title,content,logo')->select();
        $threadClassSql=Db::view('com_thread_class a','fid,name,status')->view('com_thread_class_extend b',true,'a.id=b.class_id')->buildSql();
        
        foreach($childrenData as &$item)//遍历当前主版块的子板块（章节）
        {          
            $item['runing_state']=0;//未启用
            $classData=Db::table($threadClassSql.' temp')->where('fid',$item['id'])->select();//查找该板块下的所有分类

            $totalCount=sizeof($classData);
            $finishCount=0;
            $runingCount=0;
            $notStarCount=0;
            $enableCount=0;
            ...
            {
                if($classItem['tg_enable']==1)
                {
                    $state=\app\admin\model\com\ComThreadClassExtend::get_runing_state($classItem['tg_start_time'],$classItem['tg_end_time']);
                    if($state==1)//进行中
                    {
            ...
            {
                $item['runing_state']=0;
            }else if($totalCount==$finishCount)
            {
                $item['runing_state']=3;//已经结束状态
            }
            else if($totalCount==$notStarCount)
            {
                $item['runing_state']=2;
            }
            else if($totalCount==$enableCount)
            {
                $item['runing_state']=0;
            }
            else if($runingCount>0)
            {
                $item['runing_state']=1;
            }  

            // if($classItem && $classItem['tg_enable']==1)
            // {
    
            // } 
            // $item['is_runing']=Db::table($threadClassSql.' temp')->field('count(1) as c')
            // ->where('fid',$item['id'])
            // ->where('tg_enable',1)
            // ->where('tg_start_time','<=',$time)
            // ->where('tg_end_time','>',$time)
            // // ->where('')
            // ->find()['c'];
        }

        $forum['forum']['children']=  $childrenData;
        $this->apiSuccess($forum);
    }
    ...
    // {
    //    \app\core\behavior\EventBehavior::eventSureCheckAfter(['uid'=>25],null);
    // }

    public function forum_fiction_detail()
    {
        $uid = get_uid();
        $forumId=input('fid',0,'intval');
        $forum['forum'] = ComForum::getForumDetail($forumId, $uid);  //获取版块详情信息
        $time=time();
        $threadClassSql=Db::view('com_thread_class a','fid,name,status')->view('com_thread_class_extend b',true,'a.id=b.class_id')->buildSql();
        $childrenData=Db::table($threadClassSql.' temp')
        ->where('fid',$forumId)
        ->where('tg_enable',1)
        ->where('tg_start_time','<=',$time)
        // ->where('tg_end_time','>',$time)
        ->field('name,class_id id,tg_start_time,tg_end_time')
        ->select();
        ...
        {
            $item['runing_state']=\app\admin\model\com\ComThreadClassExtend::get_runing_state($item['tg_start_time'],$item['tg_end_time']);
            $item['runing_state_text']=$stateData[$item['runing_state']] ;
            // ->where('fid',$item['id'])
            // ->where('tg_enable',1)
            // ->where('tg_start_time','<=',$time)
            // ->where('tg_end_time','>',$time)
        ...
        $forum['forum']['runing']=  $childrenData;
        $this->apiSuccess($forum);
    }
    ...
    public function getIndexTop()
    {
        ...
        //接口返回进行阅读量统计
        ComSite::views($other_info['index_top']);
        //end
        $this->apiSuccess($data);
    }
    ...
    public function addToForum()
    {
        ...
        $pid=ComForum::where('id',$forumId)->value('pid');
        if($pid==0) {
            $this->apiError(['info'=>'参数id不是版块id！']);
        }
        $action='audit';
        ...
        if($res!==false){
            ...
            //加入版块 加分
            $this->apiSuccess(['info'=>'操作成功！'.$res]);
        }else{
            $this->apiError(['info'=>'操作失败！']);
        }
    }
    ...
    public function postAdd()
    {
        ...
        if ($result) {
            ...
            if($uid!=$to_reply_uid){
                ...
                if($level==2){
                    ...
                    if($set['status']==1){
                        ...
                        $message_id=Message::sendMessage($to_reply_uid,$uid,$template,2,$set['title'],2,'','reply',$to_reply_thread_id,$to_reply_id,$result,$uid);
                    ...
                    if($type==1){
                        ...
                        if($set['status']==1){
                            $template=str_replace('{用户昵称}', $nickname, $set['template']);
                            $message_id=Message::sendMessage($to_reply_uid,$uid,$template,2,$set['title'],2,'','reply',$to_reply_thread_id,0,$result,$uid);
                            MessageRead::createMessageRead($to_reply_uid,$message_id,$set['popup'],2);
                        }
                    }elseif($type==4){
                    ...
                    RoutineTemplateService::sendTemplate(WechatUser::getMiniOpenId($to_reply_uid),RoutineTemplateService::NEW_POST, [
                        'thing2'=>['value'=>$content],
                        'time3'=>['value'=>date('Y/m/d H:i',time())],
                        'thing5'=>['value'=>$nickname],
                        'thing6'=>['value'=>$thread['title']],
                    ],'','/packageA/post-page/post-page?id='.$thread_id);
                }
            }
            //加任务积分
            ...
            //行为加积分
            $info=Gong::actionadd('pinglun','com_post','author_uid');
            $res['post_id']=$result;
            $res['info']='发布成功'.''.$info;
            census('comment',1);
            Cache::rm('thread_detail_'.$thread_id);
...
//            Cache::clear('thread_detail_'.$thread_id);
            Cache::rm('thread_detail_view_num_'.$thread_id);
            if($level==2){
    ...
    public function threadInfo()
    {
        ...
        website_connect_notify($now_login,$thread_id,$uid,'osapi_see_com_thread');//通知第三方平台，任务回调
        $this->apiSuccess($data);
    }
    ...
    public function announce(){
        $list=db('com_announce')->where('status',1)->where('start_time','<',time())->where(function ($query) {
    ...
    public function deleteCom(){
        ...
        if($old_status==-1){
            $this->apiError(['info'=>'该帖子已删除，请勿重复删帖！']);
        }
        ...
        if($res!==false&&$res2!==false){
            ...
            if($del_user=='forum_admin'){
                ...
                if($set['status']==1){
                    $message_id=Message::sendMessage($uid,$now_uid,$template,1,$set['title'],1,'index');
            ...
            $is_weibo = ComThread::where('id', $id)->value('is_weibo');
            $type = ComThread::where('id', $id)->value('type');
            if($is_weibo==1){
                Gong::delaction('fadongtai',$uid,'动态被删除');
            }elseif($type==6){
                Gong::delaction('fashipin',$uid,'视频被删除');
            }else{
            ...
            Channel::delPost($id);
            $this->apiSuccess(['info'=>'删除成功']);;
        }else{
            $this->apiSuccess(['info'=>'删除失败']);
        }
    }
    ...
    public function deleteComPost(){
        ...
        if($res!==false){
            if($del_user=='forum_admin'){
                Gong::delaction('pinglun',$uid,'删除评论');
                ...
                RoutineTemplateService::sendTemplate(WechatUser::getMiniOpenId($uid),RoutineTemplateService::POST_DEL, [
                    'thing1'=>['value'=>$content],
                    'thing2'=>['value'=>'被管理员删除'],
                ],'','/packageA/post-page/post-page?id='.$tid);
            }
...
//        $forum=db('com_post')->where(['id'=>$forum['fid']])->find();
    ...
    public function management(){
        $type=input('type','recommend','text');
        $reward=osx_input('reward');
        $reward_explain=input('reward_explain','');
        $is_on=input('is_on',1);
        $id = input('id',0);
        $end_time=input('end_time',0,'text');
        $custom=input('custom',0);
        if($end_time){
            if($custom==1){
        ...
        switch($type){
            case 'essence':
                ...
                if($is_on==1){
                    $score_log=array();
                    if($reward){
                        $have_score=ComForumAdminScore::getHaveScore($thread['fid'],$uid);
                        foreach($reward as &$value){
                            foreach($have_score as &$val){
                                if($value['flag']==$val['flag']){
                                    if($val['have_num']<$value['num']){
                                        $this->apiError(['info'=>'可奖励积分余额不足！']);
                                    }
                                }
                            }
                            unset($val);
                            $score_res=UserModel::where('uid',$thread['author_uid'])->setInc($value['flag'],$value['num']);
                            if($score_res!==false){
                                $score_log[$value['flag']]=$value['num'];
                            }
                        }
                        unset($value);
                        Support::jiafenlog($thread['author_uid'],'版主加精积分奖励',$score_log,1,'行为') ;

                    }
                    $score_log['uid']=$thread['author_uid'];
                    $score_log['do_uid']=$uid;
                    $score_log['from']='加精';
                    $score_log['fid']=$thread['fid'];
                    $score_log['tid']=$thread['id'];
                    $score_log['explain']=$reward_explain;
                    $score_log['type']=1;
                    $score_log['model']=1;
                    $score_log['create_time']=time();
                    ComForumAdminScoreLog::add($score_log);
            ...
            case 'top':
                //判断是否置顶
                if($end_time&&$thread['is_top']==1&&$thread['top_end_time']>time()){
                    $this->apiError('该帖已经置顶');
                }
                ...
                if($is_on==1){
                    ...
                    if($reward){
                        ...
                        unset($value);
                        Support::jiafenlog($thread['author_uid'],'版主置顶积分奖励',$score_log,1,'行为') ;
                        $score_log['uid']=$thread['author_uid'];
                        $score_log['do_uid']=$uid;
                        $score_log['from']='置顶';
                        $score_log['fid']=$thread['fid'];
            ...
            case 'detail_top':
                //判断是否置顶
                if($end_time&&$thread['detail_top']==1&&$thread['detail_top_end_time']>time()){
                    $this->apiError('该帖已经置顶');
                }
                ...
                if($is_on==1){
                    ...
                    if($reward){
                        ...
                        unset($value);
                        Support::jiafenlog($thread['author_uid'],'版主详情置顶积分奖励',$score_log,1,'行为') ;
                        $score_log['uid']=$thread['author_uid'];
                        $score_log['do_uid']=$uid;
                        $score_log['from']='详情置顶';
                        $score_log['fid']=$thread['fid'];
            ...
            case 'index_top':
                //判断是否置顶
                if($end_time&&$thread['index_top']==1&&$thread['index_top_end_time']>time()){
                    $this->apiError('该帖已经置顶');
                }
                //清除信息流缓存
                Cache::clear('channel_list_change');
                ...
                if($is_on==1){
                    ...
                    if($reward){
                        ...
                        unset($value);
                        Support::jiafenlog($thread['author_uid'],'版主首页置顶积分奖励',$score_log,1,'行为') ;
                        $score_log['uid']=$thread['author_uid'];
                        $score_log['do_uid']=$uid;
                        $score_log['from']='首页置顶';
                        $score_log['fid']=$thread['fid'];
...
//                }
                ...
                if($is_on==1){
                    ...
                    if($reward){
                        ...
                        unset($value);
                        Support::jiafenlog($thread['author_uid'],'版主推荐积分奖励',$score_log,1,'行为') ;
                        $score_log['uid']=$thread['author_uid'];
                        $score_log['do_uid']=$uid;
                        $score_log['from']='推荐';
                        $score_log['fid']=$thread['fid'];
            ...
            case 'reward':
                ...
                if($reward) {
                    ...
                    unset($value);
                    Support::jiafenlog($thread['author_uid'], '版主积分奖励', $score_log, 1, '行为');
                    $score_log['uid'] = $thread['author_uid'];
                    $score_log['do_uid'] = $uid;
                    $score_log['from'] = '积分奖励';
                    $score_log['fid'] = $thread['fid'];
    ...
    public function get_top(){
        $id=osx_input('id',0);
        $top=ComThread::where('id',$id)->field('id,is_top,top_end_time,detail_top,detail_top_end_time,index_top,index_top_end_time')->find();
    ...
    public function is_forum_admin(){
        ...
        $data['channel_admin']=(ChannelAdmin::where('uid',$uid)->where('status',1)->count())>0?1:0;
        $this->apiSuccess($data);
    }
    ...
    public function topic_weibo_list(){
        $id=osx_input('post.id','','intval');
        $type=osx_input('post.order','','text');
        $uid=get_uid();
        $page=osx_input('post.page',1,'intval');
        $row=osx_input('post.row',10,'intval');
        $postListCache=Cache::get('index_topic_weibo_'.$page.'_'.$row.'_'.$id.$type);
        if(!$postListCache || true){  
            $postList = ComThread::getTopicWeibo($page,$row,$id,$type);
            $postListCache = ['list'=>$postList,'recache_time'=>time(),'time_end'=>time()+10*60];
            Cache::tag('index_topic_list')->set('index_topic_weibo_'.$page.'_'.$row.'_'.$id.$type,$postListCache,10*60);
        }
        $postList=$postListCache['list'];
        $weibo_ids=array_column($postList,'id');
        ComThread::where('id','in',$weibo_ids)->setInc('view_count');
            ...
            if($uid){
                ...
                if($selfHasChange>$postListCache['recache_time']){
                    ...
                    if($postListCache['time_end']>time()){//有效期还有一段时间
                        ...
                        $postListCache['list']=$postList;
                        $postListCache['recache_time']=time();
                        Cache::tag('index_topic_list')->set('index_topic_weibo_'.$page.'_'.$row.'_'.$id.$type,$postListCache,$has_time);
                    }
                }
            }
            $postList=ComThread::initListUserRelation($postList);
...
//        action_power('audit_admin',$now_uid);
    ...
    public function prohibit(){
        ...
        if($res===false){
            ...
            Gong::actionAddAll('beijinyan',$prohibit_uid);
     ...
     * @author qhy
     */
    public function get_user_have_score()
    {
        $fid=osx_input('id',0);
        $uid=$this->_needLogin();
        $info=ComForumAdminScore::getHaveScore($fid,$uid);
        if($info===false){
            $this->apiError(['info'=>'没有管理权限']);
        }else{
            $this->apiSuccess($info);
        }
    }
     ...
     * @author qhy
     */
    public function forum_admin_score_log_list()
    {
        $fid=osx_input('post.fid','','intval');
        $type=osx_input('post.type','all','text');
        $page = osx_input('page',1,'intval');
        ...
        switch($type){
            case 'all':
                $fids=self::_ForumAdminAll($uid);
                $list=ComForumAdminScoreLog::LogListAll($fids,$page,$row);
                break;
            default:
                $is_admin=ComForum::_ForumAdmin($fid,$uid);
                if($is_admin['admin_one']==0 && $is_admin['admin_two']==0 && $is_admin['admin_three']==0){
                ...
                $list=ComForumAdminScoreLog::LogList($fid,$page,$row);
                break;
        }
        $this->apiSuccess($list);
    }
     ...
     * @date slf
     */
    public function threadInfoDraft()
    {
        $thread_id = input('post.thread_id/d', 0);
        $data = ComThreadDraft::threadInfo($thread_id); //获取帖子详细信息
        $this->apiSuccess($data);
    }
     ...
     * 检测发送帖子
     */
    public function check_send_thread(){
        $fid=osx_input('fid',0,'intval');
        $uid=get_uid();
        if(!$uid){
            $this->apiError(['status'=>1,'info'=>'有发帖权限']);
        }
        $res=action_power('send_thread',$uid);
        $res2= forum_power('send_thread',$uid,$fid);
        if($res&&$res2){
            $this->apiSuccess(['status'=>1,'info'=>'有发帖权限']);
        }else{
            $this->apiError(['status'=>0,'info'=>'没有发帖权限']);
        }
    }
    /**
     * qxh  2020/12/16
     * 动态、视频、帖子字数上限
    */
    public function thread_word_limit(){
        $data = SystemConfig::getMore('weibo_content_limit,forum_content_limit,video_content');
        $this->apiSuccess($data);
    }
}