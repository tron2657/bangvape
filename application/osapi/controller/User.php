...
class User extends Base
{
    ...
    public function quickVerify()
    {
        if (is_post()) {
            ...
            //获取短信验证码机制修改 zxh
            //用户输入不同号码，每次获取验证，不需要间隔60s，可直接获取，操作三次（≥3）,时间限制120秒,
            //则系统提示：【你操作频率过快，请稍后重试  确定 】，接下来每次不管是否手机号码是否相同，都需要间隔60s获取验证码
            $verfiy_time= Cache::get('verify_time_'.get_client_ip());
            if(is_array($verfiy_time)){
                foreach ($verfiy_time as $key=>$v){
                    if($v<time()-120){
                        unset($verfiy_time[$key]);
                    }
                }
                if(count($verfiy_time)>=3){
                    Cache::set('verify_time_limit_'.get_client_ip(),1,30*60);
                    $this->apiError('你操作频率过快，请稍后重试');
                }
            }else{
                $verfiy_time=[];
            }
            //end
            $verfiy_limit=(time() <= Cache::get('verify_time_phone_ip'.get_client_ip()) + $resend_time)&& Cache::get('verify_time_limit_'.get_client_ip())==1?true:false;
            if ((time() <= Cache::get('verify_time_phone_'.$account) + $resend_time)||$verfiy_limit) {
                $this->apiError('60秒内请勿重复获取验证码');
            }
            Cache::set('verify_time_phone_'.$account, time());
            Cache::set('verify_time_phone_ip'.get_client_ip(), time());
            //调整时间
            $verfiy_time[]=time();
            Cache::set('verify_time_'.get_client_ip(),$verfiy_time,120);
            //end
    ...
    public function quickRegister()
    {
        ...
        if($type=='email'){
            if (empty($aAccount)) {
                $this->apiError('请输入邮箱号');
            }
            if(filter_var($aAccount,FILTER_VALIDATE_EMAIL)===false){
                $this->apiError('请输入正确邮箱格式');
            }
            $is_email=UserModel::where('phone',$aAccount)->find();
            if($is_email){
                $this->apiError('该邮箱已被注册');
            }
        }
        
        if($type=='password'||$type=='email'||$type == 'phone' && $platform == 'PC'){
        ...
        /* 注册用户 */
        $uid = UserModel::register($aAccount, $aNickname, $aPassword, 'osx', $type,$is_password);
        ...
        //注册加分
        $info=Gong::actionadd('zhuce','com_post','uidflag',$uid);
        //注册方式统计
        if (0 < $uid) {
            //邀请记录添加
    ...
    public function quickCheckVerify(){
        ...
        $code=UserVerify::checkVerify($aAccount,$type, $aRegVerify);
        switch($code){
            case 1:
                if($type=='email'){
                    $user=UserModel::where('email',$aAccount)->find();
                }else{
                    $user=UserModel::where('phone',$aAccount)->find();
                }
                if($user){
                    if($type=='email'){
                        $this->apiError(['info'=>'邮箱已注册']);
                    }
                    $data=UserModel::quickLogin($aAccount);
                    if($data===false){
                        $this->apiError(['info'=>'账号存在异常']);
                    }
                }else{
                    $data['is_exit']=0;
                    $data['info']='不存在用户';
    ...
    public function passwordLogin()
    {
        ...
        $aPassword=osx_input('post.password', '');
        $type=osx_input('post.type', 'mobile');
        $platform = osx_input('post.platform');
        /**解密 start**/
        $iv = "1234567890123412";//16位 向量
        $key= '201707eggplant99';//16位 默认秘钥
        $aAccount=trim(openssl_decrypt(base64_decode($aAccount),"AES-128-CBC",$key,OPENSSL_RAW_DATA,$iv));
        ...
        /**解密 end**/
        if($type=='email'){
            $uid=db('user')->where('email',$aAccount)->where('status',1)->find();
        }else{
    ...
    public function passwordEdit()
    {
        ...
        if($aPassword==''){
            $this->apiError(['info'=>'请输入密码']);
        }
        if(preg_match('/^[0-9a-z_$]{6,14}$/i',$aPassword)){
            $data['pwd']=md5($aPassword);
        }else{
            $this->apiError(['info'=>'密码不符合要求，请重新设置']);
        }
        $data['is_password']=1;
        $result = UserModel::where('uid',$uid['uid'])->update($data); //状态码是1时候登录成功
        if ($result !== false) {
            $this->apiSuccess(['info'=>'密码修改成功']);
        } else {
            $this->apiError(['info'=>'密码修改失败']);
        }
    }
    ...
    public function forgetPassword()
    {
        $aAccount = osx_input('post.phone', '');
        $aPassword=osx_input('post.password', '');
        $aRegVerify = osx_input('post.quick_verify', '');
        $type=osx_input('post.type', 'mobile');
        /**解密 start**/
        $iv = "1234567890123412";//16位 向量
        $key= '201707eggplant99';//16位 默认秘钥
    ...
    public function qiandao()
    {
        ...
        if(empty($qiandao)){
            ...
            $config=SystemConfig::getMore('qiandao_cishu_lxcyle',[
                'qiandao_cishu_lxcyle'=>'7'
            ]);//签到连续周期默认为7天

            if($qiandao['cishu']==intval($config['qiandao_cishu_lxcyle']) ){
                $data = [
                    'time' => $date,
                    'user_id' => $user_id,
                    'cishu' => 1
        ...
        ],$h5_url.'packageA/sign-in/sign-in');
        $num=intval($qiandao['cishu']) +1;
        RoutineTemplateService::sendTemplate(WechatUser::getMiniOpenId($user_id),RoutineTemplateService::CHECK_IN, [
            'thing3'=>['value'=>'每日签到'],
            'time2'=>['value'=>date("Y-m-d H:i:s",time())],
            'thing5'=>['value'=>$num.'天'],
        ],'','/packageA/sign-in/sign-in');
    ...
    public function updateUserInfo()
    {
        ...
        if (is_post()) {
            ...
            $birthday_input=osx_input('birthday','','text');
            $birthday=0;//生日
            if($birthday_input!=""){
                $birthday=strtotime($birthday_input);
            }
    ...
    public function updateAvatar()
    {
        ...
        if($file==false){
            ...
            if($res==false){
                ...
                Cache::rm('user_info_'.$uid);
                if(file_exists('public/uploads/common/user/'.'avatar_'.$uid.'.png')){
                    unlink('public/uploads/common/user/'.'avatar_'.$uid.'.png');
                }
                $this->apiSuccess($res);
            }
        }
     ...
     * pc端我的关注页面
     */
    public function pc_myFollow()
    {
        $uid = input('post.uid/d', 0);
        !$uid && $uid = $this->_needLogin(); 
        $page = input('post.page', 1);
        $row = input('post.row', 10);
        $follow['user'] =UserFollow::getFollowListPc($uid,$page,$row);
        $follow['forum'] =ComForumMember::getUserFollow($uid,$page,$row);
        // var_dump($follow);die;
        $this->apiSuccess($follow);
    }
...
//        }
     ...
     * pc端收藏页面
     */
    public function pc_myCollect()
    {
        $uid = osx_input('uid');
        if(!isset($uid)){
            $this->apiError('缺少用户信息');
        }
        $page = input('post.page', 1);
        $row = input('post.row', 10);
        $type = input('post.type', 'post');
        $postList = ComThread::getPostCollectPc($uid,$page,$row,'create_time desc',$type);
        $this->apiSuccess($postList);
    }
    /**
     * 个人中心头部用户信息
    ...
    // public  function grant_vip_user()
    // {

    //     $res= \service\MemberShipService::grant_vip_user(2);        
    //     if($res['success'])
    //     {
    //         // return Json::successful('发放成功!',$res['data']);
    //     }
    //     else{          
    ...
    public function is_vip(){
        ...
        $uid=$this->_needLogin();
        return MemberShip::memberExpiration($uid)==false;
    ...
    private function _checkNickname($nickname)
    {
        ...
        $length = mb_strlen($nickname, 'gbk'); // 当前数据长度
        if ($length < 2||$length >24) {
            $this->apiError('用户昵称字符长度在2-24个字符');
        }
        if(is_numeric($nickname)){
            $this->apiError('昵称不能用纯数字组成');
        }
    ...
    public function follow()
    {
        ...
        if($res!==false){
            ...
            Cache::rm('user_info_'.$uid);
            Cache::rm('add_follow_uid_'.$self_uid);
    ...
    public function copyRight()
    {
        ...
        if(!in_array('right_copy',$open_list)){
            ...
            $website_logo='/public/uploads/config/image/5e23cad0947a4.png';
            $website_url ='http://osx.opensns.cn/h-col-139.html?mf08';
        }
        $url='http://'.$_SERVER['SERVER_NAME'];
        $is_logo=$website_logo? true:false;
        $website_logo = get_root_path($website_logo);
        $this->apiSuccess(['logo' => $website_logo, 'website_url' =>$website_url,'is_logo'=>$is_logo]);
    }
    ...
    public function userCard()
    {
        ...
        $certification=db('certification_entity')->where('uid',$uid)->where('status',1)->field('id,cate_id,datum_data')->select();
        foreach($certification as &$val){
            $val['name']=db('certification_cate')->where('id',$val['cate_id'])->value('name');
            $datum_data=unserialize($val['datum_data']);
            unset($val['datum_data']);
    ...
    public function UserBindInfo(){
        ...
        $data['email']=UserModel::where('uid',$uid)->value('email');
     ...
     * 微信登录绑定邀请码
     */
    public function WeixinInvite(){
        $type = input('post.type/d',0);
        $invite_code = input('post.invite_code','','text');
        $uid=$this->_needLogin();
        if($type==1){
            $res=InviteCode::addInviteLog($invite_code,$uid);
            if($res===false){
                $this->apiError('绑定失败');
            }
        }
        $data['is_invite'] = 1;
        UserModel::where('uid',$uid)->update($data);
    ...
    public function following_users(){
        ...
        $follow =UserFollow::getFollowList($uid,$page,$row,'',$nickname);
        $this->apiSuccess($follow);
    }
    /**
     * qxh
    ...
    */
    public function be_recommend(){
        ...
        $row = input('post.row', 10);
        //查询推荐用户的uids
        $rec_uids = RecommendModel::where(['status'=>'1','attention' => 0])->field('uid')->order("sort desc,create_time desc")->select()->toArray();
        //查询该用户已关注的uids
        $map=[
            'uid'=>$uid,
            'status'=>1
        ];
        $bei_uids = UserFollow::where($map)->field('follow_uid as uid')->select()->toArray();
        //从推荐用户中删除该用户已经关注的用户
        foreach ($rec_uids as $k => $v){
            foreach($bei_uids as $k1 => $v1){
                if($v['uid'] == $v1['uid']){
                    unset($rec_uids[$k]);
                }
            }
        ...
        $rec_uids=array_column($rec_uids, 'uid');
        $uids_info = UserModel::getUsersInfo($rec_uids,$page,$row);
        $this->apiSuccess($uids_info);
    }
    ...
    public function emailSend()
    {
        ...
        $iv = "1234567890123412";//16位 向量
        $key= '201707eggplant99';//16位 默认秘钥
        $email=trim(openssl_decrypt(base64_decode($email),"AES-128-CBC",$key,OPENSSL_RAW_DATA,$iv));
        /**解密 end**/
        if(filter_var($email,FILTER_VALIDATE_EMAIL)===false){
            $this->apiError('请输入正确邮箱格式');
        }
        $aVerify = UserVerify::addData($email,'email'); //生成验证码
        if($aVerify){
            $config=SystemConfig::getMore('email_title,email_content');
            $content = str_replace('{验证码}', $aVerify, $config['email_content']); //根据模板添加验证码
            $result = SendEmail::SendEmail($config['email_title'],$content,$email);
            if($result){
                $this->apiSuccess('发送验证码成功');
            }else{
                $this->apiError('发送验证码失败');
            }
        }else{
            $this->apiError('生成验证码失败');
        }
 ...
 /**
     * qxh
     * pc端新增单个用户所有的发布内容的浏览量、获赞数、评论数、收藏量的统计
    */
    public function post_data(){
        $uid=$this->_needLogin();
        $post_datas = ComThread::postDataAll($uid);
        $this->apiSuccess($post_datas);
    }
    /**
     * qxh
     * pc端用户个人中心内容管理
    */
    public function content_management(){
        ...
        Cache::clear(); 
        $type = osx_input('type');
        $page = osx_input('page',1);
        $row = osx_input('row',10);
        if(!isset($type)){
            $this->apiError('传入参数无效');
        }
        if($type == 1 || $type == 6){
           $weibo = 0;
        }else{
        ...
        ,'type'=>$type,'is_weibo'=>$weibo,'status' => 1];
            $data = array();
            //获取我的热度的相关数据
            $data['all_view_count'] = ComThread::where($where)->sum('view_count'); 
            $data['all_reply_count'] = ComThread::where($where)->sum('reply_count'); 
            $data['all_support_count'] = ComThread::where($where)->sum('support_count'); 
            $data['all_collect_count'] = ComThread::where($where)->sum('collect_count'); 
            //帖子的总数据
            $data['all_thread'] = ComThread::where($where)->page($page, $row)->field('id,author_uid,title,create_time,view_count,reply_count,support_count,collect_count,content')->select()->toArray(); 
            $data['all_count_thread'] = ComThread::where($where)->field('id,author_uid,title,create_time,view_count,reply_count,support_count,collect_count')->count(); 
            foreach($data['all_thread'] as $k => $v){
                $data['all_thread'][$k]['create_time'] = date('Y-m-d H:i',$v['create_time']);
                $data['all_thread'][$k]['operation'] = '删除';
                $html_string = htmlspecialchars_decode($v['content']);
                
                //函数剥去字符串中的HTML、XML以及PHP的标签,获取纯文本内容
                $contents = strip_tags($html_string);
                //返回字符串中的前60字符串长度的字符
                $contents = json_decode($contents);
                $text = mb_substr($contents,0,11,"utf-8");   
                if($weibo == 1){
                    $data['all_thread'][$k]['title'] = $text;
                }
     ...
     * qxh
    */
    public function user_header_attestation(){
        $uid = osx_input('uid');
        if(!isset($uid)){
            $this->apiError('缺少用户信息');
        }
        $map['uid'] = $uid;
        $map['status'] = 1;
        $data = db('certification_entity')->where($map)->select();
        $i = 0;
        $list = [];
        foreach($data as $k=>$v){
            $list[$i]['cate'] = db('certification_cate')->where('id',$v['cate_id'])->find();
            $datum_data=unserialize($v['datum_data']);
                if(!empty($datum_data['rztx'])){
                    $list[$i]['datum_data'] = $datum_data['rztx'];
                }
            $i = $i+1;
        }
        $this->apiSuccess($list); 
     ...
     * qxh
    */
    public function update_user_background(){
        $user_background = osx_input('image');
        if(!isset($user_background)){
            $this->apiError('缺少关键参数');
        }
        $file = File::uploadAvatar($user_background);

        if($file==false){
            $this->apiError(File::getError());
        }else{
            $res = UserModel::user_background_update($file['path']);
            if($res){
                $this->apiSuccess($res); 
            }else{
                $this->apiError('修改失败');
