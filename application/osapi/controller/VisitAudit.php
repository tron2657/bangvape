...
class VisitAudit extends Base
{
    ...
    public function add_audit(){
        ...
        if(ForumAudit::where(['uid'=>$uid,'fid'=>$fid,'status'=>1])->count()){
            $this->apiError('审核已经通过,请勿重复申请');
        }
        $data['fid']=$fid;
        $data['reason']=$reason;
        $data['status']=2;
    ...
    public function get_fid_power(){
        ...
        $message=db('bind_forum_group')->where(['bind_forum'=>$fid,'type'=>'audit','status'=>1])->value('group');
        $message=$message?$message:'填写原因，更容易通过审核哦！';
        $fid=ComForum::where('id',$fid)->field('logo,name')->find();
        $this->apiSuccess([$action=>$power,'fid'=>$fid,'audit_message'=>$message],'获取权限成功');
    }
