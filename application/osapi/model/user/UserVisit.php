...
class UserVisit extends ModelBasic
{
    ...
    public static  function getUserVisitList($map,$page,$limit){
        $user=self::where($map)->group('visit_uid')->page($page,$limit)->select();
        $data=[];
        foreach ($user as $key=>$vo){
            $userinfo=UserModel::getUserInfo($vo['visit_uid'],'uid,nickname,avatar');
            if($userinfo){
                $data[$key]=$userinfo;
            }
        }
        unset($vo,$key);
        $count=self::where($map)->group('visit_uid')->count();
    ...
    public static  function getPicture($uid){
        ...
        foreach ($pic as $key=>$vo){
            ...
            if($path){
                $path['path']=get_root_path($path['path']);
                $data[$key]=$path;
            }
        }
        unset($key,$vo);
