...
class UserModel extends BaseModel
{
    ...
    public static function register($account, $nickname, $password, $from='osx', $type='phone',$is_password=0)
    {
        ...
        if($type=='email'){
            self::update(['account' => $account.'_del','email'=>$account.'_del'],['email'=>$account,'status'=>array('neq',1)]);//重置其他账号已删除的同手机号账户
        }else{
        ...
        $grade=db('system_user_grade')->where('is_del',0)->where('type',1)->order('experience asc')->value('id');
        $gid=db('group')->where('type',3)->where('status',1)->where('bind_condition',$grade)->value('id');
        $group['uid']=$uid;
        $group['g_id']=$gid;
    ...
    public static function addSyncData($info)
    {
        ...
        // 截取字数
        $data['nickname'] = mb_substr($data['nickname'], 0, 12, 'utf-8');
        // 为空则随机生成
    ...
    public static function login($uid, $remember = false)
    {
        ...
        if($res){
            ...
            $userInfo['is_bind_phone']=$user['phone']!=null;
            $userInfo['can_check']=self::hasCheckPrivate($uid);
            if($res_login){
                $result['token'] = $token;
                ...
                $result['success']=1;
                $result['uid']=$uid;
                $result['info']='登录成功!'.self::actionsystem($uid);
    ...
    public static function actionsystem($uid)
    {
        ...
        $info=Gong::actionadd('meiridenglu','user_login_log','uid',$uid);
        return $info;
    }
     ...
     * @date slf
     */
    public static function user_background_update($user_background)
    {
        $uid=get_uid();
        $res=self::update(['user_background'=>$user_background],['uid'=>$uid]);
        if($res){
    ...
    public static function hasCheckPrivate($uid)
    {
        $can_check=0;
        $event_check_count=db('event_check')->where(['uid'=>$uid,'status'=>1])->count();
        $active_check_count=db('active_check')->where(['uid'=>$uid,'status'=>1])->count();
        if($event_check_count>0||$active_check_count>0){
            $can_check=1;
        }
        return $can_check;
    }
    ...
    public static function getUserInfo($uid,$fid='')
    {
        ...
        if(!$user){
            $field='uid,nickname,avatar,sign_num,status,level,user_type,pay_count,fans,follow,sex,birthday,signature,total_sign,support_count,introduction,forum_count,post_count,exp,fly,gong,buy,one,two,three,four,five,is_collect,mark,icon,is_red,collect,cate_id,user_background,overdue_time,is_permanent,phone';
            ...
            if(!isset($user['user_background']))
            {
                $user['user_background']=null;
            }
            if($user['user_background'] == null){
                $user['user_background']=SystemConfig::getValue('pc_user_background');
                $user['user_background'] = get_root_path($user['user_background']);
            }
            $user['rztx']='';
            if(isset($user['cate_id']) &&  $user['cate_id']){
            ...
            //用户是否有核销权限
            $user['can_check']=self::hasCheckPrivate($uid);
     ...
     * 
    */
    public static function getUsersInfo($uids,$page,$row,$fid=''){
        //获取多条id的数据
        ...
        $users=self::where('uid','in',$uids)->where('status','>',0)->page($page,$row)->field($field)->select()->toArray();
        $users_info = [];
        foreach($users as $k=>$v){
            if($v['birthday']!=0){
                $users[$k]['birthday']=date('Y-m-d',$users[$k]['birthday']);//生日
            }
            if($v['avatar']){
                $users[$k]['avatar']=get_root_path($v['avatar']);
                $users[$k]['avatar_64']=thumb_path($v['avatar'],64,64);
                $users[$k]['avatar_128']=thumb_path($v['avatar'],128,128);
                $users[$k]['avatar_256']=thumb_path($v['avatar'],256,256);
            }
            if($fid){
                $pid=db('com_forum')->where('id',$fid)->value('pid');
                $users[$k]['forum_admin_one']=db('com_forum_admin')->where('uid',$v['uid'])->where('status',1)->where('fid',$fid)->count();
                $users[$k]['forum_admin_two']=db('com_forum_admin')->where('uid',$v['uid'])->where('status',1)->where('fid',$pid)->count();
            }
            //认证图标
            $icon_field=is_icon('');
            if ($icon_field=='') {
            ...
            $v['rztx']='';
            if($v['cate_id']){
                $datum_data=db('certification_entity')->where('uid',$v['uid'])->where('status',1)->where('cate_id',$v['cate_id'])->value('datum_data');
                $datum_data=unserialize($datum_data);
                if(!empty($datum_data['rztx'])){
                    $users[$k]['rztx']=$datum_data['rztx'];
                }
            }
            $users[$k]['grade'] = self::cacugrade($v['exp']) ;
            $tids=db('collect')->where(['uid'=>$v['uid'],'status'=>1])->column('tid');
            $users[$k]['collect']=db('com_thread')->where(['id'=>['in',$tids],'status'=>1])->count();
        }
        return $users;
    }
    ...
    public static function searchUser($keyword,$page=1,$row=10){
        $user=self::where('nickname','like','%'.$keyword.'%')->page($page, $row)->limit(50)->order('add_time desc')->column('uid');
//        $user['allCount']=self::where('nickname','like','%'.$keyword.'%')->count();
        foreach($user as &$value){
            $value=self::getUserInfo($value);
        }
        unset($value);
        return $user;
    }
