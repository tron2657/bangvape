...
class UserFollow extends BaseModel
{
     ...
     * @date slf
     */
    public static function getFollowList($uid,$page=1,$row=10,$order='create_time desc',$nickname='')
    {
        $map=[
            'uid'=>$uid,
            'status'=>1
        ...
        $list=self::_toUserList($uids,1,$is_self,$page,$row,$nickname);
        return $list;
    }
    /**
     * qxh
     * pc端获取关注列表
    */
    public static function getFollowListPc($uid,$page=1,$row=10,$order='create_time desc',$nickname='')
    {
        $map=[
            'uid'=>$uid,
            'status'=>1
        ...
        $count = UserModel::where($maps)->count();
        $some = array('user_list'=>$list,'user_count'=>$count);
        return $some;
    }
     ...
     * @date slf
     */
    private static function _toUserList($uids,$follow=0,$is_self=0,$page=1,$row=10,$nickname='')
    {
        $uid=get_uid();
        if(count($uids)){
            //认证图标
            $icon_field=is_icon('');
            $fields = 'uid,nickname,sex,fans,signature,exp,avatar,cate_id,'.$icon_field.'is_red';
            $uid_list=array_column($uids,'uid');
            ...
            $follow_me_ids=self::where('follow_uid',$uid)->where('status',1)->column('uid');
            foreach ($list as &$val){
                ...
                $val['follow_me'] = in_array($val['uid'],$follow_me_ids)?1:0;
                ...
                $val['rztx']='';
                if($val['cate_id']){
                    $datum_data=db('certification_entity')->where('uid',$val['uid'])->where('status',1)->where('cate_id',$val['cate_id'])->value('datum_data');
                    $datum_data=unserialize($datum_data);
                    if(!empty($datum_data['rztx'])){
                        $val['rztx']=$datum_data['rztx'];
                    }
                }
                if($follow&&$is_self){
    ...
    public static function doFollow($follow_uid)
    {
        ...
        if($data){
            if($data['status']==1){//已关注改成取消关注
                ...
                self::canceltask();
                //减行为积分
                Gong::delaction('guanzhu',$self_uid,'取消关注用户');
                $info_jf='';
                if($res1){
                ...
                //发送关注动态
                $message_id=Message::sendMessage($follow_uid,$self_uid,'关注了你',4,'关注了你',4);
                MessageRead::createMessageRead($follow_uid,$message_id,0,4);

                //加任务积分
                self::finishtask();
                //加行为积分
                $info_jf=Gong::actionadd('guanzhu','user_follow','uid');
                if($res1){
