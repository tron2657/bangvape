...
class InviteCode extends ModelBasic
{
    ...
    public static function addInviteLog($inviteCode,$uid){
        if($inviteCode&&$uid){
            ...
            $father_name=db('user')->where('uid',$invite['uid'])->value('nickname');
            RoutineTemplateService::sendTemplate(WechatUser::getMiniOpenId($invite['uid']),RoutineTemplateService::INVITE_REGISTER, [
                'thing3'=>['value'=>$father_name],
                'thing4'=>['value'=>$name],
                'time5'=>['value'=>date('Y/m/d H:i',time())],
            ],'','/pages/user/index');
