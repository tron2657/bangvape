...
class Support extends BaseModel
{
                ...
                case 'reply':
                    ...
                    if($author_uid!=$uid){
                        ...
                        if($set['status']==1){
                            $nickname=UserModel::where('uid',$uid)->value('nickname');
                            $template=str_replace('{用户昵称}', $nickname, $set['template']);
                            $message_id=Message::sendMessage($post['author_uid'],$uid,$template,3,$set['title'],3,'',$model,$tid,$row);
        ...
        if(!empty($jifenzhonglei)){
            ...
            foreach ($jifenzhonglei as $item) {
                ...
                if($count <= $guanzhu['num']){
                    ...
                    UserModel::where('uid',$user_id)->setInc($item['flag'],intval($guanzhu[$tag[0]])) ;
                    $res = 1;
    ...
    // 增加行为积分
    public static function addjifenAll($guanzhu,$user_id)
    {
    ...
    //减去行为积分
    public static function delsubjifen($guanzhu,$user_id,$name)
    {
        ...
        if(!empty($jifenzhonglei)){
            ...
            foreach ($jifenzhonglei as $item) {
                ...
                //减分
                //$res = $res && UserModel::where('uid',$user_id)->where($item['flag'],'>=',intval($guanzhu[$tag[0]]))->setDec($item['flag'],intval($guanzhu[$tag[0]])) ;
                $temp = UserModel::where('uid',$user_id)->setDec($item['flag'],intval($guanzhu[$tag[0]])) ;
                $temp = $temp>=0 ? true : false ;
                $res = $res && $temp;
                if(intval($guanzhu[$tag[0]])!=0){
                    $log[$item['flag']] = -intval($guanzhu[$tag[0]]) ;
            ...
            //记录日志
            if(!empty($log)){
                self::jiafenlog($user_id,$name,$log,-1,'行为') ;
            }
    ...
    public static function jiafenlog($uid,$explain,$log,$type=1,$model)
    {
        ...
        $log['type'] = $type;
        $log['model'] = $model;
        if(isset($log['exp']) && $log['exp']!=0){
            $user_exp=UserModel::where('uid',$uid)->value('exp');
            $gid=db('bind_group_uid')->where('status',1)->where('type',3)->where('uid',$uid)->find();
            $bind_condition=db('group')->where('id',$gid['g_id'])->value('bind_condition');
            $grade=UserModel::cacugrade($user_exp);
            if($grade['true_id']!=$bind_condition){
                $new_gid=db('group')->where('bind_condition',$grade['true_id'])->where('type',3)->value('id');
                db('bind_group_uid')->where('id',$gid['id'])->update(['g_id'=>$new_gid]);
            }
        }
