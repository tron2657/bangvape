...
class ComForum extends BaseModel
{
    ...
    public static function _ForumAdmin($fid,$uid,$comment_id=''){
        ...
        $data['admin_three']=$is_admin&&$uid>0?1:0;
    ...
    /**
     * -判断是否是版主，版主、超级版主、前台管理员均返回true，否则返回false
     * @param $fid
     * @param $uid
     * @return bool
     ...
     * @date 2020-7
     */
    public static function isForumAdmin($fid,$uid)
    {
        $checkData=self::_ForumAdmin($fid,$uid);
        if($checkData['admin_one']>0||$checkData['admin_two']>0||$checkData['admin_three']>0){
            return true;
        }else{
            return false;
        }
