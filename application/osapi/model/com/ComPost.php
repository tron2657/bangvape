...
//        }
        ...
        foreach ($postList as &$post) {
            ...
            $post['user']=UserModel::getUserInfo($post['author_uid']);
        }
        unset($post);
    ...
    public static function getPostReplyList($to_reply_id, $post=null, $page = 1, $row = 10, $sort = 'create_time asc')
    {
        ...
        if ($post != null) { //若是数组则获取帖子详情中的5条楼中楼评论
            ...
            if(count($postReplyList)){
                ...
                $author=UserModel::where('uid','in',$uids)->where('status',1)->field('uid,nickname,avatar,icon,exp,is_red,cate_id')->select()->toArray();
                ...
                foreach ($postReplyList as $key=>&$value) {
                    ...
                    $value['reply_user_name'] = UserModel::where('uid',$value['to_reply_uid'])->value('nickname');
                    if(isset($author[$value['author_uid']])){
                        ...
                        $value['user']['rztx']='';
                        if($value['user']['cate_id']){
                            $datum_data=db('certification_entity')->where('uid',$value['user']['uid'])->where('status',1)->where('cate_id',$value['user']['cate_id'])->value('datum_data');
            ...
            if(count($postReplyList)){
                ...
                $uid_now=get_uid();
                $post_ids=array_column($postReplyList,'id');
                $support_list=Support::where('row','in',$post_ids)->where('uid',$uid_now)->where('model','reply|event')->where('status',1)->column('row');
...
//                    $thread['audio_url']=TencentFile::yunKeyMediaUrl($val['audio_url'],$getYunConfig['pkey']);
//                }
            }
     ...
     * @date slf
     */
    public static function getEventReplyList($tid, $page=1, $row=10, $sort='create_time asc')
    {
        $field = 'id,tid,author_uid,create_time,to_reply_id,content,image,position,comment_count,support_count,event_id';
        $lordInfo=null;
        $map=[
            'event_id'=>$tid,
            'level'=>1,
