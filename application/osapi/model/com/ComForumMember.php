...
class ComForumMember extends BaseModel
{
    ...
    public static function addToForum($forum_id,$uid)
    {
        ...
        try{
            if ($res) {
                if ($res['status'] == 1) {  //若有记录并且已加入，则改为退出状态，并将该版块用户数减一
                    ...
                    if($is_admin['admin_one']==0 && $is_admin['admin_two']==0){
                        ...
                        //行为扣分
                        Gong::delaction('jiarubankuai',$uid,'退出版块') ;
                    ...
                    //行为加积分
                    $info=Gong::actionadd('jiarubankuai','com_forum_member','uid') ;
            ...
            //清除权限缓存 zxh
            $tag='power_forum_value_fid_'.$forum_id.'_uid_'.$uid;
            Cache::rm($tag);
            self::commitTrans();
            return $info;
        }catch (\Exception $e){
    ...
    {
        $mav=['status'=>1,'display'=>1];
        if($access[1] == '微信小程序' && $video_is_on==0){
            $mav['type']=['neq',6];
        }
        $com_forum=db('com_forum')->where($mav)->column('id');
        ...
        if(count($forum_list)){
            ...
            foreach ($forums as &$val){
                ...
                $val['post_count']=ComThread::where('fid',$val['id'])->where('status',1)->count();
    ...
    {
        $com_forum=db('com_forum')->where(['status'=>1,'display'=>1])->column('id');
        $map=[
            'uid'=>$uid,
            'status'=>1,
            'fid'=>['in',$com_forum]
