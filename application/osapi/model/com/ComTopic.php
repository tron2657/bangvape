...
class ComTopic extends BaseModel
{
    ...
    public static function hotTopicList($class_id)
    {
        ...
        if($list){
            ...
            foreach($list as &$value){
                ...
                $value['view_count']=ComThread::where($mvp)->where('fid','in',$forum_ids)->where('find_in_set(:id,oid)',['id'=>$value['id']])->sum('view_count');
            }
            unset($value);
        }
        return $list;
     ...
     * 话题列表
     */
    public static function TopicList($page,$row,$class_id,$fid=0)
    {
        $field='id,title,summary,image,view_count,follow_count,post_count,type';
    ...
    public static function UserSendTopicList($page,$row,$uid)
    {
        ...
        if($list){
            ...
            foreach ($list as &$value){
                ...
                $value['view_count']=ComThread::where($map)->where('fid','in',$forum_ids)->where('find_in_set(:id,oid)',['id'=>$value['id']])->sum('view_count');
            }
        }
        return $list;
    }
    ...
    public static function detailTopic($id){
        ...
        $forum_ids=ComForum::where('status',1)->column('id');
        $topic['view_count']=ComThread::where($map)->where('fid','in',$forum_ids)->where('find_in_set(:id,oid)',['id'=>$id])->sum('view_count');
        $topic['type']=$id==5?'video':'';//视频告示牌
        return $topic;
    }
