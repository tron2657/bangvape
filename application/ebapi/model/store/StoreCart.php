...
class StoreCart extends ModelBasic
{
    ...
    public static function setCart($uid,$product_id,$cart_num = 1,$product_attr_unique = '',$type='product',$is_new = 0,$combination_id=0,$seckill_id = 0,$bargain_id = 0,$buy_type='add_to_cart',$trial_id=0)
    {
        // self::clearCartCache($uid,null,$type);
        ...
        {//普通立即购买、加入购物车
            ...
            $where = ['type'=>$type,'uid'=>$uid,'product_id'=>$product_id,'product_attr_unique'=>$product_attr_unique,'is_new'=>$is_new,'is_pay'=>0,'is_del'=>0,'combination_id'=>$combination_id,'trial_id'=>$trial_id];
            if($cart = self::where($where)->find()){
                ...
                return self::set(compact('uid','product_id','cart_num','product_attr_unique','is_new','type','combination_id','trial_id'),true);
            }
        }
    }

    public static function setCartExchange($uid,$product_id,$cart_num = 1,$product_attr_unique = '',$type='product',$is_new = 0,$combination_id=0,$seckill_id = 0,$bargain_id = 0,$buy_type='add_to_cart',$trial_id=0,$coupon_id=0)
    {
      //普通立即购买、加入购物车
        if(!StoreProduct::isValidProduct($product_id))
            return self::setErrorInfo('该产品已下架或删除',false,true);
        ...
        if($cart = self::where($where)->find()){
            ...
            return self::set(compact('uid','product_id','cart_num','product_attr_unique','is_new','type','combination_id','trial_id','coupon_id'),true);
        }
    ...
    {
        $result=self::where('uid',$uid)->where('id','IN',$ids)->update(['is_del'=>1]);
        self::clearCartCache($uid,$ids);
        return $result;
    }
    ...
    {
        $cart_list=StoreCart::getUserProductCartList($uid);
        $cart_list=$cart_list['valid'];
    ...
    public static function clearCartCache($uid,$cartIds='',$type='product'){
        $cache_key=self::getCartCacheKey($uid,$cartIds,$type);
        Cache::rm($cache_key);
    }

    public static function setCartCache($uid,$data,$cartIds='',$type='product')
    {
        $cache_key=self::getCartCacheKey($uid,$cartIds,$type);
        \think\Cache::set($cache_key,$data,20);
    }

    private static function getCartCacheKey($uid,$cartIds='',$type='product')
    {
        $cache_key='cart_list_'.$uid.'_'.$type.'_'.$cartIds;
        return $cache_key;
    }

    public static function getCartCache($uid,$cartIds,$type='product')
    {
        $cache_key=self::getCartCacheKey($uid,$cartIds,$type);
    ...
    public static function changeUserCartNum($cartId,$cartNum,$uid)
    {
        ...
        self::clearCartCache($uid,$cartId);
    ...
    public static function getUserProductCartList($uid,$cartIds='',$status=0,$type='product',$isVip=false)
    {
 
        // $cart=self::getCartCache($uid,$cartIds);
        // if($cart!=null)
        // {           
        //     return $cart;
        // }
        ...
        $model = new self();
        $valid = $invalid = [];
        $model = $model->where('uid',$uid)->where('type',$type)->where('is_pay',0)
            ->where('is_del',0);
        ...
        foreach ($list as $k=>$cart){
            if($cart['seckill_id']){
                $product = StoreSeckill::where(['start_time'=>['lt',$time],'id'=>$cart['seckill_id'],'stop_time'=>['gt',$time],'status'=>1,'is_del'=>0])->field($seckillInfoField)
                    ->find();
            ...
            if(!$product){
                ...
                if($cart['product_attr_unique']){
                    ...
                    if(!$attrInfo || !$attrInfo['stock'])
                        ...
                        $cart['price']=(float)$attrInfo['price'];
                        
                        if($isVip)
                        {
                            $cart['truePrice'] = (float)StoreProduct::setLevelPrice($attrInfo['price'],$uid,true);
                        }else{
                            $cart['truePrice'] = $cart['price'];
                        }                         
  
                        $cart['vip_truePrice'] = (float)StoreProduct::setLevelPrice($attrInfo['price'],$uid,true,true);
                    ...
                    $cart['price']=(float)$cart['productInfo']['price'];
                    if($isVip)
                    {
                        $cart['truePrice'] = (float)StoreProduct::setLevelPrice($cart['price'],$uid,true);
                    }else{
                        $cart['truePrice'] = $cart['price'];
                    }
                    // $cart['truePrice'] = (float)StoreProduct::setLevelPrice($cart['productInfo']['price'],$uid,true);
                    $cart['vip_truePrice'] = (float)StoreProduct::setLevelPrice($cart['productInfo']['price'],$uid,true,true);
        ...
        self::setCartCache($uid,$result,$cartIds);
        return $result;
    }
...
//                $invalid[] = $cart;
    ...
    */
    ...
    {
        /*$field = 'b.mer_id,cart.id as cart_id,b.id,b.image,b.store_name,b.price,b.sales+b.ficti sales,b.store_info,a.name,a.type,b.add_time';
        ...
        $lists = db('column_text')->alias('a')->join('store_cart b','b.product_id=a.id')->where('a.status',1)->where('a.is_show',1)
            ->where('b.is_del',0)->where('b.is_pay',$is_pay)->where('b.uid',$uid)->where('b.type','is_zg')->order('b.id desc')
            ->field('a.id,a.image,a.name as store_name,a.price,a.sales+a.ficti_sales sales,a.name,a.type,a.create_time as add_time,b.id as cart_id')
...
//        $model = $model->where('SP.is_del',0)->where('SP.is_show',1);
        $model = $model->join('ColumnText b','b.id=cart.product_id','left');
        $model = $model->where('b.status',1)->where('b.is_show',1);
        $model = $model->count();
        return $model;
    }
