...
class StoreBargainUserHelp extends ModelBasic
{
    ...
    /**
     * TODO 获取砍价帮
     * @param int $bargainUserTableId 用户参与砍价表编号
     * @param int $limit 查询的条数
     * @return array
     */
    public static function getList($bargainUserTableId,$offset = 0,$limit = 15){
         if(!$bargainUserTableId) return [];
         $list = self::where('bargain_user_id',$bargainUserTableId)->order('add_time desc')->limit($offset,$limit)->column("uid,price,from_unixtime(add_time,'%Y-%m-%d %H:%i:%s') as add_time",'id');
         if($list){
    ...
    public static function setBargainUserHelp($bargainId = 0,$bargainUserUid = 0,$uid = 0){
        if(!self::isBargainUserHelpCount($bargainId,$bargainUserUid,$uid) || !$bargainId || !$bargainUserUid || !$uid || !StoreBargain::validBargain($bargainId) || !StoreBargainUser::be(['bargain_id'=>$bargainId,'uid'=>$bargainUserUid,'status'=>1,'is_del'=>0])) return false;
