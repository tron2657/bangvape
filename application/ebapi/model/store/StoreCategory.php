...
class StoreCategory extends ModelBasic
{
    ...
    {
        $model = self::where('pid',$pid)->where('is_show',1)->field($field)->order('sort desc');
    ...
    public static function byIndexList($limit = 4,$field = 'id,cate_name,pid,pic'){
        $data=self::where('pid','>',0)->where('is_show',1)->field($field)->order('sort DESC')->limit($limit)->select()->toArray();
        array_unshift($data,['id'=>'-1','cate_name'=>'全部','pid'=>'-1','pic'=>'']) ;
