...
class StoreSeckill extends ModelBasic
{
     ...
     *
     * */
    public static function seckillList($dataTime,$offset = 0,$limit = 20)
    {
        $startTime=strtotime(date('Y-m-d',time()).'00:00:00');
        $stopTime=$startTime+24*3600;
        $list = StoreSeckill::where('is_del',0)->where('status',1)->where('time_type','like','%|'.$dataTime.'|%')->where('start_time','<=',$startTime)->where('stop_time','>=',$stopTime)->order('sort desc')->limit($offset,$limit)->select();
     ...
     *时间：2020.6.16
     */
    public static function seckill_count($startTime,$stopTime){
        return StoreSeckill::where('is_del',0)->where('status',1)->where('start_time','<',$startTime)->where('stop_time','>',$stopTime)->count();
    }
    /**
     * 获取所有秒杀产品
     * @param string $field
    ...
    public static function decSeckillStock($num = 0,$seckillId = 0){
        $res = self::where('id',$seckillId)->where('stock','>=',$num)->dec('stock',$num)->inc('sales',$num)->update();
        return $res;
    }
    ...
    public static function incSeckillStock($num = 0,$seckillId = 0){
        $res = self::where('id',$seckillId)->where('sales','>=',$num)->inc('stock',$num)->dec('sales',$num)->update();
        return $res;
    }
}