...
class StoreBargain extends ModelBasic
{
    ...
    {
        $res = self::where('id',$bargainId)->where('stock','>=',$num)->dec('stock',$num)->inc('sales',$num)->update();
        return $res;
    }
    ...
    {
        $res = self::where('id',$bargainId)->where('sales','>=',$num)->inc('stock',$num)->dec('sales',$num)->update();
        return $res;
    }
