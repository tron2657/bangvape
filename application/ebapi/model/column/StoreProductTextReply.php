...
class StoreProductTextReply extends ModelBasic
{
    use ModelTrait;
    ...
    public static function reply($group,$type = 'is_zg')
    {
...
//        });
    ...
    {
        $data['reply']=self::setProductReplyWhere($gid)->limit(2)->select();
        $data['comment_num']=self::setProductReplyWhere($gid)->count();
        $data['reply_chance']=self::setProductReplyWhere($gid,1)->count();
        return $data;
    }
    static public function getProductReplyTextList($tid,$page=1,$size=10000)
    {
        $field = 'tl.id,tl.comment,tl.add_time,tl.is_zan,tl.merchant_reply_content,tl.merchant_reply_time,u.avatar,u.nickname,u.uid';
        $model = new self;
        $model = $model->alias('tl');
        $model = $model->join('User u','u.uid=tl.uid');
        $model = $model->where('is_del',0)->where('tl.tid',$tid)
                    ->field($field)
                    ->order('tl.id DESC')
