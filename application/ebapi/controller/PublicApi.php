...
class PublicApi extends AuthController
{
    ...
    public function get_user_score()
    {
        ...
        $cart_id=osx_input('cart_id',0,'text');
        $cart_id=explode(',',$cart_id);
        $cart_info=db('store_cart')->where(['id'=>['in',$cart_id]])->field('id,product_id,product_attr_unique,cart_num')->select();
        ...
        $product_id=array_column($cart_info,'product_id');
        $product=db('store_product')->where(['id'=>['in',$product_id]])->field('id,price')->select();
        ...
        //判断最多有多少积分价格可以兑换
        $money=0;
        $cartGroup = \app\ebapi\model\store\StoreCart::getUserProductCartList($this->userInfo['uid'], $$cart_id, 1,'product',$this->isVip);          
        $money=abs($cartGroup['priceGroup']['totalPrice']*$score_limit*$vip_discount);
        
        // foreach ($cart_info as $v){
        //     $m_discount=$vip_discount;

        //     if($v['product_attr_unique']){
        //         list($attrDetail,$values)=\app\ebapi\model\store\StoreProductAttr::getProductAttrDetail($v['product_id']);
        //         $product_money=0;
        //         foreach ($values as $vo){
        //             if($vo['unique']==$v['product_attr_unique']){
        //                 $product_money=$vo['price']*$v['cart_num']*$m_discount;
        //             }
        //         }
        //     }else{
        //         $product_money=$product[$v['product_id']]['price']*$m_discount;
        //     }
        //     if($product_money>$score_full){
        //         if($product_money>$score_max&&$score_limit!=0){
        //             $money+=floor($score_max)*$v['cart_num'];
        //         }else{
        //             $money+=floor($product_money)*$v['cart_num'];
        //         }
        //     }
        // }
        //最多多少积分
        ...
        $use_score_now['score_on']=SystemConfig::getValue('score_on');
        // $use_score_now['score_buy_name']=db('system_rule')->where('flag','buy')->value('name');
        ...
        // }
        // $use_score_now['cash']=$can_use_score;
        // return $this->successful($use_score_now);
    }
    ...
    * */
    ...
    public function get_cart_limit(){
        $data=SystemConfig::getValue('cart_limit');
        return $this->successful($data);
    }
