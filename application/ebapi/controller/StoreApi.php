...
class StoreApi extends AuthController
{
     ...
     */
    /**
     * @api {post} /ebapi/store_api/get_product_list 花间一壶酒活动-兑酒流程1.商品列表
    ...
    public function get_product_list()
    {
        ...
        $coupon=MemberCouponPlan::get_coupon_attach($data['couponId']);
        if(!$coupon) return JsonService::fail('这不是您的优惠券');

        $userCoupon=StoreCouponUser::getCouponById($data['couponId']);
        $now_time=time();
        if($userCoupon['add_time']>$now_time||$userCoupon['end_time']<$now_time) return JsonService::fail('此优惠券不在使用时间期内');
        
        // $product_ids=$coupon->limit_product_ids;
        
        $product_idsstr = join(",", $coupon->limit_product_ids);
        $data['product_ids']=$product_idsstr;
        if($data['product_ids']==''){
            //免费商品
            $event=Event::getEvent($coupon->event_id);
            if(!$event) return JsonService::fail('活动不能为空');
            $data['product_ids']=$event['product_id'];
            if(empty($event['product_id'])) return JsonService::fail('活动商品为空');
        }

        $product_list=StoreProduct::getProductList($data,$this->uid,$access);
    ...
    /**
     * @api {post} /ebapi/store_api/details 花间一壶酒活动-兑酒流程2.商品详情页
     * @apiName details
     * @apiGroup 花间一壶酒活动
     * @apiHeader {string} Access-Token=4bb116d3-d820-47ce-a04b-b9860f8792a2
     * @apiHeader {string} content-type=application/x-www-form-urlencoded;charset=utf-8
    ...
    public function details(){
        ...
        if(!$coupon) return JsonService::fail('您不能使用此优惠券');
        $event=Event::getEvent($coupon->event_id);

        $userCoupon=StoreCouponUser::getCouponById($couponId);
        $now_time=time();
        if($userCoupon['add_time']>$now_time||$userCoupon['end_time']<$now_time) return JsonService::fail('此优惠券不在使用时间期内');
        $data['address']=$event['province'].$event['city'].$event['district'].$event['address'];
        $data['lat']=$event['lat'];
        $data['lng']=$event['lng'];
        ...
        // $data['storeInfo'] = StoreProduct::setVipPrice($storeInfo,$this->userInfo);
