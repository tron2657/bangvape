...
class StoreProductRelation extends ModelBasic
{
     ...
     * @return bool
     */
    public static function productRelation($productId,$uid,$relationType,$category = 'product',$is_zg)
    {
        if(!$productId) return self::setErrorInfo('产品不存在!');
        $relationType = strtolower($relationType);
        $category = strtolower($category);
        $data = ['uid'=>$uid,'product_id'=>$productId,'type'=>$relationType,'category'=>$category,'is_zg'=>$is_zg];
        ...
        Gong::actionadd('shangpinshoucang','store_product_relation','uid');
        return true;
    }
     ...
     * @return bool
     */
    public static function productRelationAll($productIdS,$uid,$relationType,$category = 'product',$is_zg){
        $res = true;
        if(is_array($productIdS)){
            self::beginTrans();
            foreach ($productIdS as $productId){
                $res = $res && self::productRelation($productId,$uid,$relationType,$category,$is_zg);
            }
            self::checkTrans($res);
            return $res;
        }
    ...
    public static function unProductRelation($productId,$uid,$relationType,$category = 'product')
    {
        ...
        Gong::delaction('shangpinshoucang',$uid,'取消商品收藏');
        return true;
    }
    ...
    public static function getUserCollectProduct($uid,$page,$limit)
    {
        $list = self::where('A.uid',$uid)
            ->field('B.id pid,B.store_name,B.price,B.ot_price,B.sales,B.image,B.is_del,B.is_show,A.*')->alias('A')
        ...
        foreach ($list as $k=>&$product){
            ...
            $product['image_150']=thumb_path($product['image'],150,150);
            $product['image_350']=thumb_path($product['image'],350,350);
            $product['image_750']=thumb_path($product['image'],750,750);
        }
        return $list;
    }
