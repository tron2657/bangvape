...
use app\columnapi\model\column\ColumnCouponUser;
use app\columnapi\model\column\ColumnProductReply;
...
use app\columnapi\model\user\User;
use app\columnapi\model\user\UserAddress;
...
class StoreOrder extends ModelBasic
{
        ...
        if($score_num>0){
            ...
            if($score_on==1){
                ...
                if($payPrice>=$score_full){
                    ...
                    $score_limit=SystemConfig::getValue('score_limit');
    ...
    public static function ZgCreateOrderNew($uid,$key,$addressId=0,$useIntegral = false,$couponId = 0,$mark = '',$combinationId = 0,$pinkId = 0,$seckill_id=0,$bargain_id=0,$is_zg=1)
    {
        ...
        if($couponId){
            // $couponInfo = StoreCouponUser::validAddressWhere()->where('id',$couponId)->where('uid',$uid)->find();
            $couponInfo = ColumnCouponUser::validAddressWhere()->where('id',$couponId)->where('uid',$uid)->find();
            if(!$couponInfo) return self::setErrorInfo('选择的优惠劵无效!');
            if($couponInfo['use_min_price'] > $payPrice)
                return self::setErrorInfo('不满足优惠劵的使用条件!');
            $payPrice = (float)bcsub($payPrice,$couponInfo['coupon_price'],2);
            // $res1 = StoreCouponUser::useCoupon($couponId);
            $res1 = ColumnCouponUser::useCoupon($couponId);
            $couponPrice = $couponInfo['coupon_price'];
        }else{
            $couponId = 0;
            $couponPrice = 0;
        ...
        foreach ($cartInfo as $cart){
            $cartIds[] = $cart['id'];
            $totalNum += $cart['cart_num'];
            $gainIntegral = bcadd($gainIntegral,isset($cart['productInfo']['score']) ? : 0,2);

            $user_order_one['amount']=$priceGroup['totalPrice'];
            $user_order_one['unique']=md5($order_id.$cart['productInfo']['id']);
            $user_order_one['info']='购买'.$cart['productInfo']['name'];
        ...
        $orderInfo = [
            ...
            'unique'=>$key,
            'is_zg'=>1,
            'is_column'=>$cartInfo[0]['productInfo']['is_column'],
        ];
        $order = self::set($orderInfo,true);
        if(!$order->result)return self::setErrorInfo('订单生成失败!');
        //新订单的功能
			...
			if( $is_zg==1 ){
				...
				if($invite_level){
						...
						foreach ($cartInfo as $cart)
						{
							...
							$goods_title[]='['.(isset($cart['productInfo']['store_name'])?$cart['productInfo']['store_name']:(isset($cart['productInfo']['name'])?$cart['productInfo']['name']:'')).$cart['productInfo']['attrInfo']['suk'].'×'.$cart['cart_num'].']';
							$back_money=bcadd($back_money,bcmul($cart['trueStrip_num'],$cart['cart_num'],2),2);
						}
						unset($cart);
    ...
    public static function RegressionStock($order)
    {
        ...
        {
            //增库存减销量
            $res5 =  ColumnText::where('id',$cart['cart_info']['productInfo']['id'])->where('sales','>=',$cart['cart_info']['cart_num'])->inc('stock',$cart['cart_info']['cart_num'])->dec('sales',$cart['cart_info']['cart_num'])->update();
        }
        return $res5!==false;
    }
        ...
        if($res){
            ...
            if($res2){
                ...
                WechatTemplateService::sendTemplate(WechatUser::uidToOpenid($order['uid']),WechatTemplateService::ORDER_CANCEL, [
                    ...
                    'keyword2'=>$order['cartInfo'][0]['productInfo']['name'].'...',
...
//        }
    ...
    public static function getUserOrderList($uid,$status = '',$page = 0,$limit = 8)
    {
        $list = self::statusByWhere($status,$uid)->where('is_del',0)->where('uid',$uid)->where('is_zg',0)
            ->field('add_time,delivery_time,seckill_id,bargain_id,combination_id,id,order_id,pay_price,total_num,total_price,pay_postage,total_postage,paid,status,refund_status,pay_type,coupon_price,deduction_price,pink_id,delivery_type,delivery_name,delivery_id,no_delivery')
    ...
    public static function checkOrderOver($oid)
    {
        ...
        if(ColumnProductReply::where('unique','IN',$uniqueList)->where('oid',$oid)->count() == count($uniqueList)){
    ...
    public static function getUserOrderSearchListZg($uid,$type,$page,$limit,$search)
    {
        ...
        foreach ($list as $k=>$order){
            ...
            if($list[$k]['_status']['_type'] == 3){
                foreach ($order['cartInfo']?:[] as $key=>$product){
                    $list[$k]['cartInfo'][$key]['is_reply'] = ColumnProductReply::isReply($product['unique'],'product');
                    $list[$k]['cartInfo'][$key]['add_time'] = date('Y-m-d H:i',$product['add_time']);
                }
            }
            $time=time()-86400;
