...
use app\columnapi\model\store\StoreOrder;
...
class User extends ModelBasic
{
    ...
    {
        $field='uid,nickname,phone,avatar,sign_num,status,level,user_type,pay_count,fans,follow,sex,birthday,signature,total_sign,support_count,introduction,forum_count,post_count,exp,fly,gong,buy,one,two,three,four,five,is_collect,mark,icon,is_red,collect';
     ...
     * 获取用户下级推广人
     * @param int $uid  当前用户
     * @param int $grade 等级  1  一级 2 二级
     * @param string $orderBy  排序
     * @return array|bool|void
     */
    public static function getUserSpreadGrade($uid = 0,$grade = 1,$orderBy = '',$keyword = '',$offset = 0,$limit = 20){
        $userTotal = self::column('uid');
        foreach ($userTotal as &$item) self::setUserSpreadCount($item);
        if(!$uid) return [];
        $gradeGroup = [1,2];
        ...
        if($grade == 1) return self::getUserSpreadCountList(implode(',',$userStair),$orderBy,$keyword,$offset,$limit);
