...
class ColumnProductReply extends ModelBasic
{
    ...
    public static function setProductReplyWhere($productId,$type=0,$alias='A')
    {
        ...
        switch ($type){
            case 1:
                ...
                break;
            case 2:
                $model=$model->where('A.product_score',['<',5],['>',1]);//中评
                break;
            case 3:
    ...
    public static function setProductReplyWheres($productId,$type=0,$alias='A')
    {
        ...
        //'.$icon_field.'B.is_red,
        $model = db('column_product_reply')->alias('A')->where('A.is_del', 0)->where('A.product_id',$productId)->join('__USER__ B','A.uid = B.uid')
            ->join('__STORE_ORDER_CART_INFO__ C','A.unique = C.unique')
        ...
        /*$model = self::productValidWheres($alias)->where('A.product_id',$productId)
            ->field('A.product_score,A.service_score,A.comment,A.merchant_reply_content,A.merchant_reply_time,A.pics,A.add_time,B.nickname,B.avatar,B.uid,B.exp,C.cart_info,A.merchant_reply_content')->join('__USER__ B','A.uid = B.uid')
            ->join('__STORE_ORDER_CART_INFO__ C','A.unique = C.unique');*/
        switch ($type){
            case 1:
                $model=$model->where('A.product_score',5);//好评
    ...
    public static function getProductReplyList($productId,$order = 0,$page = 0,$limit = 8)
    {
        ...
        $map['is_del']=0;
        $list =db('store_product_reply')->Where($map)->order('add_time desc')->page((int)$page,(int)$limit)->select();
    ...
    {
        $list = self::setProductReplyWheres($productId,$order)->page((int)$page,(int)$limit)->select()?:[];
        foreach ($list as $k=>$reply){
    ...
    public static function tidyProductReply($res)
    {
        ...
        $res['pics'] = empty($res['pics']) ? [] : explode(',', $res['pics']);
        unset($res['cart_info']);
        return $res;
    }
