...
class WechatUser extends ModelBasic
{
    ...
    public static function setNewUser($openid)
    {
        ...
        self::beginTrans();
        $wechatUser = self::set($userInfo,true);
        if(!$wechatUser->result){
            self::rollbackTrans();
            exception('用户储存失败!');
        }
     ...
     * @return mixed
     */
    public static function getMiniOpenId($uid = ''){
        if($uid == '') return false;
        return db('user_sync_login')->where('uid',$uid)->value('mini_open_id');
    }
