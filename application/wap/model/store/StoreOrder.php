...
class StoreOrder extends ModelBasic
{
    ...
    public static function jsPay($orderId,$field = 'order_id')
    {
        ...
        return WechatService::jsPay($openid,$orderInfo['order_id'],$orderInfo['pay_price'],'product',SystemConfigService::get('website_name'));
    }
    ...
    public static function paySuccess($orderId)
    {
        ...
        if($order['is_zg']==1){//生成购买记录
            $pid=StoreOrderCartInfo::where('oid',$oid)->value('product_id');
            ...
            db('column_text')->where('id',$pid)->setInc('sales');
        }

        //新的支付统计中的内容绑定
        $res2=db('user_order')->where(['order_id'=>$orderId])->update(['pay_type'=>$order['pay_type'],'status'=>1,'pay_time'=>time()]);
        ...
        RoutineTemplateService::sendTemplate(WechatUser::getMiniOpenId($order['uid']),RoutineTemplateService::REFUND_SUCCESS, [
            'character_string4'=>['value'=>$order['order_id']],
            'amount1'=>['value'=>$order['pay_price']],
            'thing6'=>['value'=>$goodsName],
            'time7'=>['value'=>date('Y/m/d H:i',time())],
    ...
    public static function createOrderTemplate($order)
    {
        ...
        WechatTemplateService::sendTemplate(WechatUser::uidToOpenid($order['uid']),'', [
            'first'=>'亲，您购买的商品已支付成功',
