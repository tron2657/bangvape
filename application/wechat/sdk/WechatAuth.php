...
class WechatAuth {
    ...
    public function getAccessToken($type = 'client', $code = null){
        ...
        $access_token = Cache::get('weixin_access_token_'.$this->appId);
        if ($access_token && $type != 'code') return ['access_token' => $access_token];
        ...
        if(is_array($token)){
            if(isset($token['errcode'])){
                ...
                if ($type != 'code') {
                    Cache::set('weixin_access_token_'.$this->appId, $token['access_token'], 3600);
                }
