...
class MiniProgramService
{
    ...
    public static function options()
    {
        ...
        if(isset($payment['pay_weixin_open']) && $payment['pay_weixin_open'] == 1){
            $config['payment'] = [
                ...
                'notify_url'=>$wechat['site_url'].Url::build('/ebapi/notify/notify',['pay_type'=>'routine'])
            ];
        }
        return $config;
    }
     ...
     */
    public static function refund($orderNo, $refundNo, $totalFee, $refundFee = null, $opUserId = null, $refundReason = '' , $type = 'out_trade_no', $refundAccount = 'REFUND_SOURCE_UNSETTLED_FUNDS',$pay_type)
    {
        $totalFee = floatval($totalFee);
        $refundFee = floatval($refundFee);
        return self::paymentService()->refund($orderNo,$refundNo,$totalFee,$refundFee,$opUserId,$type,$refundAccount,$refundReason,$pay_type);
    }
     ...
     * @return bool
     */
    public static function payOrderRefund($orderNo, array $opt,$pay_type)
    {
        ...
        try{
            $res = (self::refund($orderNo,$refundNo,$totalFee,$refundFee,$opUserId,$refundReason,$type,$refundAccount,$pay_type));
