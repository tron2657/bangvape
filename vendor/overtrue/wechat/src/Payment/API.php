...
class API extends AbstractAPI
{
    ...
    public function refund(
        ...
        return $this->safeRequest($this->wrapApi(self::API_REFUND), $params,'post',$pay_type);
    }
     ...
     * @return \EasyWeChat\Support\Collection
     */
    protected function safeRequest($api, array $params, $method = 'post',$pay_type)
    {
        if($pay_type=='weixin'){
            $cert=realpath('.'.SystemConfig::getValue('pay_weixin_client_cert'));
            $key=realpath('.'.SystemConfig::getValue('pay_weixin_client_key'));
        }else{
            $cert=realpath('.'.SystemConfig::getValue('pay_routine_client_cert'));
            $key=realpath('.'.SystemConfig::getValue('pay_routine_client_key'));
        }
        $options = [
            'cert' => $cert,
            'ssl_key' => $key,
